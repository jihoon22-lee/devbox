...
| Notices | 두 lockfile + package metadata | 629 Rust package와 151 frontend runtime package의 version/license/source/digest를 결정적으로 재생성해 checked-in 파일과 byte 비교 |
...
현재 `THIRD_PARTY_NOTICES.md`는 128,169 bytes다. installer에서는 동일 파일을 압축 resource로
