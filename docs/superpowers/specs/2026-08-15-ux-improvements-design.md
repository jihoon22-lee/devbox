...
전 앱에 `onContextMenu` 핸들러가 0개라 우클릭은 웹뷰 기본 반응(또는 무반응)에 그치고,
wsl-desktop은 Alt+Arrow 팬 이동(#189)은 있지만 복사·붙여넣기 단축키가 없어
실질적으로 불편하다. 이 문서는 그 격차를 메우는 작업을 정리한다.
...
`lib/shortcuts.ts`는 탭/팬 이동(Ctrl+Shift+T/D/W, Ctrl+Tab, Alt+Arrow 팬 포커스 — #189)은
처리하지만, 복사·붙여넣기·선택은 xterm.js 기본값에 의존한다. Tauri 웹뷰에서는 브라우저
