...
export async function exerciseWorkspaceRuntime({
  cdp,
  directory,
  call,
  success,
  ...
  const root = path.join(directory, "Runtime owned fixture");
  mkdirSync(root);
  const script = path.join(root, "listener.cjs"),
    ready = path.join(root, "ready.json"),
    starts = path.join(root, "starts.txt");
  writeFileSync(
    script,
...
if(process.argv.includes('--child')){
 ...
 cp.fork(__filename,['--child'],{stdio:'inherit'});console.log('synthetic-parent-ready');setInterval(()=>{},1000);
  ...
  const until = async (action, message, timeout = 20000) => {
  ...
  const navigate = (route) =>
    cdp.evaluate(
      `(async()=>{const d=await window.__TAURI_INTERNALS__.invoke("plugin:product-shell|describe");const label=d.features.find(feature=>feature.route===${JSON.stringify(route)}).label;Array.from(document.querySelectorAll('nav[aria-label="제품 화면"] button')).find(button=>button.textContent.trim()===label).click();})()`,
    );
  const job = success(
  ...
  try {
    ...
    let rejected = false;
    try {
      await runtime("run_job_now", { id: job.id });
    } catch {
      rejected = true;
    }
    assert.ok(rejected);
    const operationId = randomUUID();
    const first = success(await control("run_job_now", { id: job.id }, operationId));
    const second = success(await control("run_job_now", { id: job.id }, operationId));
    assert.deepEqual(second, first);
    const child = await until(
      async () => existsSync(ready) && JSON.parse(readFileSync(ready, "utf8")),
      "Owned child did not listen",
    );
    assert.equal(readFileSync(starts, "utf8"), "start\n");
    const run = await until(
      async () => success(await runtime("get_active_run", { id: job.id })),
      "Active run is missing",
    );
    const findOwned = async () => {
      const snapshot = success(await observe());
      const row = snapshot.rows.find(
        (row) => row.pid === child.pid && row.port === child.port && row.local_addr === `127.0.0.1:${child.port}`,
      );
    ...
    await waitForRenderer(
      cdp,
      '!!document.querySelector(".workspace-feature-runtime:not([hidden])")',
      "Runtime route is missing",
    );
    success(
      await call("workspace.processes", "open_port_owner", { actionKey: observed.correlation.action_key }, 29000),
    );
    await waitForRenderer(
      cdp,
      '!!document.querySelector(".workspace-feature-tasks:not([hidden])")',
      "Owning task navigation failed",
    );
    const endpoint = {
      proto: observed.row.proto,
      local_addr: observed.row.local_addr,
      port: observed.row.port,
      state: observed.row.state,
    };
    const broker = success(
      await call(
        "workspace.process-actions",
        "kill_listener",
        { request: { endpoint, identity: observed.row.identity } },
        29000,
      ),
    );
    ...
    failed(
      await call("workspace.process-actions", "kill_listener", { request: { endpoint, identity: mismatch } }, 29000),
    );
    ...
    const resolved = success(
      await logs("reconnect_runtime_sources", {
        sources: [{ kind: "runtimeRun", runId: run.id, stream: "stdout", revision: "1".repeat(64) }],
        filter: { text: "", regex: false },
      }),
    );
    assert.equal(resolved.unavailableSources, 0);
    assert.match(resolved.sources[0].revision, /^[a-f0-9]{64}$/);
    const read = () =>
      logs("read_sources", {
    ...
    const snapshot = success(await read());
    assert.ok(JSON.stringify(snapshot).includes("synthetic-child-ready"));
    await navigate("runtime");
    success(
      await call(
        "workspace.processes",
        "open_port_log",
        { actionKey: observed.correlation.action_key, stream: "stdout" },
        29000,
      ),
    );
    await waitForRenderer(
      cdp,
      '!!document.querySelector(".workspace-feature-logs:not([hidden])")',
      "Runtime log navigation failed",
    );
    ...
    success(await control("stop_active_run", { id: job.id }));
    stopped = true;
    await until(async () => {
      try {
        await fetch(`http://127.0.0.1:${child.port}`, { signal: AbortSignal.timeout(500) });
        return false;
      } catch {
        return true;
      }
    }, "Stopped Runtime left a child listener");
    failed(
      await call(
        "workspace.processes",
        "open_port_log",
    ...
    const terminal = success(await read());
    assert.ok(
      JSON.stringify(terminal).includes("synthetic-child-ready"),
      "Terminalization changed a retained log reference",
    );
    if (measurePerformance) {
      const measuredJob = success(
        await runtime("create_job", {
          input: {
            name: "Owned task latency",
            command: "exit /b 0",
            cwd: root,
            targetKind: "windows",
            targetDistro: null,
      ...
      try {
        ...
        const completed = await until(
          async () => {
            const runs = success(await runtime("list_runs", { jobId: measuredJob.id, limit: 1 }));
            return runs[0]?.endedAt != null && runs[0];
          },
          "Measured owned task did not finish",
          30000,
        );
        assert.equal(completed.exitCode, 0);
        const completeMs = Math.round(performance.now() - started);
        const budget = JSON.parse(readFileSync(".github/scripts/product-foundation-performance.json", "utf8")).budgets
          .ownedTaskCompleteMs;
        assert.ok(completeMs <= budget);
        writeFileSync(
          "product-foundation-evidence/workspace-task-performance.json",
          JSON.stringify(
            {
              source: process.env.GITHUB_SHA,
              build: "exact candidate release",
              kind: "disabled-windows-job-explicit-owned-execution",
              completeMs,
              budget,
              result: "passed",
            },
    ...
    return {
      ...
      rendererReload: true,
      ownedDescendant: true,
      exactCreationMismatch: true,
      ownerBrokerRoutesWithoutKill: true,
      listenerLogTaskNavigation: true,
      terminalLogLease: true,
      staleActionRejected: true,
      stopWaitsForDescendants: true,
    };
  } catch (error) {
    ...
    try {
      ...
      for (const run of runs)
        stderr.push({
          runId: run.id,
          value: success(
            await runtime("tail_log", { input: { runId: run.id, stream: "stderr", cursor: null, maxBytes: 8192 } }),
          ),
        });
      writeFileSync(
        "product-foundation-evidence/workspace-runtime-failure.json",
        JSON.stringify({ source: process.env.GITHUB_SHA, environment: "github-hosted-windows", runs, stderr }, null, 2),
      );
    } catch {
