...
const discover = (id) => ({
  ...
  result: {
    ...
    ttlMs: 60000,
    cacheScope: "public",
    _meta: { "io.modelcontextprotocol/serverInfo": { name: "owned fixture", version: "1" } },
  },
});
const template = (url) => ({
  method: "GET",
...
const descriptor = protobuf(
  field(1, "control.proto"),
  field(2, "fixture"),
  field(4, field(1, "Empty")),
  field(
    6,
    protobuf(
      field(1, "Control"),
      field(2, protobuf(field(1, "Hold"), field(2, ".fixture.Empty"), field(3, ".fixture.Empty"))),
    ),
  ),
  field(12, "proto3"),
);
...
export async function exerciseControlAdmission({ ui, root, call, success, evidence, progress }) {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (url.pathname === "/sse") {
      ...
      held.set("sse", response);
      response.on("close", () => held.delete("sse"));
      return;
    }
    ...
    if (url.pathname === "/mcp") {
      ...
      if (message.method === "server/discover") {
        ...
        response.end(JSON.stringify(discover(message.id)));
      } else {
        held.set("mcp", response);
        response.on("close", () => held.delete("mcp"));
      }
      return;
    }
    response.writeHead(404).end();
  ...
  server.on("connection", (socket) => {
    observeControlSocket(socket, socketState);
    sockets.add(socket);
  ...
  server.on("upgrade", (request, socket) => {
    ...
    held.set("websocket", socket);
    socket.on("close", () => held.delete("websocket"));
  ...
  h2.on("stream", (stream, headers) => {
    ...
    if (headers[":path"] === "/fixture.Control/Hold") {
      held.set("grpc", stream);
      stream.on("close", () => held.delete("grpc"));
      stream.resume();
      return;
    }
  ...
  const completed = async (key) => {
    await until(
      () => ui.cdp.evaluate(`window.__controlPending[${JSON.stringify(key)}].done`),
      "protocol request did not retire",
    );
    return ui.cdp.evaluate(`window.__controlPending[${JSON.stringify(key)}]`);
  };
  async function saturated(occupied, action) {
    const id = randomUUID(),
      responses = [];
    loads.set(id, responses);
    const keys = await Promise.all(
      Array.from({ length: 64 - occupied }, () =>
        pending("fetch_openapi_source", { url: `${endpoint}/load?id=${id}` }),
      ),
    );
    try {
      ...
      const stillHeld = await ui.cdp.evaluate(
        `[${keys.map((key) => `window.__controlPending[${JSON.stringify(key)}].done`).join(",")}].every(done=>!done)`,
      );
      ...
      loads.delete(id);
      await Promise.all(keys.map(completed));
    }
  }
  const recorded = [];
  ...
  async function control(method, args, occupied, released, pendingKey) {
    ...
    await saturated(occupied, async () => {
      ...
      if (pendingKey) {
        const result = await completed(pendingKey);
        assert.ok(!result.error);
        assert.notEqual(result.value.operation.outcome.state, "succeeded");
      }
    });
  ...
  try {
    let key = await pending("send_request", {
      req: template(`${endpoint}/http`),
      environment: [],
      requestId: "control-http",
    });
    ...
    for (const method of ["cancel_mcp_http", "disconnect_mcp_http"]) {
      const connection = await success("api-studio.api", "connect_mcp_http", {
        profile: { endpoint: `${endpoint}/mcp`, era: "modern", headers: [], timeoutMs: 30000 },
        environment: [],
      });
      key = await pending("invoke_mcp_http", {
        connectionId: connection.connectionId,
        requestId: "control-mcp",
        method: "tools/list",
        params: {},
      });
      ...
      await control(
        method,
        { connectionId: connection.connectionId, ...(method.startsWith("cancel") ? { requestId: "control-mcp" } : {}) },
        1,
        () => !held.has("mcp"),
        key,
      );
      if (method.startsWith("cancel"))
        await success("api-studio.api", "disconnect_mcp_http", { connectionId: connection.connectionId });
    }
    const sse = await success("api-studio.api", "start_sse_stream", {
      req: template(`${endpoint}/sse`),
      environment: [],
      options: { connectTimeoutMs: 10000, idleTimeoutMs: 30000, totalTimeoutMs: 60000, reconnect: false },
    });
    ...
    for (const method of ["close_websocket", "disconnect_websocket"]) {
      const sessionId = await success("api-studio.api", "start_websocket", {
        req: template(endpoint.replace("http:", "ws:") + "/ws"),
        environment: [],
      });
      await until(() => held.has("websocket"), "WebSocket did not connect");
      await control(
        method,
        { sessionId, ...(method === "close_websocket" ? { close: { code: 1000, reason: "owned fixture" } } : {}) },
        0,
        () => !held.has("websocket"),
      );
    }
    for (const method of ["cancel_grpc", "disconnect_grpc"]) {
      stage("grpc-connect");
      const connection = await success("api-studio.api", "connect_grpc", {
        profile: {
          endpoint: `http://127.0.0.1:${h2.address().port}`,
      ...
      key = await pending("invoke_grpc", {
        connectionId: connection.connectionId,
        requestId: "control-grpc",
        method: "fixture.Control.Hold",
        messages: ["{}"],
      });
      ...
      await control(
        ...
        {
          connectionId: connection.connectionId,
          ...(method.startsWith("cancel") ? { requestId: "control-grpc" } : {}),
        },
        1,
        () => !held.has("grpc"),
        key,
      ...
      if (method.startsWith("cancel"))
        await success("api-studio.api", "disconnect_grpc", { connectionId: connection.connectionId });
    }
    key = await pending("authorize_mcp_http", {
      requestId: "control-oauth",
    ...
    const selecting = call("api-studio.api", "pick_mcp_stdio_executable").then(
      (value) => ({ value }),
      (error) => ({ error }),
    );
    const picker = spawn(
      ...
      [
        ...
        "-NonInteractive",
        "-File",
        path.resolve(".github/scripts/windows-native-file-dialog.ps1"),
        "-TargetProcessId",
        String(ui.child.pid),
        "-ExpectedExecutable",
        ui.child.spawnfile,
    ...
    const [pickerCode] = await once(picker, "exit");
    assert.equal(pickerCode, 0);
    const selected = await selecting;
    assert.ok(!selected.error);
    const selectionResponse = selected.value;
    assert.equal(selectionResponse.operation.outcome.state, "succeeded");
    ...
    for (const method of ["cancel_mcp_stdio", "disconnect_mcp_stdio"]) {
      ...
      key = await pending("invoke_mcp_stdio", {
        connectionId: connection.connectionId,
        requestId: "control-stdio",
        method: "tools/list",
        params: {},
      });
      ...
      await control(
        ...
        {
          connectionId: connection.connectionId,
          ...(method.startsWith("cancel") ? { requestId: "control-stdio" } : {}),
        },
        1,
        () => {
          try {
    ...
    if (socketState.failure) throw socketState.failure;
    evidence.controlAdmission = {
      normalSlots: 64,
      released: recorded,
      expectedDisconnects: socketState.expectedDisconnects,
      result: "pass",
    };
