...
test("multiplexer busy polling repeats only bounded read-only requests with fresh IDs", async () => {
  for (const method of ["list_sessions", "terminal_layout", "write_session", "save_terminal_layout"]) {
      ...
      window: {
        __TAURI_INTERNALS__: {
          invoke: async (command, input) => {
            if (command === "plugin:workspace|terminal_describe")
              return { handshake: { installationId: "i", sessionId: "s" }, context: null };
            calls++;
            requests.push(JSON.parse(JSON.stringify(input.request)));
    ...
    if (["write_session", "save_terminal_layout"].includes(method)) {
      await assert.rejects(result, /busy/);
      assert.equal(calls, 1);
    } else {
      assert.equal(await result, "observed");
      assert.equal(calls, 3);
      assert.equal(new Set(requests.map((r) => r.header.requestId)).size, 3);
      assert.ok(requests.every((r) => r.header.deadlineMs === 30000));
    }
  }
        ...
        window: {
          __TAURI_INTERNALS__: {
            invoke: async (command) => {
              if (command === "plugin:workspace|terminal_describe")
                return { handshake: { installationId: "i", sessionId: "s" }, context: null };
              if (++calls > 20) throw "unbounded";
              throw problem;
            },
          },
        },
        crypto: { randomUUID: () => String(calls) },
        Date: { now: () => 1000 },
        setTimeout: (callback) => callback(),
      }),
      new RegExp(problem),
...
test("native registration probe executes generated requests with the described context", async () => {
  for (const context of [
    null,
    { projectId: "project", worktreeId: "tree", revision: 2, target: { kind: "windows" } },
  ]) {
    const calls = [];
        ...
        __TAURI_INTERNALS__: {
          invoke: async (command, input) => {
            ...
            if (command === "plugin:product-shell|describe")
              return { handshake: { installationId: "installation", sessionId: "session" }, context };
...
test("product renderer probes contain valid decoded JavaScript expressions", () => {
  ...
  const fixturePaths = [
    ...workspaceFixtures.map((name) => `./windows-workspace-${name}.mjs`),
    "./windows-product-foundation.mjs",
    "./windows-knowledge-lifecycle.mjs",
  ];
  for (const relative of fixturePaths) {
    ...
    function visit(node) {
      if (ts.isCallExpression(node)) {
        ...
        const argument =
          ts.isIdentifier(callee) && callee.text === "waitForRenderer"
            ? node.arguments[1]
            : ts.isPropertyAccessExpression(callee) && callee.name.text === "evaluate"
              ? node.arguments[0]
              : undefined;
        if (literal(argument)) {
...
test("long feature fixture requests select their own route inside the native deadline ceiling", async () => {
  for (const [component, route] of [
    ...
    await runInNewContext(
      workspaceRequestExpression(
        component,
        "fixture_method",
        { request: { path: "C:\\fixture" } },
        ["workspace.terminal", "workspace.problems"].includes(component) ? 29000 : undefined,
      ),
      {
        window: {
          __TAURI_INTERNALS__: {
            invoke: async (command, input) =>
              command === "plugin:product-shell|describe"
                ? { handshake: { installationId: "installation", sessionId: "session" }, context: null }
                : (sent = input.request),
          },
        },
        crypto: { randomUUID: () => "request" },
...
test("native component rejection retains its structured problem in renderer diagnostics", async () => {
  await assert.rejects(
    runInNewContext(workspaceRequestExpression("workspace.source", "trust_status"), {
      window: {
        __TAURI_INTERNALS__: {
          invoke: async (command) => {
            if (command === "plugin:product-shell|describe")
              return { handshake: { installationId: "installation", sessionId: "session" }, context: null };
...
test("only pre-admission busy rejection retries with fresh IDs and a fixed context/deadline", async () => {
  ...
  const result = await runInNewContext(
    workspaceRequestExpression("workspace.lsp", "save_lsp_config", { nativeRevision: "old" }),
    {
      window: {
        __TAURI_INTERNALS__: {
          invoke: async (command, input) => {
            if (command === "plugin:product-shell|describe")
              return { handshake: { installationId: "i", sessionId: "s" }, context };
  ...
  assert.equal(result.value.issue, "lsp_config_changed");
  assert.equal(attempts.length, 3);
  assert.deepEqual(
    attempts.map((request) => request.header.requestId),
    ["1", "2", "3"],
  );
  for (const request of attempts) {
    assert.deepEqual(request.header.context, context);
    assert.equal(request.header.deadlineMs, 30000);
    assert.deepEqual(request.args, { nativeRevision: "old" });
  }
});
test("busy retries are bounded and never repeat accepted or ambiguous failures", async () => {
  const busy = {
    code: "unavailable",
    provenance: { product: "workspace", component: "workspace.dispatch", requestId: "rejected", revision: 1 },
  };
  for (const [failure, expected] of [
    [busy, 20],
    [{ ...busy, provenance: { ...busy.provenance, requestId: "accepted-request" } }, 1],
    [{ code: "unauthorized" }, 1],
    [new Error("transport failed"), 1],
  ]) {
    let calls = 0;
    await assert.rejects(
      runInNewContext(
        workspaceRequestExpression("workspace.registry", "apply_registration", { previewId: "one-use" }),
        {
          window: {
            __TAURI_INTERNALS__: {
              invoke: async (command) => {
  ...
  assert.equal(
    await runInNewContext(workspaceRequestExpression("workspace.registry", "apply_registration"), {
      window: {
        __TAURI_INTERNALS__: {
          invoke: async (command) => {
            if (command === "plugin:product-shell|describe")
...
test("native editor trace observes IPC with Tauri's immutable invoke and preserves original responses", async () => {
  ...
  const originalInvoke = () => "immutable",
    internals = {};
  ...
  assert.equal(rows[0].phase, "succeeded");
  assert.equal(rows[0].method, "save_lsp_document");
  assert.deepEqual(Object.keys(rows[0]).sort(), ["elapsedMs", "method", "phase"]);
  assert.ok(!JSON.stringify(rows).includes("private"));
  window.__workspaceLspTrace.restore();
...
test("native editor trace ignores unrelated bodies and retains only static failure metadata", async () => {
  const { installWorkspaceEditorTrace } = await import("./windows-workspace-lsp.mjs");
  const value = {
    operation: { outcome: { state: "failed" } },
    value: { issue: "file_snapshot_changed", private: "synthetic response" },
  };
  ...
  while (window.__workspaceLspTrace.rows[0]?.phase === "pending" && Date.now() < deadline)
    await new Promise((resolve) => setTimeout(resolve, 5));
  const row = window.__workspaceLspTrace.rows[0];
  assert.equal(row.phase, "failed");
  assert.equal(row.issue, "file_snapshot_changed");
...
test("bounded writer-wait fixtures retain their short original deadline", async () => {
  let sent;
  await runInNewContext(
    workspaceRequestExpression("workspace.files", "save_file", { request: { path: "C:\\fixture" } }, 500),
    {
      window: {
        __TAURI_INTERNALS__: {
          invoke: async (command, input) =>
  ...
  for (const budget of [0, 99, 29001, Infinity, "500"])
    assert.throws(() => workspaceRequestExpression("workspace.files", "save_file", {}, budget));
});
        ...
        __TAURI_INTERNALS__: {
          invoke: async (command, input) => {
            if (command === "plugin:product-shell|describe")
              return { handshake: { installationId: "i", sessionId: "s" }, context: null };
            calls++;
            requests.push(input.request.header.requestId);
            if (mode === "transient" && calls === 3)
              return { operation: { outcome: { state: "succeeded" } }, value: { projects: [] } };
            return {
              operation: {
                provenance: {
                  product: "workspace",
                  component: "workspace.registry",
                  requestId: mode === "foreign" ? "other" : input.request.header.requestId,
                },
...
test("Quick Summon preserves receipt replay and permits one show before hide after focus changes", async () => {
  for (const states of [[false], [true, false]]) {
    ...
    let focuses = 0;
    const observed = await summonHiddenFixture(
      async (id) => {
    ...
    assert.equal(receipts.size, states.length);
    assert.equal(focuses, states.length - 1);
  }
  const ids = new Set();
  await assert.rejects(
    summonHiddenFixture(
