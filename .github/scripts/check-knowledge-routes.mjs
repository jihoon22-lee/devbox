...
const spec = {
  ...
  entry: "/notes/App.tsx",
  budget: { rawBytes: 970000, gzipBytes: 325000 },
  deferred: [{ suffix: "/activity/App.tsx" }, { suffix: "/search/App.tsx" }, { name: "mermaid.core" }],
  forbiddenMarkers: ["mermaidAPI"],
};
export function measureNotesRoute(manifest, readAsset) {
...
function selfTest() {
  ...
  assert.throws(
    () => measureNotesRoute({ ...manifest, "index.html": { ...manifest["index.html"], imports: ["mermaid"] } }, read),
    /eagerly loads/,
  );
...
if (isInvoked(import.meta.url)) {
  ...
  else reportBuiltRoute("knowledge", measureNotesRoute, spec);
}
