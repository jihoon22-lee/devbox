...
export function binary(file) {
  ...
  assert.ok(
    Number.isSafeInteger(offset) && size >= 56 && count > 0 && count < 1024 && offset + size * count <= bytes.length,
    "invalid ELF program headers",
  );
  for (let index = 0; index < count; index++)
    assert.notEqual(bytes.readUInt32LE(offset + size * index), 3, "helper must not require a dynamic interpreter");
...
export function run(mode, args, destination = directory) {
  if (mode === "prepare") {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        { schemaVersion: 1, protocol: 1, target: "x86_64-unknown-linux-musl", sourceSha, ...info },
        null,
        2,
      ) + "\n",
    );
    ...
    assert.deepEqual(
      ...
      ["schemaVersion", "protocol", "target", "sourceSha", "bytes", "sha256"].sort(),
    );
...
export function stageKnowledge(
  sourceSha,
  source = directory,
  destination = fileURLToPath(new URL("../../apps/devbox-knowledge/src-tauri/resources/wsl/", import.meta.url)),
) {
