param([Parameter(Mandatory=$true)][string]$Staging, [Parameter(Mandatory=$true)][string]$Bootstrap, [switch]$RemainingOnly)
...
function Wait-Until([scriptblock]$Condition) {
  ...
  while (-not (& $Condition)) {
    # NSIS's initial launcher can exit before its temporary uninstaller and
    # verified helper finish. Three retained debug generations require hashing
    # several GB; wait for the durable completion receipt, not launcher exit.
    if ($timer.Elapsed.TotalSeconds -gt 300) { throw 'Owned Suite removal did not finish within 300 seconds.' }
