...
export async function exerciseMultiplexerReconnect({ call, success, connectTerminal, wsl, distro, multiplexer }) {
  ...
  wsl([
    "/usr/bin/python3",
    "-c",
    "import pathlib,sys;p=pathlib.Path(sys.argv[1]);p.mkdir();(p/'owner').write_text(sys.argv[2])",
    root,
    id,
  ]);
  let companion,
    ...
    stage = "open",
    startupOutput = "";
  const peer = (method, args = {}) =>
    companion.evaluate(multiplexerRequestExpression(method, args), { timeoutMs: 35000 });
  ...
  try {
    ...
    const profile = {
      id,
      name: "Owned " + multiplexer + " fixture",
      tabs: [{ id: "main", title: "Fixture", layout: "grid", paneKeys: ["pane"], sizing: { columns: [1], rows: [1] } }],
      panes: [{ key: "pane", distro, cwd: root, multiplexer }],
      activeTabId: "main",
      activePaneKey: "pane",
    };
    ...
    success(await terminal("open_terminal_profile", { operationId: id, profileId: id, revision: selected.revision }));
    opened = true;
    companion = await connectTerminal(id);
    const first = await until(async () => {
      const rows = await peer("list_sessions");
      return rows.length === 1 && rows[0];
    }, "Multiplexer did not start");
    assert.equal(first.multiplexer, multiplexer);
    assert.equal(first.resumed, false);
    sessionId = first.id;
    stage = "initial-shell";
    await shellReady(sessionId);
    stage = "initial-input";
    ...
    await until(
      async () =>
        wsl([
          "/usr/bin/python3",
          "-c",
          "import pathlib,sys;p=pathlib.Path(sys.argv[1])/'proof';print(p.read_text() if p.exists() else '')",
          root,
        ]) === "first",
      "Multiplexer shell did not execute input",
    );
    ...
    success(await terminal("stop_terminal", { id }));
    opened = false;
    companion.close();
    companion = null;
    const request = { id, operationId: randomUUID(), expectedGeneration: 0 };
    const restored = success(await terminal("restore_terminal", request));
    opened = true;
    assert.equal(restored.restoreOnly, true);
    assert.deepEqual(success(await terminal("restore_terminal", request)), restored);
    companion = await connectTerminal(id);
    const second = await until(async () => {
      const rows = await peer("list_sessions");
      return rows.length === 1 && rows[0];
    }, "Multiplexer did not reconnect");
    assert.notEqual(first.id, second.id);
    assert.equal(second.multiplexer, multiplexer);
    ...
    await until(
      async () =>
        wsl([
          "/usr/bin/python3",
          "-c",
          "import pathlib,sys;print((pathlib.Path(sys.argv[1])/'proof').read_text())",
          root,
        ]) === "first\nsecond",
      "Reconnected multiplexer did not execute input",
    );
    assert.equal(
      wsl([
        "/usr/bin/python3",
        "-c",
        "import pathlib,sys;print((pathlib.Path(sys.argv[1])/'must-not-run').exists())",
        root,
      ]),
      "False",
    );
    ...
    return {
      multiplexer,
      start: true,
      detachedSessionRetained: true,
      freshPtyAttach: true,
      noStartCommandReplay: true,
    ...
    try {
      const store = success(await terminal("list_workspace_profiles"));
      if (store.profiles.some((value) => value.id === id))
        success(await terminal("delete_workspace_profile", { id, expectedRevision: store.revision }));
    } catch (error) {
      errors.push(error);
    }
    ...
    if (!opened)
      try {
        wsl([
          "/usr/bin/python3",
          "-c",
          "import pathlib,shutil,sys;p=pathlib.Path(sys.argv[1]);assert (p/'owner').read_text()==sys.argv[2];shutil.rmtree(p)",
          root,
          id,
        ]);
      } catch (error) {
    ...
    if (errors.length)
      throw new AggregateError(primary ? [primary, ...errors] : errors, "Multiplexer fixture cleanup incomplete");
  }
}
