...
const reports = runCheck({
  scope: "apps",
  frontendApps: baseline.apps.join(" "),
  root,
  config: path.join(root, ".github/scripts/frontend-bundle-budgets.json"),
});
...
writeFileSync(
  output,
  `${JSON.stringify({ schemaVersion: 1, source: baseline.baselineCommit, baselineTag: baseline.baselineTag, measurement: "initial frontend module closure from a source build at the pinned release commit; byte counts are decimal strings", reports }, (_key, value) => (typeof value === "bigint" ? value.toString() : value), 2)}\n`,
  { flag: "wx" },
);
