...
export async function exerciseOwnedContainers({ cdp, call, success, wsl, distro, nativeDockerProbe }) {
  ...
  const nonce = randomUUID(),
    root = "/tmp/devbox-container-" + nonce,
    name = "devbox-fixture-" + nonce;
  let stage = "prepare",
    lastOperation;
  const terminal = async (method, args = {}) => {
    stage = method;
    const result = await call("workspace.terminal", method, args);
    lastOperation = { method, action: args.action, result };
    return result;
  };
  ...
  try {
    ...
    const action = async (kind, id = cid, operationId = randomUUID()) => {
      ...
      success(await terminal("docker_action", args));
      return args;
    };
    const started = await action("start");
    success(await terminal("docker_action", started));
    assert.equal(
      success(await terminal("wsl_control_status", { operationId: started.operationId })).state,
      "completed",
    );
    ...
    await until(async () => {
      try {
        return await cdp.evaluate("!!document.querySelector('.workspace-registry')");
      } catch {
        return false;
      }
    }, "Workspace did not reload for Logs");
    success(await terminal("open_wsl_file_in_log_lens", { distro, wslPath: root + "/fixture.log" }));
    await until(
      async () => cdp.evaluate("(document.body.innerText||'').includes('synthetic-container-log')"),
      "WSL file did not reach the Logs surface",
    );
    await until(
      async () => success(await terminal("read_terminal_log")) === null,
      "Consumed WSL file request remained queued",
    );
    return {
      realDaemon: true,
      exactContainerId: true,
      publishedPorts: true,
      start: true,
      restart: true,
      stop: true,
      duplicateReceipt: true,
      replacedNameRejected: true,
      wslFileToLogsUi: true,
    };
  } catch (error) {
    primary = error;
    const failure = { stage, error: String(error), lastOperation };
    try {
      failure.nativeDockerProbe = nativeDockerProbe?.(cid);
    } catch (probeError) {
      failure.probeError = String(probeError);
    }
    writeFileSync("product-foundation-evidence/containers-failure.json", JSON.stringify(failure, null, 2));
    throw error;
  } finally {
    const errors = [];
    for (const id of [replacement, cid].filter(Boolean))
      try {
    ...
    if (errors.length)
      throw new AggregateError(primary ? [primary, ...errors] : errors, "Owned container cleanup incomplete");
  }
}
