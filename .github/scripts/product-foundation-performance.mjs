...
export function loadPerformanceConfig(file, tag, commit, hosted, allowSubset = false) {
  ...
  assert.deepEqual(config.knownBaselineFailures, [
    {
      app: "run-manager",
      command: "run_job_now",
      code: "run-execution-failed",
      runFailureCode: "spawn-failed",
      evidenceRun: 34099044832,
      trackingIssue: 547,
    },
  ]);
  const anchors = [
  ...
  if (allowSubset)
    assert.ok(
      config.apps.length > 0 &&
        new Set(config.apps).size === config.apps.length &&
        config.apps.every((app) => anchors.includes(app)),
    );
...
export function evaluateBudgets(measured, config, appId = null) {
  ...
  const violations = Object.entries(checks)
    .filter(
      ([name, value]) =>
        !Number.isFinite(value) || value < 0 || !Number.isFinite(config.budgets[name]) || value > config.budgets[name],
    )
    .map(([name]) => name);
  ...
  const knownBaselineFailureObserved =
    violations.length === 1 &&
    violations[0] === "owned-task-failed" &&
    config.baselineTag === "v0.7.0" &&
    config.baselineCommit === "3a23f49c85aa3c3d04b86f227e8aa184ef964085" &&
    known !== undefined &&
    workload.failure?.command === known.command &&
    workload.failure?.code === known.code &&
    workload.failure?.runFailureCode === known.runFailureCode;
  return {
    passed: violations.length === 0,
    violations,
    knownBaselineFailureObserved,
...
export function summarizeIdle(before, after, elapsedMs, logicalCpus) {
  ...
  for (const sample of [...before, ...after]) {
    assert.ok(
      Number.isInteger(sample.pid) && sample.pid > 0 && typeof sample.created === "string" && sample.created.length > 0,
    );
    assert.ok(
  ...
  for (const sample of after) {
    ...
    assert.ok(!seen.has(key(sample)));
    seen.add(key(sample));
    const old = previous.get(key(sample));
    if (!old) changed = true;
    else {
      assert.ok(sample.cpuMs >= old.cpuMs);
      delta += sample.cpuMs - old.cpuMs;
    }
  ...
  return {
    ...
    workingSetSumBytes: workingSet,
    cohortChanged: changed,
...
export async function measureInput(cdp) {
  ...
  const accepted = await cdp.evaluate(
    "(() => { const accepted = window.__devboxFoundationInput; delete window.__devboxFoundationInput; return accepted; })()",
  );
...
export async function measureWorkload(app, cdp, isolatedRoot, onStage = () => {}) {
  ...
  if (app.id === "everything-plus") {
    const root = path.join(isolatedRoot, "search-performance-fixture");
    mkdirSync(root);
    for (let i = 0; i < 500; i++)
      writeFileSync(
        path.join(root, `fixture-${String(i).padStart(4, "0")}.txt`),
        `synthetic foundation record ${i}\n`.repeat(20),
        { flag: "wx" },
      );
    ...
    const indexMs = Math.round(performance.now() - indexingStart),
      searchMs = [];
    for (let i = 0; i < 10; i++) {
      const start = performance.now();
      ...
      try {
        ...
        assert.ok(
          runs.length === 1 &&
            ...
            runs[0].endedAt != null &&
            runs[0].failureCode === "spawn-failed",
          "unexpected legacy execution failure",
        );
        return {
  ...
  if (app.id === "wsl-desktop") {
    ...
    const profile = {
      ...
      panes: [
        {
          key: "fixture-pane",
          distro: "devbox-foundation-unavailable",
          cwd: "/tmp/devbox-foundation",
          startCommand: null,
          multiplexer: "native",
        },
      ],
      activeTabId: "fixture-tab",
      activePaneKey: "fixture-pane",
    };
    const saved = await invoke("save_workspace_profile", { profile });
    const start = performance.now();
    const restored = await invoke("list_workspace_profiles");
    assert.ok(
      restored.some((p) => p.id === saved.id && p.panes.length === 1),
      "terminal profile readback failed",
    );
    const profileReadbackMs = Math.round(performance.now() - start);
    const sessions = await invoke("list_sessions");
    assert.deepEqual(
      sessions.map((s) => s.id).sort(),
      existingSessions.map((s) => s.id).sort(),
      "profile restoration must not create a PTY",
    );
    ...
    return {
      kind: "saved-terminal-profile-readback-without-execution",
      result: "measured",
      profileReadbackMs,
      distributions,
      livePtyRestore: "not-run; this fixture measures persisted metadata only",
    };
  }
  return null;
}
