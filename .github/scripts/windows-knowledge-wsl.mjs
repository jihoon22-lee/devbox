...
export async function exerciseKnowledgeWsl({
  item,
  executable,
  profile,
  command,
  ...
  const moveCorpus = async (from, to) => {
    ...
    for (let attempt = 1; ; attempt++) {
      const moved = spawnSync(
        "wsl.exe",
        ["--distribution", distro, "--user", "root", "--exec", "/bin/mv", "-T", "--", from, to],
        { encoding: "utf8", windowsHide: true, timeout: Math.max(1, Math.ceil(end - performance.now())) },
      );
      if (moved.status === 0) {
        evidence.wsl.moveAttempts = [...(evidence.wsl.moveAttempts ?? []), attempt];
        return;
      }
      evidence.wsl.moveFailure = {
        status: moved.status,
        signal: moved.signal,
        error: moved.error?.code,
        stderr: (moved.stderr ?? "").slice(0, 2048),
        stdout: (moved.stdout ?? "").slice(0, 512),
      };
      const retry =
        moved.status === 1 &&
        !moved.signal &&
        !moved.error &&
        (moved.stderr ?? "").trimEnd().endsWith(": Permission denied");
      if (!retry || performance.now() + 200 >= end)
  ...
  for (let i = 0; i < 500; i++)
    writeFileSync(
      path.join(corpus, `wslfixture${String(i).padStart(4, "0")}.txt`),
      `wslnativecontent wslgroup${Math.floor(i / 100)} fixture\n`,
      { flag: "wx" },
    );
  const identity = (p) => {
  ...
  evidence.wsl = {
    version: 1,
    boundary:
      "Actual WSL1 UNC; native Linux move of independent Search directory models root unavailability, not vault/VM suspension",
    result: "running",
  };
  const succeeded = (response) => {
    assert.equal(response.operation.outcome.state, "succeeded", JSON.stringify(response.operation.outcome));
    return response.value;
  };
  const query = async (text, mode = "name", limit = 2000) =>
    sourceQuery(item, "files", text, { sourceRootId: rootId }, limit, mode);
  const cancel = async (result) =>
    command(item, "knowledge.search", "source_cancel", { generation: result.generation });
  ...
  const linux = (...args) => {
    const result = spawnSync("wsl.exe", ["--distribution", distro, "--user", "root", "--exec", ...args], {
      encoding: "utf8",
      windowsHide: true,
      timeout: 15_000,
    });
  ...
  const saved = succeeded(
    await command(item, "knowledge.notes", "write_file", {
      rel: "Notes/Case.md",
      content,
      expectedRevision: succeeded(await command(item, "knowledge.notes", "read_file", { rel: "Notes/Case.md" }))
        .revision,
    }),
  );
  ...
  assert.equal(
    succeeded(await command(item, "knowledge.notes", "read_file", { rel: "Notes/case.md" })).content,
    "# Lower case\nSeparate Linux file\n",
  );
  ...
  await eventually(async () => {
    ...
    evidence.wsl.largeFilenameQuery = {
      state: result.state,
  ...
  for (const index of [0, 99, 199, 299, 399, 499]) {
    ...
    evidence.wsl.lastExactQuery = {
      state: exact.state,
      rows: exact.rows.length,
      availability: exact.rows[0]?.availability,
    };
    assert.equal(exact.rows.length, 1);
    assert.equal(exact.rows[0].availability, "available");
    assert.ok(exact.rows[0].reference);
    await cancel(exact);
  }
  const body = await query("wslnativecontent", "content");
  ...
  succeeded(
    await command(item, "knowledge.search-settings", "add_root", { path: alternateCorpus, indexContent: true }),
  );
  assert.equal(
    succeeded(await command(item, "knowledge.search", "list_roots")).filter((r) => !before.has(r.id)).length,
    1,
  );
  evidence.wsl.aliasRegistrationKeptRootId = true;
  evidence.wsl.pollingAnd500FileBodySearch = true;

  const held = await query("wslfixture0000");
  assert.equal(held.rows.length, 1);
  assert.ok(held.rows[0].reference);
  const target = path.join(corpus, "wslfixture0000.txt");
  renameSync(target, path.join(corpus, "previous-object.txt"));
  writeFileSync(target, "replacement fixture\n", { flag: "wx" });
  assert.equal(
    (await command(item, "knowledge.opener", "reveal_file", { reference: held.rows[0].reference })).operation.outcome
      .state,
    "failed",
  );
  await cancel(held);
  ...
  const linked = spawnSync(
    ...
    [
      ...
      "-s",
      "/etc/hostname",
      "/home/devbox-fixture/한글 project/Notes/outside.md",
    ],
    { encoding: "utf8", windowsHide: true, timeout: 30_000 },
  );
  assert.equal(linked.status, 0);
  assert.equal(
    (await command(item, "knowledge.notes", "read_file", { rel: "Notes/outside.md" })).operation.outcome.state,
    "failed",
  );
  ...
  await eventually(
    async () => {
      ...
      evidence.wsl.beforeMoveResources = {
        state: state.state,
        retainedObjects: state.bounds.retainedObjects,
        runningWorkers: state.bounds.runningWorkers,
        indexing: index.indexing,
      };
      return (
        state.state === "cancelled" &&
        state.bounds.retainedObjects === 0 &&
        state.bounds.runningWorkers === 0 &&
        !index.indexing
      );
    },
    "Cancelled WSL source work did not release its native objects",
    30_000,
  );
  ...
  try {
    ...
    evidence.wsl.offlineQuery = {
      state: offline.state,
      partial: offline.partial,
      rows: offline.rows.length,
      bounds: offline.bounds,
      indexStatus: succeeded(await command(item, "knowledge.search", "index_status")),
      watcherStatuses: succeeded(await command(item, "knowledge.search", "watcher_statuses")),
    };
    assert.equal(offline.rows.length, 500);
    assert.ok(offline.rows.every((r) => r.availability !== "available" && !r.reference));
    ...
    assert.ok(local.rows.some((r) => r.availability === "available"));
    await cancel(local);
    evidence.wsl.unavailableRootKeptLastGoodAndLocalSource = true;
  } finally {
    await moveCorpus(posixMoved, posixCorpus);
  }
  ...
  await eventually(
    async () => {
      const gone = await query("wslfixture0001"),
        fresh = await query("wslfixture0500");
      const ready = gone.rows.length === 0 && fresh.rows.length === 1 && fresh.rows[0].availability === "available";
      await cancel(gone);
      await cancel(fresh);
      return ready;
    },
    "WSL polling did not converge after reconnect",
    120_000,
  );
  assert.equal(
    succeeded(await command(item, "knowledge.notes", "read_file", { rel: "Notes/Case.md" })).content,
