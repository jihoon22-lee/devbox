...
const spec = {
  label: "API Studio Requests",
  rootAppExcluding: ["webhooks/", "transforms/"],
  budget: { rawBytes: 685000, gzipBytes: 205000 },
  deferred: [
    { suffix: "/requests/OpenApiImport.tsx" },
    { suffix: "/requests/ProtocolLab.tsx" },
    { suffix: "/transforms/App.tsx" },
  ],
  forbiddenMarkers: ["tag:yaml.org"],
};
export function measureRequestsRoute(manifest, readAsset) {
  return measureRoute(manifest, readAsset, spec);
}
function selfTest() {
  const manifest = {
  ...
  assert.throws(
    () => measureRequestsRoute({ ...manifest, requests: { ...manifest.requests, imports: ["openapi"] } }, read),
    /eagerly loads/,
  );
...
if (isInvoked(import.meta.url)) {
  if (process.argv[2] === "--self-test") selfTest();
  else reportBuiltRoute("api-studio", measureRequestsRoute, spec);
}
