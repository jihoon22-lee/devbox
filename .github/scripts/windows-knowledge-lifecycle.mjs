...
assert.equal(performanceConfig.schemaVersion, 1);
assert.equal(performanceConfig.idleSampleMs, 5000);
...
async function start(executable, title, profile) {
  ...
  const item = { executable, policy, child: null, cdp: null };
  live.add(item);
...
function visible(window) {
  ...
  assert.equal(result.status, 0);
  assert.ok(["True", "False"].includes(result.stdout.trim()));
  return result.stdout.trim() === "True";
}
function running(item) {
  return item.child?.exitCode === null && item.child?.signalCode === null;
}
async function stop(item, force = false) {
  item.cdp?.close();
  if (running(item)) {
    if (force) item.child.kill();
    else closeWindow(item);
    for (let i = 0; i < 100 && running(item); i++) await delay(100);
    if (running(item)) {
      item.child.kill();
      throw new Error("owned fixture process did not exit");
    }
  }
...
async function wait(cdp, expression, label, timeout = 60_000) {
  ...
  evidence.ui = await cdp.evaluate('(document.body?.innerText ?? "").slice(0, 10000)');
  throw new Error(label);
}
async function click(cdp, label) {
  await wait(
    cdp,
    `Array.from(document.querySelectorAll("button")).some(button => button.textContent.trim() === ${JSON.stringify(label)} && !button.disabled)`,
    `control unavailable: ${label}`,
  );
  await cdp.evaluate(
    `Array.from(document.querySelectorAll("button")).find(button => button.textContent.trim() === ${JSON.stringify(label)} && !button.disabled).click()`,
  );
}
...
async function sourceQuery(item, source, query, filter = {}, limit = 200, mode = "name") {
  ...
  for (let i = 0; i < 60 && result.value.state === "running"; i++) {
    await delay(80);
    result = await command(item, "knowledge.search", "source_poll", { generation });
    assert.equal(result.operation.outcome.state, "succeeded");
    assert.equal(result.value.generation, generation);
  }
  assert.notEqual(result.value.state, "running");
...
async function product(executable, profile) {
  ...
  await wait(
    item.cdp,
    '!!document.querySelector(".knowledge-startup") || !!document.querySelector("#vault-setup-title") || !!document.querySelector(".knowledge-feature-notes .app")',
    "Knowledge startup missing",
  );
  const added = readdirSync(base).filter((name) => name.startsWith("com.devbox.v08.knowledge.i") && !before.has(name));
  if (added.length) {
    assert.equal(added.length, 1);
    item.dataRoot = path.join(base, added[0]);
    const owner = randomUUID(),
      marker = path.join(item.dataRoot, `.knowledge-fixture-${owner}`);
    writeFileSync(marker, owner, { flag: "wx" });
    profiles.push({ root: item.dataRoot, owner, marker, identity: lstatSync(item.dataRoot, { bigint: true }) });
  }
  return item;
}
try {
  ...
  const profile = path.join(directory, "product-webview");
  let item = await product(executable, profile);
  ...
  await command(item, "knowledge.notes", "create_template", {
    draft: { name: "product fixture", content: "new product edit" },
  });
  evidence.automaticStartupReady = true;
  progress("reviewed-vault-rebinding");
  const selectedVault = path.join(directory, "selected-vault");
  mkdirSync(selectedVault);
  mkdirSync(path.join(selectedVault, "Notes"));
  ...
  await wait(
    item.cdp,
    '!!document.querySelector(".knowledge-feature-notes .app")',
    "Notes did not observe current activation",
  );
  const scheduleVault = async () => {
    await click(item.cdp, "노트 폴더");
    await wait(
      item.cdp,
      '!!document.querySelector("#vault-settings-title") && !!document.querySelector(\'[aria-label="연결할 노트 폴더"]:not(:disabled)\')',
      "vault settings did not finish loading",
    );
    await item.cdp.evaluate(
      `(() => { const input=document.querySelector('[aria-label="연결할 노트 폴더"]'); Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"value").set.call(input,${JSON.stringify(selectedVault)}); input.dispatchEvent(new Event("input",{bubbles:true})); })()`,
    );
    await click(item.cdp, "다음 시작에서 폴더 확인");
    await wait(
      item.cdp,
      'document.body.innerText.includes("다음 시작에서 확인할 폴더")',
      "vault choice was not scheduled",
    );
  ...
  assert.equal(
    (
      await command(item, "knowledge.notes", "create_file", {
        rel: "Notes/still-old.md",
        content: "# Still in original vault",
      })
    ).operation.outcome.state,
    "succeeded",
  );
  assert.ok(existsSync(path.join(vault, "Notes/still-old.md")));
  assert.equal(existsSync(path.join(selectedVault, "Notes/still-old.md")), false);
  await stop(item);
  ...
  await wait(
    item.cdp,
    '!!document.querySelector(".knowledge-feature-notes .app")',
    "cancel did not restore original editor",
  );
  assert.ok(sameDirectory((await command(item, "knowledge.notes", "get_root")).value, vault));
  await scheduleVault();
  ...
  await wait(
    item.cdp,
    '!!document.querySelector(".knowledge-feature-notes .app")',
    "explicit vault activation missing",
  );
  ...
  assert.equal(
    (await command(item, "knowledge.notes", "read_file", { rel: "Notes/existing.md" })).value.content,
    "# Existing selected note",
  );
  assert.equal(
    (
      await command(item, "knowledge.notes", "create_file", {
        rel: "Notes/new-selected.md",
        content: "# Selected vault",
      })
    ).operation.outcome.state,
    "succeeded",
  );
  assert.ok(existsSync(path.join(selectedVault, "Notes/new-selected.md")));
  assert.equal(existsSync(path.join(vault, "Notes/new-selected.md")), false);
  let selectedQuery;
  for (let i = 0; i < 40; i++) {
    selectedQuery = await sourceQuery(item, "notes", "Selected");
    if (selectedQuery.rows.some((row) => row.rootIdentity === `notes:${vaultPlan.id}`)) break;
    await delay(150);
  }
  ...
  evidence.vaultScheduleKeptLiveBinding = true;
  evidence.vaultPreviewDidNotWrite = true;
  evidence.vaultCancellationKeptOriginal = true;
  evidence.vaultExplicitRebindPreservedFilesAndTemplates = true;
  evidence.vaultSourceIdentityChanged = true;
  progress("second-installation-owner");
  const secondExe = path.join(directory, `knowledge-second-${randomUUID()}.exe`);
  copyFileSync(executable, secondExe);
  const secondProfile = path.join(directory, "second-webview");
  let second = await product(secondExe, secondProfile);
  await wait(
    second.cdp,
    '!!document.querySelector(".knowledge-feature-notes .app")',
  ...
  await stop(second);
  second = await product(secondExe, secondProfile);
  ...
  await wait(
    item.cdp,
    '!!document.querySelector(".knowledge-feature-notes .app")',
    "configured Notes did not become ready",
  );
  evidence.performance = {
    host: performanceHost(),
    build:
      process.env.DEVBOX_FIXTURE_PROFILE === "release"
        ? "exact candidate release executable"
        : "hidden Windows debug executable",
    conditions: {
  ...
  const measuredExe = lstatSync(processIdentity.Path, { bigint: true }),
    ownedExe = lstatSync(executable, { bigint: true });
  assert.equal(measuredExe.dev, ownedExe.dev);
  assert.equal(measuredExe.ino, ownedExe.ino);
  evidence.performance.idle = await measureIdle(() => {
    const current = allWindowsProcesses();
    assert.ok(
      current.some(
        (process) =>
          process.Pid === processIdentity.Pid &&
          process.Created === processIdentity.Created &&
          process.Path === processIdentity.Path &&
          process.Name === processIdentity.Name,
      ),
    );
  ...
  const performanceRoot = path.join(directory, "performance-root");
  mkdirSync(performanceRoot);
  for (let i = 0; i < 500; i++)
    writeFileSync(
      path.join(performanceRoot, `fixturesearch${String(i).padStart(4, "0")}.txt`),
      "synthetic UTF-8 index fixture\n",
      { flag: "wx" },
    );
  const priorRoots = new Set((await command(item, "knowledge.search", "list_roots")).value.map((root) => root.id));
  const indexStarted = performance.now();
  assert.equal(
    (await command(item, "knowledge.search-settings", "add_root", { path: performanceRoot, indexContent: false }))
      .operation.outcome.state,
    "succeeded",
  );
  const addedRoots = (await command(item, "knowledge.search", "list_roots")).value.filter(
    (root) => !priorRoots.has(root.id),
  );
  assert.equal(addedRoots.length, 1);
  ...
  evidence.performance.workload = {
    kind: "500-file-native-index-and-10-source-searches",
    result: "measured",
    indexMs: Math.round(performance.now() - indexStarted),
    searchMs: [],
  };
  for (let i = 0; i < 10; i++) {
    const query = `fixturesearch${String(i).padStart(4, "0")}`,
    ...
    assert.equal(found.rows[0].value.name, `${query}.txt`);
  ...
  const secondary = {
    ...
    policy: null,
    cdp: null,
    child: spawn(executable, [], { env: childEnvironment({ WEBVIEW2_USER_DATA_FOLDER: profile }), stdio: "ignore" }),
  };
  live.add(secondary);
  ...
  assert.equal(
    evidence.performance.budget.passed,
    true,
  ...
  evidence.error = String(error.message).slice(0, 2000);
  throw error;
} finally {
  ...
  for (const profile of profiles) {
    try {
      const actual = lstatSync(profile.root, { bigint: true });
      const marker = lstatSync(profile.marker);
      assert.ok(
        actual.isDirectory() &&
          !actual.isSymbolicLink() &&
          actual.dev === profile.identity.dev &&
          actual.ino === profile.identity.ino,
      );
      assert.ok(marker.isFile() && !marker.isSymbolicLink() && marker.size === profile.owner.length);
      assert.equal(readFileSync(profile.marker, "utf8"), profile.owner);
