...
export async function exerciseWorkspaceSource({
  cdp,
  directory,
  call,
  success,
  ...
  const folder = path.join(directory, "Git 한글 source");
  mkdirSync(folder);
  const outside = path.join(directory, "unselected Git target");
  mkdirSync(outside);
  const global = path.join(directory, "source-fixture-global.config");
  ...
  const git = (args) => {
    const result = spawnSync("git", ["-C", folder, ...args], {
      ...
      windowsHide: true,
      timeout: 10_000,
      env: { ...process.env, GIT_CONFIG_NOSYSTEM: "1", GIT_CONFIG_GLOBAL: global, GIT_CONFIG_SYSTEM: global },
    });
    assert.equal(result.status, 0, "owned Git fixture command failed");
  ...
  writeFileSync(
    path.join(folder, ".git/hooks/pre-commit"),
  ...
  const registered = success(
    await call("workspace.registry", "apply_registration", {
  ...
  await cdp.evaluate(
    `(async()=>{const d=await window.__TAURI_INTERNALS__.invoke("plugin:product-shell|describe");const label=d.features.find(f=>f.route==="source").label;Array.from(document.querySelectorAll('nav[aria-label="제품 화면"] button')).find(button=>button.textContent.trim()===label).click();})()`,
  );
  const click = async (label, scope = ".workspace-native-source") => {
    ...
    await waitForRenderer(
      ...
      `Source action unavailable: ${label}`,
    );
  };
  const navigate = async (route) =>
    cdp.evaluate(
      `(async()=>{const d=await window.__TAURI_INTERNALS__.invoke("plugin:product-shell|describe");const label=d.features.find(f=>f.route===${JSON.stringify(route)}).label;Array.from(document.querySelectorAll('nav[aria-label="제품 화면"] button')).find(button=>button.textContent.trim()===label).click();})()`,
    );
  ...
  const dirty =
    'Array.from(document.querySelectorAll(".workspace-feature-files [role=tab]")).some(tab=>tab.textContent.includes("●"))';
  const openHistoryLine = async () => {
    ...
    try {
      ...
      await waitForRenderer(
        cdp,
        '!!document.querySelector(".history-entry:not(:disabled)")',
        "Source history entry unavailable",
      );
      ...
      await waitForRenderer(
        cdp,
        '!!document.querySelector(".workspace-feature-files:not([hidden]) .cm-content")',
        "Source diff did not open Files",
      );
    } finally {
      const attempts = await cdp.evaluate(
        "(() => {window.__workspaceHistoryRestore?.();return window.__workspaceHistoryProbe?.slice(-8)??[];})()",
      );
      writeFileSync(
        path.join("product-foundation-evidence", `workspace-history-${Date.now()}.json`),
  ...
  const saveAndCloseEditor = async () => {
    ...
    await waitForRenderer(
      cdp,
      '!document.querySelector(".workspace-feature-files [role=tab]")',
      "Source editor did not close",
    );
    await navigate("source");
    await waitForRenderer(
      cdp,
      'Array.from(document.querySelectorAll(".workspace-registry button")).some(button=>button.textContent.trim()==="프로젝트 선택 해제"&&!button.matches(":disabled"))',
      "Source editor still retains context",
    );
  };
  const selectTree = async (tree) => {
    ...
    await waitForRenderer(
      ...
      "Worktree Source did not settle",
    );
  };
  await click("Git 실행 검토");
  await click("검토한 Git 실행 승인");
  await waitForRenderer(
    cdp,
    '!!document.querySelector(".stage-commit-panel") && document.querySelector(".workspace-source-trust")?.textContent.includes("현재 Git 실행 근거의 승인을 확인했습니다.")',
    "Source approval did not finish",
  );
  assert.equal(existsSync(monitorMarker), false);
  ...
  await waitForRenderer(
    cdp,
    `!!document.querySelector('.stage-commit-panel input[aria-label="stage tracked.txt"]')`,
    "Native Git changes missing",
  );
  assert.equal(existsSync(monitorMarker), true);
  await cdp.evaluate(
    "document.querySelector('.stage-commit-panel button[aria-label=\"Files에서 tracked.txt 열기\"]').click()",
  );
  await waitForRenderer(
    cdp,
    '!!document.querySelector(".workspace-feature-files:not([hidden]) .cm-content")',
    "Source change did not open Files",
  );
  ...
  await waitForRenderer(
    cdp,
    `!!document.querySelector('.stage-commit-panel input[aria-label="unstage tracked.txt"]')`,
    "Selected native stage missing",
  );
  await cdp.evaluate(
    `(()=>{const input=document.querySelector('.stage-commit-panel textarea[aria-label="커밋 메시지"]');Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype,"value").set.call(input,"reviewed Source fixture");input.dispatchEvent(new Event("input",{bubbles:true}));})()`,
  );
  await waitForRenderer(
    cdp,
    'Array.from(document.querySelectorAll(".workspace-registry button")).find(button=>button.textContent.trim()==="프로젝트 선택 해제").disabled',
    "Commit draft did not retain project selection",
  );
  ...
  await waitForRenderer(
    cdp,
    '!document.querySelector(".stage-commit-panel [role=dialog]") && document.querySelector(".stage-commit-panel textarea")?.value==="" && document.querySelector(".stage-commit-panel")?.getAttribute("aria-busy")==="false"',
    "Reviewed native commit did not finish",
  );
  ...
  assert.equal(
    await cdp.evaluate('document.querySelector(".workspace-feature-files .cm-content").textContent'),
    editorDraft,
  );
  assert.equal(
    git(["show", "HEAD:tracked.txt"]),
    "edited in the selected root",
    "Git must not commit the unsaved editor buffer",
  );
  ...
  const preCancelled = await source("repo_stage", {
    request: { path: root, paths: ["keep-untracked.txt"], operationId: "source-before-admission" },
  });
  failed(preCancelled);
  assert.equal(preCancelled.value.issue, "source_cancelled");
  assert.equal(git(["diff", "--cached", "--name-only"]), beforeEarlyCancel);
  const cancelledTarget = path.join(directory, "cancelled new worktree");
  const createCancelled = success(
    await source("preview_worktree", { branch: "cancelled-worktree", targetDir: cancelledTarget }),
  );
  assert.equal(existsSync(cancelledTarget), false);
  success(await source("cancel_worktree", { previewId: createCancelled.previewId }));
  failed(
    await source("create_worktree", { previewId: createCancelled.previewId, operationId: "cancelled-worktree-create" }),
  );
  assert.equal(existsSync(cancelledTarget), false);
  ...
  writeFileSync(path.join(staleTarget, "sentinel.txt"), "concurrent owner");
  ...
  await fill("source-worktree-branch", "source-fixture-worktree");
  await fill("source-worktree-target", linkedTarget);
  await click("작업 폴더 생성 검토", ".workspace-source-worktree");
  await waitForRenderer(
    cdp,
    "!!document.querySelector('.workspace-source-worktree [aria-label=\"작업 폴더 생성 확인\"]')",
    "Native worktree review missing",
  );
  ...
  await waitForRenderer(
    cdp,
    'document.querySelector(".workspace-registry")?.textContent.includes("기존 프로젝트의 연결된 작업 폴더입니다.")',
    "Created worktree was not recognized as a linked project",
  );
  await click("등록", ".workspace-registry");
  await waitForRenderer(
    cdp,
    "!document.querySelector('.workspace-registry [aria-label=\"프로젝트 등록 확인\"]')",
    "Created worktree registration did not finish",
  );
  const linkedRegistry = success(await call("workspace.registry", "snapshot"));
  const linked = linkedRegistry.worktrees.find(
    (tree) => realpathSync.native(tree.binding.root) === realpathSync.native(linkedTarget),
  );
  const originalTree = linkedRegistry.worktrees.find((tree) => tree.id === registered.context.worktreeId);
  assert.ok(linked);
  assert.equal(linked.projectId, originalTree.projectId);
  assert.equal(linked.repoId, originalTree.repoId);
  assert.notEqual(linked.id, originalTree.id);
  assert.equal(linked.trustedDigest, null);
  assert.equal(
    (await cdp.evaluate('window.__TAURI_INTERNALS__.invoke("plugin:product-shell|describe")')).context.worktreeId,
    originalTree.id,
    "Registration must not automatically select a new context",
  );
  await selectTree(linked);
  assert.equal(
    success(await source("trust_status")).approved,
    false,
    "Another worktree cannot reuse Git execution approval",
  );
  await click("Git 실행 검토");
  await click("검토한 Git 실행 승인");
  await openHistoryLine();
  ...
  assert.match(readFileSync(path.join(folder, "tracked.txt"), "utf8"), /^unsaved /);
  success(
    await source("repo_stage", {
      request: { path: linked.binding.root, paths: ["tracked.txt"], operationId: "linked-stage" },
    }),
  );
  success(
    await source("repo_commit", {
      request: {
        path: linked.binding.root,
        message: "linked worktree edit",
        operationId: "linked-commit",
        indexRevision: success(await source("repo_commit_preview", { request: { path: linked.binding.root } }))
          .revision,
      },
    }),
  );
  ...
  assert.equal(scopeStatus.hasApproval, false);
  assert.deepEqual(scopeStatus.selectedIds, []);
  assert.ok(scopeStatus.available.some((tree) => tree.id === linked.id));
  const noScope = success(
    await source("repo_cleanup_preview", { request: { path: root, operationId: "cleanup-without-scope" } }),
  );
  assert.equal(
    noScope.worktrees.find((tree) => realpathSync.native(tree.path) === realpathSync.native(linkedTarget)).eligible,
    false,
  );
  ...
  await waitForRenderer(
    cdp,
    '!!document.querySelector(".workspace-source-cleanup-scope input[type=checkbox]")',
    "Registered cleanup sibling missing",
  );
  await cdp.evaluate(
    `Array.from(document.querySelectorAll(".workspace-source-cleanup-scope label")).find(label=>label.textContent===${JSON.stringify(linked.binding.root)}).querySelector("input").click()`,
  );
  await click("선택한 정리 범위 검토", ".workspace-source-cleanup-scope");
  await click("검토한 정리 범위 승인", ".workspace-source-cleanup-scope");
  await waitForRenderer(
    cdp,
    'document.querySelector(".workspace-source-cleanup-scope")?.textContent.includes("검토한 정리 범위를 승인했습니다.")',
    "Native cleanup scope did not finish",
  );
  ...
  const staleScope = await source("repo_cleanup_preview", {
    request: { path: root, operationId: "cleanup-stale-scope" },
  });
  failed(staleScope);
  assert.equal(staleScope.value.issue, "source_cleanup_scope_changed");
  success(await source("repo_changes", { request: { path: root } }));
  await click("선택한 정리 범위 검토", ".workspace-source-cleanup-scope");
  await click("검토한 정리 범위 승인", ".workspace-source-cleanup-scope");
  await waitForRenderer(
    cdp,
    'document.querySelector(".workspace-source-cleanup-scope")?.getAttribute("aria-busy")==="false"&&!document.querySelector(".workspace-source-cleanup-scope > section")',
    "Refreshed cleanup scope did not settle",
  );
  const beforePickerTrust = success(await source("trust_status"));
  assert.equal(
    beforePickerTrust.approved,
    true,
    JSON.stringify({ stage: "before-sibling-picker", changedEvidence: beforePickerTrust.changedEvidence }),
  );
  const pickingSibling = call("workspace.files", "pick_files");
  const [pickedSibling] = await Promise.all([
    pickingSibling,
    nativeFileDialog({
      processId,
      executable,
      directory,
      action: "Open",
      selectedFile: path.join(linkedTarget, "tracked.txt"),
    }),
  ]);
  const siblingChoices = success(pickedSibling);
  assert.equal(siblingChoices.length, 1);
  const siblingDocument = success(
    await call("workspace.files", "open_file", { request: { path: siblingChoices[0], encoding: null } }),
  );
  const afterPickerTrust = success(await source("trust_status"));
  assert.equal(
    afterPickerTrust.approved,
    true,
    JSON.stringify({ stage: "after-sibling-picker", changedEvidence: afterPickerTrust.changedEvidence }),
  );
  const openSiblingPreview = success(
    await source("repo_cleanup_preview", { request: { path: root, operationId: "cleanup-open-sibling-document" } }),
  );
  assert.equal(
    openSiblingPreview.worktrees.find((tree) => realpathSync.native(tree.path) === realpathSync.native(linkedTarget))
      .eligible,
    false,
    "An open native picker document must prevent worktree removal",
  );
  ...
  await waitForRenderer(
    cdp,
    `!!(${linkedCheckbox})&&!(${linkedCheckbox}).matches(":disabled")`,
    "Approved clean sibling was not eligible",
  );
  await cdp.evaluate(`${linkedCheckbox}.click()`);
  await click("선택 항목 정리 (1)", ".cleanup-panel");
  await waitForRenderer(
    cdp,
    '!!document.querySelector(".cleanup-panel [role=dialog]")',
    "Final worktree cleanup confirmation missing",
  );
  assert.equal(existsSync(linkedTarget), true);
  await click("정리 실행", ".cleanup-panel");
  await waitForRenderer(
    cdp,
    'document.querySelector(".cleanup-panel .cleanup-result")?.textContent.includes("1개 제거")',
    "Reviewed worktree cleanup did not finish",
  );
  assert.equal(existsSync(linkedTarget), false);
  assert.equal(
    git(["show", "source-fixture-worktree:tracked.txt"]),
    linkedText.trim(),
    "Worktree removal must preserve its committed branch",
  );
  await click("정리 범위 승인 철회", ".workspace-source-cleanup-scope");
  await waitForRenderer(
    cdp,
    'document.querySelector(".workspace-source-cleanup-scope")?.textContent.includes("다른 작업 폴더의 정리 승인을 철회했습니다.")',
    "Removed sibling scope could not be revoked without IO",
  );
  const afterLinked = success(await call("workspace.registry", "snapshot"));
  success(
    await call("workspace.registry", "remove", {
      revision: afterLinked.revision,
      context: {
        projectId: linked.projectId,
        worktreeId: linked.id,
        revision: linked.revision,
        target: linked.binding.target,
      },
    }),
  );
  unlinkSync(monitorMarker);
  appendFileSync(monitor, "# changed hook source\n");
  failed(await source("repo_changes", { request: { path: root } }));
  assert.equal(existsSync(monitorMarker), false);
  ...
  success(
    await source("repo_stage", {
      request: { path: root, paths: ["tracked.txt"], operationId: "source-stage-cancel-fixture" },
    }),
  );
  const doc = success(
    await call("workspace.files", "open_file", { request: { path: path.join(root, "tracked.txt"), encoding: null } }),
  );
  const operationId = "source-cancel-fixture";
  const committing = source("repo_commit", {
    request: {
      path: root,
      message: "cancelled fixture",
      operationId,
      indexRevision: success(await source("repo_commit_preview", { request: { path: root } })).revision,
    },
  });
  const started = Date.now();
  while (!existsSync(cancellationMarker) && Date.now() - started < 5000)
  ...
  const blockedSave = await call(
    "workspace.files",
    "save_file",
    { request: nativeFileSave(doc, "must wait for Git\n") },
    500,
  );
  failed(blockedSave);
  assert.equal(blockedSave.value.issue, "request_expired");
  assert.equal(readFileSync(path.join(folder, "tracked.txt"), "utf8"), "pending cancelled commit\n");
  const recoverySaved = success(
    await call("workspace.files", "save_recovery", {
      nativeRevision: success(await call("workspace.files", "load_recovery", {})).nativeRevision,
      entries: [
        {
          path: doc.path,
          content: "editor buffer survives Git",
  ...
  success(
    await call("workspace.files", "save_file", {
      request: nativeFileSave(doc, "editor saved after Git cancellation\n"),
    }),
  );
  assert.equal(readFileSync(path.join(folder, "tracked.txt"), "utf8"), "editor saved after Git cancellation\n");
  success(
    await call("workspace.files", "discard_recovery", { path: doc.path, nativeRevision: recoverySaved.nativeRevision }),
  );
  success(await call("workspace.files", "unwatch_file", { path: doc.path }));
  await click("Git 실행 승인 철회");
  await waitForRenderer(
    cdp,
    'document.querySelector(".workspace-source-trust")?.textContent.includes("Git 실행 승인을 철회했습니다.")',
    "Source revocation did not finish",
  );
  ...
  return {
    unapprovedGitAndHooksNotExecuted: true,
    cancelledAndStaleApprovalDenied: true,
    uiExplicitGitApproval: true,
    rendererRootDenied: true,
    nativeWorktreeOverridesConfigRedirect: true,
    uiSelectedStage: true,
    commitReviewedAndHooksOwned: true,
    unselectedFilesPreserved: true,
    changedHookRevokesExecution: true,
    gitApprovalDoesNotGrantTaskTrust: true,
    sourceChangesAndDiffOpenFiles: true,
    gitDoesNotSaveOrCommitEditorDrafts: true,
    preAdmissionCancellationPreventsGit: true,
    worktreeReviewCancelAndConcurrentTarget: true,
    uiWorktreeCreateAndRegistrationProposal: true,
    linkedContextFilesStageCommit: true,
    cleanupRequiresSeparateSiblingScope: true,
    changedSiblingEvidenceRevokesOnlyCleanup: true,
    uiReviewedSiblingCleanupPreservesBranch: true,
    nativePickerDocumentBlocksSiblingCleanup: true,
    gitBlocksFileWritesButKeepsRecovery: true,
    expiredSaveIsNotReplayed: true,
    ownedGitCancellationReleasesEditor: true,
  };
}
