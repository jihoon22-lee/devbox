...
const [directory,mode]=process.argv.slice(2);assert.ok(["import","health","committed"].includes(mode));
...
async function start(member){
 ...
 assert.equal((await item.cdp.evaluate("window.__TAURI_INTERNALS__.invoke('plugin:product-shell|describe')")).deliveryState,mode);
 return item;
}
try {
 ...
 for(const item of Object.values(apps)) {
  const route={workspace:"overview","api-studio":"requests",knowledge:"notes","control-center":"recovery"}[item.product];
