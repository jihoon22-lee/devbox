...
export async function createWorkspaceLspProxy() {
  ...
  const deny = (socket) => {
    ...
    socket.end("HTTP/1.1 502 Fixture Offline\r\nContent-Length: 0\r\nConnection: close\r\n\r\n");
  };
  const server = createServer((socket) => {
    ...
    socket.once("data", () => {
      attempts++;
      holding ? pending.add(socket) : deny(socket);
    });
  });
  ...
  return {
    ...
    release: () => {
      holding = false;
      for (const socket of pending) deny(socket);
    },
...
async function downloadArchive(artifact, destination) {
  ...
  for (let redirects = 0; redirects <= 5; redirects++) {
    assert.ok(
      ...
      "Unexpected reviewed artifact transport",
    );
    const response = await fetch(url, { redirect: "manual", signal: AbortSignal.timeout(120_000) });
    if (response.status >= 300 && response.status < 400) {
      const location = response.headers.get("location");
      await response.body?.cancel();
      assert.ok(location, "Reviewed artifact redirect lacks a destination");
    ...
    for await (const chunk of response.body) {
      size += chunk.length;
      assert.ok(size <= expected, "Reviewed artifact exceeds expected size");
      chunks.push(chunk);
    }
...
export async function exerciseWorkspaceLspInstaller({
  cdp,
  root,
  directory,
  call,
  ...
  const state = async (item) => {
    ...
    if (result.operation.outcome.state !== "succeeded") {
      ...
      if (existsSync(index))
        writeFileSync(path.join("product-foundation-evidence", "workspace-lsp-owned-index.json"), readFileSync(index));
    }
  ...
  const beforeBlocked = network.attempts();
  network.hold();
  const installing = lsp("lsp_install", key(rust)).then(
    (value) => ({ value }),
    (error) => ({ error }),
  );
  const editing = path.join(root, "lsp-install-edit.txt");
  writeFileSync(editing, "before", { flag: "wx" });
  try {
    await network.waitForAttempt(beforeBlocked);
    const document = success(
      await call("workspace.files", "open_file", { request: { path: editing, encoding: null } }),
    );
    success(
      await call("workspace.files", "save_file", {
        request: nativeFileSave(document, "edited during blocked download"),
      }),
    );
  ...
  rejected(blocked.value);
  assert.equal((await state(rust)).state, "not_installed");

  const archives = path.join(directory, "reviewed-lsp-archives");
  mkdirSync(archives);
  ...
  const pick = async (action, selectedFiles = []) => {
    ...
    const [selected] = await Promise.all([
      result,
      nativeFileDialog({ processId, executable, directory, action, selectedFiles }),
    ]);
    return success(selected);
  };
  assert.deepEqual(await pick("Cancel"), []);
  const discarded = await pick("Open", [rustArchive]);
  assert.equal(discarded.length, 1);
  assert.match(discarded[0], /^[0-9a-f-]{36}$/);
  assert.notEqual(discarded[0], rustArchive);
  ...
  assert.equal(imported.state, "installed");
  assert.equal(imported.installed.install_source, "local_archive");
  assert.deepEqual(success(await lsp("language_server_statuses")), []);
  const unapprovedStart = await lsp("start_language_server", {
    languageId: "rust",
    operationId: "unapproved-install-proof",
  });
  rejected(unapprovedStart);
  assert.equal(unapprovedStart.value.issue, "lsp_settings_save_required");

  await cdp.evaluate(
    `(async()=>{const d=await window.__TAURI_INTERNALS__.invoke("plugin:product-shell|describe");const label=d.features.find(f=>f.route==="files").label;Array.from(document.querySelectorAll('nav[aria-label="제품 화면"] button')).find(b=>b.textContent.trim()===label).click();})()`,
  );
  await waitForRenderer(
    cdp,
    'Array.from(document.querySelectorAll(".workspace-feature-files:not([hidden]) button")).some(b=>b.textContent.trim()==="언어 서버"&&!b.disabled)',
    "LSP settings button unavailable",
  );
  await cdp.evaluate(
    'Array.from(document.querySelectorAll(".workspace-feature-files button")).find(b=>b.textContent.trim()==="언어 서버").click()',
  );
  ...
  const click = async (label, scope = "document") => {
    ...
    await waitForRenderer(
      cdp,
      `(()=>{const button=${predicate};if(!button)return false;button.click();return true;})()`,
      "LSP installer action unavailable",
    );
  };
  await click("제거", card);
  await click("제거 확인");
  await waitForRenderer(
    cdp,
    `!!(${card})?.querySelector(".lsp-state.not_installed")`,
    "Native LSP uninstall did not settle",
  );
  assert.equal((await state(rust)).archive_cached, true);
  await click("설치", card);
  await click("취소", 'document.querySelector(".lsp-confirmation")');
  assert.equal((await state(rust)).state, "not_installed");
  ...
  await waitForRenderer(
    cdp,
    `!!(${card})?.querySelector(".lsp-state.installed")`,
    "Offline native LSP install did not settle",
  );
  const cached = await state(rust);
  assert.equal(cached.installed.install_source, "archive_cache");
  assert.equal(network.attempts(), beforeCache);
  ...
  const lock = JSON.parse(lockBytes),
    packages = lock.roots[node.id].map((root) =>
      lock.packages.find((item) => item.name === root.name && item.version === root.version && item.path === root.path),
    );
  ...
  const recovery = await exerciseWorkspaceLspRecovery({
    cdp,
    root,
    directory,
    executable,
  ...
  return {
    ...execution,
    ...recovery,
    blockedDownloadLeavesFilesUsable: true,
    nativeCancelAndOpaqueChoices: true,
    arbitraryPathAndReplayRejected: true,
    verifiedNativeArchiveImported: true,
    confirmedUiUninstallAndOfflineCacheInstall: true,
    reviewedNodeClosureImportedWithNativeMultiPicker: true,
    installationNeverStartsLanguageServers: true,
    offlineProxyAttempts: network.attempts(),
  };
}
...
async function exerciseWorkspaceLspExecution({ cdp, root, call, success, waitForRenderer }) {
  ...
  try {
    const owned = path.join(root, "lsp-owned-fixture");
    mkdirSync(owned);
    const script = path.join(owned, "server.mjs"),
      marker = path.join(owned, "child.pid"),
      file = path.join(owned, "lsp-owner-main.rs");
      ...
      custom_servers: [
        {
          language_ids: ["rust"],
          executable: script,
          args: [marker, "documents"],
          runtime: { kind: "node", executable: process.execPath, min_version: null },
          source: "synthetic Windows fixture",
          version: "1",
          license: "UNLICENSED",
        },
      ],
    ...
    success(
      await lsp("save_lsp_config", {
        config: configured,
        nativeRevision: original.nativeRevision,
        recoverInvalid: false,
      }),
    );
    const denied = await lsp("start_language_server", { languageId: "rust", operationId: "unapproved-server-proof" });
    assert.equal(denied.operation.outcome.state, "failed");
    assert.equal(denied.value.issue, "lsp_execution_approval_required");
    const click = async (label, scope = "document") => {
      const predicate = `Array.from((${scope})?.querySelectorAll("button")??[]).find(b=>b.textContent.trim()===${JSON.stringify(label)}&&!b.disabled)`;
      await waitForRenderer(cdp, `!!(${predicate})`, "Native LSP action unavailable");
      await cdp.evaluate(`(${predicate}).click()`);
    };
    await cdp.evaluate(
      `(()=>{const input=document.getElementById("path-input");Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"value").set.call(input,${JSON.stringify(file)});input.dispatchEvent(new Event("input",{bubbles:true}));})()`,
    );
    await click("파일 열기", 'document.querySelector(".workspace-feature-files")');
    await waitForRenderer(
      cdp,
      'Array.from(document.querySelectorAll(".workspace-feature-files [role=tab]")).some(tab=>tab.textContent.includes("lsp-owner-main.rs"))',
      "LSP editor fixture did not open",
    );
    ...
    await waitForRenderer(
      cdp,
      'document.querySelector(".lsp-execution-review")?.textContent.includes("server.mjs")',
      "Native execution review omitted the script",
    );
    assert.ok(
      (await cdp.evaluate('document.querySelector(".lsp-execution-review").textContent')).includes("SystemRoot"),
    );
    ...
    await waitForRenderer(
      cdp,
      'document.querySelector(".lsp-panel").textContent.includes("검토한 설정의 실행을 승인했습니다")',
      "Native LSP approval did not settle",
    );
    assert.deepEqual(success(await lsp("language_server_statuses")), []);
    await click("시작", 'document.querySelector(".lsp-panel")');
    await waitForRenderer(
      cdp,
      'document.querySelector(".lsp-panel").textContent.includes("준비됨")',
      "Native language server did not initialize",
    );
    await click("닫기", 'document.querySelector(".lsp-panel")');
    ...
    const hover = async () => {
      ...
      while (performance.now() < deadline) {
        ...
        assert.ok(
          ["lsp_document_denied", "file_snapshot_changed", "lsp_busy", "files_unavailable"].includes(
            result.value.issue,
          ),
          "Unexpected document synchronization failure",
        );
        await delay(100);
      }
    ...
    await waitForRenderer(
      cdp,
      "window.__workspaceLspTrace.rows.length>0",
      "Native editor trace did not observe IPC fetches",
    );
    const previousChanges = await cdp.evaluate(
      'window.__workspaceLspTrace.rows.filter(row=>row.method==="change_lsp_document"&&row.phase==="succeeded").length',
    );
    ...
    const dirty =
      'Array.from(document.querySelectorAll(".workspace-feature-files [role=tab]")).some(tab=>tab.textContent.includes("lsp-owner-main.rs")&&tab.textContent.includes("●"))';
    await waitForRenderer(cdp, dirty, "LSP editor change did not remain dirty");
    await waitForRenderer(
      cdp,
      `window.__workspaceLspTrace.rows.filter(row=>row.method==="change_lsp_document"&&row.phase==="succeeded").length>${previousChanges}`,
      "Native editor change did not synchronize",
    );
    ...
    const reading = lsp("request_lsp_hover", { languageId: "rust", uri, position: { line: 0, character: 2 } }).then(
      (result) => ({ result }),
      (error) => ({ error }),
    );
    let readingStarted = false;
    for (let attempt = 0; attempt < 100; attempt += 1) {
      if (existsSync(marker + ".hover")) {
        readingStarted = true;
        break;
      }
      await delay(10);
    ...
    await waitForRenderer(
      cdp,
      'window.__workspaceLspTrace.rows.filter(row=>row.method==="save_file").length===1&&window.__workspaceLspTrace.rows.some(row=>row.method==="save_file"&&row.phase==="succeeded")',
      "Native save did not complete exactly once",
    );
    assert.ok(readFileSync(file, "utf8").includes("// editor"));
    await waitForRenderer(
      cdp,
      'window.__workspaceLspTrace.rows.some(row=>row.method==="save_lsp_document"&&row.phase==="succeeded")',
      "Native editor didSave did not complete",
    );
    assert.equal((await hover()).stale, false);
    await click("언어 서버", 'document.querySelector(".workspace-feature-files")');
    ...
    await waitForRenderer(
      cdp,
      'document.querySelector(".lsp-panel").textContent.includes("실행 승인을 해제했습니다")',
      "Native LSP revocation did not settle",
    );
    assert.deepEqual(success(await lsp("language_server_statuses")), []);
    await click("닫기", 'document.querySelector(".lsp-panel")');
    ...
    success(
      await lsp("save_lsp_config", {
        ...
        nativeRevision: current.nativeRevision,
        recoverInvalid: false,
      }),
    );
    await cdp.evaluate(
      'Array.from(document.querySelectorAll(".workspace-feature-files .document-tab")).find(tab=>tab.textContent.includes("lsp-owner-main.rs"))?.querySelector(".tab-action").click()',
    );
    await waitForRenderer(
      cdp,
      '!Array.from(document.querySelectorAll(".workspace-feature-files [role=tab]")).some(tab=>tab.textContent.includes("lsp-owner-main.rs"))',
      "LSP editor fixture did not close",
    );
    return {
      explicitNativeExecutionReview: true,
      approvalDoesNotAutoStart: true,
      actualNodeLspInitialize: true,
      codeMirrorDidOpenChangeSaveHover: true,
      saveWaitsForNativeLspReadWithoutReplay: true,
      revocationConfirmsNativeShutdown: true,
    };
  } finally {
    const trace = await cdp.evaluate(
      "(()=>{const trace=window.__workspaceLspTrace;trace.restore();delete window.__workspaceLspTrace;return trace.rows;})()",
    );
...
async function exerciseWorkspaceLspRecovery({ cdp, root, directory, executable, call, success, waitForRenderer }) {
  ...
  try {
    ...
    const backups = path.join(
      ...
      "files",
      "views",
      `worktree-${description.context.worktreeId}`,
      "lsp",
      "runtime",
      "rename-backups",
    );
    assert.ok(existsSync(backups));
    const journalId = "native-recovery-fixture",
      journalDir = path.join(backups, journalId);
    mkdirSync(journalDir);
    const target = path.join(root, "lsp-recovery-fixture.rs"),
      backup = path.join(journalDir, "backup-fixture.bak");
    const before = "let before = 1;\r\n",
      after = "let after = 1;\r\n",
      hash = (text) => createHash("sha256").update(text).digest("hex");
    writeFileSync(target, after, { flag: "wx" });
    ...
    writeFileSync(
      ...
      JSON.stringify({
        schema: 1,
        planId: journalId,
        workspaceRoot: path.toNamespacedPath(realpathSync.native(root)),
        state: "rollbackfailed",
        entries: [
          {
            target: path.toNamespacedPath(realpathSync.native(target)),
            backup,
    ...
    rejected(await lsp("lsp_recovery_apply", { previewId: preview.previewId }));
    assert.equal(readFileSync(target, "utf8"), "external fixture edit");
    assert.equal(readFileSync(backup, "utf8"), before);
    writeFileSync(target, after);
    const click = async (label, scope = "document") => {
      const predicate = `Array.from((${scope})?.querySelectorAll("button")??[]).find(b=>b.textContent.trim()===${JSON.stringify(label)}&&!b.disabled)`;
      await waitForRenderer(cdp, `!!(${predicate})`, `Native recovery action unavailable: ${label}`);
      await cdp.evaluate(`(${predicate}).click()`);
    };
    await click("언어 서버", 'document.querySelector(".workspace-feature-files")');
    await click("복구 기록 확인", 'document.querySelector(".lsp-panel")');
    const records = success(await lsp("lsp_recovery_list")).records,
      index = records.findIndex((record) => record.journalId === journalId);
    assert.ok(index >= 0);
    assert.equal(records[index].available, true);
    await click(`기록 ${index + 1} 복구 검토`, 'document.querySelector(".lsp-panel")');
    await waitForRenderer(
      cdp,
      `document.querySelector('section[aria-label="이름 변경 복구"]')?.textContent.includes("let before")`,
      "Native recovery preview omitted original bytes",
    );
    ...
    await waitForRenderer(
      cdp,
      `document.querySelector('section[aria-label="이름 변경 복구"]')?.textContent.includes("원본 복원이 완료되었습니다")`,
    ...
    await click("닫기", 'document.querySelector(".lsp-panel")');
    return {
      nativeJournalPreviewDoesNotWrite: true,
      recoveryRejectsOpenEditorAndChangedBytes: true,
      recoveryCancelAndReplayRejected: true,
      explicitUiRecoveryRestoresCrlfWithoutServerApproval: true,
    };
  } finally {
    const trace = await cdp.evaluate(
      "(()=>{const trace=window.__workspaceLspTrace;trace.restore();delete window.__workspaceLspTrace;return trace.rows;})()",
