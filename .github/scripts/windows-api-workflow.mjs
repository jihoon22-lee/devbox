...
const server = createServer(async (request, response) => {
  ...
  hits++;
  credentialMatched = request.headers.authorization === `Bearer ${secret}`;
  requestBodyMatched = Buffer.concat(chunks).toString() === '{"fixture":true}';
  response.statusCode = credentialMatched ? 201 : 401;
...
async function wait(expression, label, timeout) {
  await until(() => ui.cdp.evaluate(expression), label, timeout);
}
...
async function input(selector, value) {
  await ui.cdp.evaluate(
    `(() => {const input=document.querySelector(${JSON.stringify(selector)});if(!input)throw new Error("fixture input missing");const prototype=input instanceof HTMLTextAreaElement?HTMLTextAreaElement.prototype:HTMLInputElement.prototype;Object.getOwnPropertyDescriptor(prototype,"value").set.call(input,${JSON.stringify(value)});input.dispatchEvent(new Event("input",{bubbles:true}));})()`,
  );
}
...
async function success(component, method, args) {
  const result = await call(component, method, args);
...
async function start() {
  ...
  const env = {
    ...process.env,
    DEVBOX_API_MIGRATION_FIXTURE_ROOT: root,
  ...
  const child = spawn(executable, [], { env, stdio: "ignore", windowsHide: true });
  live.add(child);
  await once(child, "spawn");
  const target = await waitForCdp(port, "Devbox API Studio");
...
async function stop() {
  ...
  await until(() => item.child.exitCode !== null || item.child.signalCode !== null, "product did not quit");
  assert.equal(item.child.exitCode, 0);
  item.cdp.close();
  live.delete(item.child);
  if (item.policy) {
    restoreElevatedCdpPolicy(item.policy);
    policies.delete(item.policy);
  }
  ui = null;
}
try {
  ...
  await ui.cdp.evaluate(
    'localStorage.setItem("apip-environments",JSON.stringify({version:1,environments:[{id:"s03-env",name:"S03 fixture",variables:[{key:"WEBHOOK_SECRET",value:"",secret:true}]}]}))',
  );
  ...
  await wait(
    ...
    "explicit environment selection did not expose the empty reference",
  );
  progress("capture");
  const capturePort = await unusedPort();
  await success("api-studio.webhooks", "start_server", { bind: "127.0.0.1", port: capturePort, allowLan: false });
  const captured = await fetch(`http://127.0.0.1:${capturePort}/s03`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${secret}` },
    body: '{"fixture":true}',
  });
  await captured.arrayBuffer();
  const captures = await success("api-studio.webhooks", "list_history");
  ...
  await success("api-studio.webhooks", "send_history_to_api", { historyId: captures[0].id });
  await wait(
    '!!document.querySelector(".api-feature-requests:not([hidden]) .handoff-dialog")',
    "capture preview did not open",
  );
  assert.equal(hits, 0);
  assert.equal(await ui.cdp.evaluate('document.querySelector(".url-input").value'), "");
  await click(".api-feature-requests .handoff-dialog", "적용");
  await wait(
    'document.querySelector(".url-input").value === "/s03" && !document.querySelector(".handoff-dialog")',
    "explicit capture apply failed",
  );
  assert.equal(hits, 0);
  progress("credential-reconnect");
  ...
  const answerPrompt = (event) => {
    ...
    promptAnswered = expected;
    promptError = !expected;
    void ui.cdp
      .send("Page.handleJavaScriptDialog", { accept: expected, ...(expected ? { promptText: secret } : {}) })
      .catch(() => {
        promptError = true;
      });
  };
  ...
  await input(
    `.header-row:nth-child(${headerIndex + 1}) input[placeholder="Value 또는 \${SECRET_NAME}"]`,
    "Bearer ${WEBHOOK_SECRET}",
  );
  await input(".url-input", endpoint);
  assert.equal(hits, 0);
  progress("explicit-send");
  await click(".api-feature-requests .request-bar", "보내기");
  await until(() => hits === 1, "explicit request was not sent");
  await wait(
    '!!document.querySelector(".resp-body")?.textContent.includes("s03-response")',
    "native response did not render",
  );
  assert.equal(credentialMatched, true);
  assert.equal(requestBodyMatched, true);
  const masked = await ui.cdp.evaluate('document.querySelector(".resp-body").textContent');
  ...
  await wait(
    ...
    "response selection was not applied",
  );
  progress("comparison");
  await click(".api-feature-transforms", "비교의 이전 입력으로");
  ...
  progress("mock-draft");
  await click('[aria-label="새 버전 차이 결과"]', "Mock 초안 만들기");
  await wait(
    '!!document.querySelector(".api-feature-webhooks:not([hidden]) .mock-draft-dialog")',
    "comparison result Mock preview did not open",
  );
  await click(".mock-draft-dialog", "현재 규칙 초안 대신 적용");
  await wait(
    `document.querySelector("#rule-body")?.value === ${JSON.stringify(changed)}`,
    "Mock editor did not receive the comparison input",
  );
  assert.equal((await success("api-studio.webhooks", "list_rules")).length, 0);
  assert.equal((await success("api-studio.webhooks", "server_status")).running, false);
  progress("knowledge-draft");
  await click("nav", "변환");
  await wait(
    '!!document.querySelector(".api-feature-transforms:not([hidden]) .diff-view")',
    "comparison state was lost on navigation",
  );
  await click('[aria-label="새 버전 차이 결과"]', "Knowledge 초안 보관");
  await click(".studio-knowledge-dialog", "마스킹 사본 보관");
  await wait(
    '!document.querySelector(".studio-knowledge-dialog") && document.querySelector(".api-feature-transforms").textContent.includes("API Studio에 마스킹한 초안을 보관했습니다")',
    "masked draft storage was not reported",
  );
  const drafts = await success("api-studio.transforms", "list_knowledge_drafts");
  assert.equal(drafts.length, 1);
  const draftId = drafts[0].artifact.id;
  const draft = await success("api-studio.transforms", "get_knowledge_draft", { id: draftId });
  assert.ok(draft.body.includes("s03-response"));
  ...
  assert.equal(reopened.body, draft.body);
  assert.equal(
    (await call("api-studio.api", "get_knowledge_draft", { id: draftId })).operation.outcome.state,
    "failed",
  );
  ...
  Object.assign(evidence, {
    ...
    captureMasked: true,
    explicitApply: true,
    nativeDpapiReconnect: true,
    oneExplicitSend: true,
    responseMasked: true,
    nativeComparison: true,
    mockEditorOnly: true,
    listenerStopped: true,
    maskedDraftStored: true,
    restartPreservesDraft: true,
    ownerIsolation: true,
  });
  progress("complete");
} catch (error) {
