...
const executable = path.join(root, `api-lifecycle-${randomUUID()}.exe`);
...
async function child(args, env) {
  const value = spawn(executable, args, { env, stdio: "ignore", windowsHide: true });
  live.add(value);
...
async function exited(value) {
  await until(() => value.exitCode !== null || value.signalCode !== null, "owned process did not exit");
  live.delete(value);
}
function powershell(value, action) {
  ...
  if (result.status !== 0) throw new Error("owned process window query failed");
...
async function startUi() {
  ...
  const process = await child([], env);
  const target = await waitForCdp(port, "Devbox API Studio");
  const cdp = new Cdp(target.webSocketDebuggerUrl);
  await cdp.connect();
  await until(
    () => cdp.evaluate('!!document.querySelector(".url-input")'),
    "interactive startup did not finish",
    30000,
  );
  return { process, cdp, env, policy, launchedAt, readyMs: Math.round(performance.now() - launchedAt) };
}
...
async function success(ui, method, args) {
  const result = await command(ui, method, args);
  assert.equal(result.operation.outcome.state, "succeeded", method);
  return result.value;
}
async function responding(port) {
  try {
    const response = await fetch(`http://127.0.0.1:${port}/lifecycle-fixture`, { signal: AbortSignal.timeout(500) });
...
function restore(ui) {
  ui.cdp.close();
  if (ui.policy) {
    restoreElevatedCdpPolicy(ui.policy);
    policies.delete(ui.policy);
  }
}
let ui;
try {
  ...
  evidence.performance = {
    ...
    conditions: {
      ...
      idle: "first UI process after ten seconds; before any listener or service; five-second owned-process sample",
      warm: "second UI lifecycle instance, hidden with an explicit listener; relaunch restores its existing window while an independent service remains running",
    },
    coldRendererReadyMs: ui.readyMs,
    firstKeyboardEventMs: await measureInput(ui.cdp),
    firstInputObservedMs: Math.round(performance.now() - ui.launchedAt),
  };
  progress("performance-idle");
  await delay(Math.max(0, 10000 - (performance.now() - ui.launchedAt)));
  assert.equal(ui.process.exitCode, null);
  assert.equal(ui.process.signalCode, null);
  const identity = allWindowsProcesses().find((item) => item.Pid === ui.process.pid);
  assert.ok(
    identity && identity.Path.toLowerCase() === executable.toLowerCase(),
    "measurement root identity does not match the owned executable",
  );
    ...
    assert.ok(
      current.some(
        (item) =>
          ...
          item.Created === identity.Created &&
          item.Path === identity.Path &&
          item.Name === identity.Name,
      ),
      "measurement root identity changed",
    );
  ...
  assert.equal(definition.services.length, 1);
  const service = definition.services[0];
  assert.equal(service.enabled, false);
  assert.equal(service.autoStart, false);
  assert.equal(service.restartPolicy, "never");
  assert.match(service.command, /--service-profile [a-f0-9-]{36}$/);
  assert.ok(service.command.includes(executable));
  progress("port-collision");
  const collision = await child(["--service-profile", service.id], environment());
  await exited(collision);
  ...
  ui = await startUi();
  assert.equal((await success(ui, "server_status")).running, false);
  const refused = await command(ui, "start_server", { bind: "127.0.0.1", port: servicePort, allowLan: false });
  ...
  const second = await child([], ui.env);
  await exited(second);
  ...
  assert.equal(ui.process.exitCode, null);
  assert.equal(ui.process.signalCode, null);
  evidence.performance.budget = evaluateBudgets(evidence.performance, performanceConfig, "api-studio");
  assert.equal(
    evidence.performance.budget.passed,
    true,
    `API Studio performance budget failed: ${evidence.performance.budget.violations.join(", ")}`,
  );
  assert.equal((await success(ui, "server_status")).running, true);
  progress("full-quit");
  ...
  await exited(ui.process);
  assert.equal(ui.process.exitCode, 0);
  ...
  Object.assign(evidence, {
    ...
    dualPortOwnerRejected: true,
    serviceOwnershipIndependent: true,
    allSocketsReleased: true,
    result: "pass",
  });
  progress("complete");
} catch (error) {
  ...
  for (const value of live) {
    if (value.exitCode === null && value.signalCode === null) value.kill();
  }
