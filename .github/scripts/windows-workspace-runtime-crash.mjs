...
const call = (cdp, method, args = {}) =>
  cdp.evaluate(workspaceRequestExpression("workspace.runtime", method, args, 29000), { timeoutMs: 35000 });
...
export async function prepareRuntimeCrash(cdp, directory) {
  const script = path.join(directory, "runtime-crash.cjs"),
    ready = path.join(directory, "runtime-crash-ready.json"),
    counter = path.join(directory, "runtime-crash-count.txt");
  writeFileSync(
    script,
    `const fs=require('node:fs'),http=require('node:http');fs.appendFileSync(${JSON.stringify(counter)},'start\\n');const server=http.createServer((_,r)=>r.end('fixture'));server.listen(0,'127.0.0.1',()=>fs.writeFileSync(${JSON.stringify(ready)},JSON.stringify({port:server.address().port,pid:process.pid})));`,
    { flag: "wx" },
  );
  const job = success(
    await call(cdp, "create_job", {
      input: {
        name: "Crash-owned Runtime fixture",
  ...
  const response = success(
    await call(cdp, "runtime_control", { operationId, method: "run_job_now", args: { id: job.id } }),
  );
  const deadline = performance.now() + 20000;
  while (!existsSync(ready) && performance.now() < deadline) await delay(100);
...
export async function verifyRuntimeCrash(cdp, fixture) {
  ...
  while (running && performance.now() < deadline) {
    running = !!success(await call(cdp, "get_active_run", { id: fixture.jobId }));
  ...
  return {
    nativeCrash: true,
    jobObjectDescendantsGone: true,
    restartSettlesOldOwnership: true,
    durableReceiptPreventsRelaunch: true,
  };
}
