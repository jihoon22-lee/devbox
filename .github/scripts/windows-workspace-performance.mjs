...
export async function measureWorkspaceStartup({
  cdp,
  child,
  executable,
  env,
  started,
  startupMs,
  product = { id: "workspace", label: "Devbox Workspace" },
}) {
  ...
  const measured = {
    ...
    build:
      process.env.DEVBOX_FIXTURE_PROFILE === "release"
        ? "exact candidate release portable"
    ...
    conditions: {
      ...
      comparison:
        process.env.DEVBOX_FIXTURE_PROFILE === "release"
          ? "same-job pinned anchor measurements in performance-baseline/runtime.json; unchanged B01 budgets"
          : "existing budgets; baseline is a different CI job",
    },
    coldRendererReadyMs: startupMs,
    result: "failed",
    stage: "input",
  };
  const record = () =>
    writeFileSync(`product-foundation-evidence/${product.id}-performance.json`, JSON.stringify(measured, null, 2));
  let secondary;
  try {
    ...
    const actual = lstatSync(identity.Path, { bigint: true }),
      owned = lstatSync(executable, { bigint: true });
    assert.equal(actual.dev, owned.dev);
    assert.equal(actual.ino, owned.ino);
    const identities = () => {
    ...
    while (
      secondary.exitCode === null &&
      secondary.signalCode === null &&
      performance.now() - warmStarted < config.budgets.warmExistingWindowMs
    )
      await delay(50);
    assert.equal(secondary.exitCode, 0, "second invocation retained another owner");
    measured.restoredWindow = await waitForOwnedRestoredWindow(
      child.pid,
      product.label,
      Math.max(1, config.budgets.warmExistingWindowMs - (performance.now() - warmStarted)),
    );
    ...
    if (product.id === "workspace")
      assert.equal(await cdp.evaluate('!!document.getElementById("workspace-project-path")'), true);
    ...
    assert.equal(
      measured.budget.passed,
      true,
      `Workspace performance budget failed: ${measured.budget.violations.join(", ")}`,
    );
    measured.stage = "complete";
