...
export function measureRoute(manifest, readAsset, spec) {
  const entries = spec.rootAppExcluding
    ? (manifest["index.html"]?.dynamicImports ?? [])
        .filter(
          (key) =>
            manifest[key]?.name === "App" &&
            !spec.rootAppExcluding.some((part) => (key + (manifest[key]?.src ?? "")).includes(part)),
        )
  ...
  function visit(key) {
    ...
    visited.add(key);
    assert.ok(visited.size <= 256, "route import graph exceeds its bound");
    const entry = manifest[key];
    assert.ok(entry && typeof entry.file === "string", "route import target is missing");
  ...
  for (const required of spec.deferred) {
    const matches = Object.entries(manifest).filter(([key, value]) =>
      required.suffix ? (value?.src ?? key).endsWith(required.suffix) : value?.name === required.name,
    );
  ...
  for (const file of files) {
    ...
    assert.ok(Buffer.isBuffer(bytes), "route asset must be bytes");
    for (const marker of spec.forbiddenMarkers)
      assert.equal(bytes.includes(Buffer.from(marker)), false, `${spec.label} eagerly loads a deferred engine`);
    rawBytes += bytes.length;
    gzipBytes += gzipSync(bytes, { level: 9, mtime: 0 }).length;
  }
  assert.ok(
    rawBytes <= spec.budget.rawBytes && gzipBytes <= spec.budget.gzipBytes,
    `${spec.label} exceeds its preserved ceiling`,
  );
...
export function reportBuiltRoute(product, measure, spec) {
  ...
  const report = measure(manifest, (file) => {
    ...
    const resolved = realpathSync(path.join(root, file)),
      relative = path.relative(realRoot, resolved);
    assert.ok(
      relative && !relative.startsWith("..") && !path.isAbsolute(relative),
      "route asset leaves the built output",
    );
    assert.ok(
      statSync(resolved).isFile() && statSync(resolved).size <= spec.budget.rawBytes,
      "route asset is invalid or too large",
    );
    return readFileSync(resolved);
  });
  console.log(
    `${spec.label}: ${report.rawBytes} raw / ${report.gzipBytes} gzip bytes; required engines deferred (preserved ceilings ${spec.budget.rawBytes}/${spec.budget.gzipBytes}).`,
  );
}
export function isInvoked(url) {
  return fileURLToPath(url).toLowerCase() === path.resolve(process.argv[1] ?? "").toLowerCase();
}
