...
export function requireHostedNetworkFixture(env = process.env) {
  if (
    ...
    env.GITHUB_REPOSITORY !== "jihoon22-lee/devbox" ||
    !/^\d+$/.test(env.GITHUB_RUN_ID ?? "")
  ) {
    throw new Error(
      "Network-changing fixtures are disabled on local and self-hosted machines; use a disposable GitHub-hosted Windows runner. Never spoof CI environment variables.",
    );
  }
