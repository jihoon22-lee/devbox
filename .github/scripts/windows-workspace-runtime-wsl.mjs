...
`;
  wsl(
    [
      "/usr/bin/python3",
      "-c",
      `import os,sys; os.mkdir('${root}'); open('${root}/listener.py','w').write(sys.stdin.read())`,
    ],
    script,
  );
  const runtime = (method, args = {}) => call("workspace.runtime", method, args, 29000);
  const control = (method, args, operationId = randomUUID()) =>
    runtime("runtime_control", { operationId, method, args });
  ...
  const input = {
    name: "Owned WSL Runtime fixture",
    command: `/usr/bin/python3 -u ${root}/listener.py`,
    cwd: root,
    targetKind: "wsl",
    targetDistro: distro,
    environment: { action: "replace", values: { FIXTURE_SECRET: "synthetic-wsl-private" } },
    restartPolicy: "never",
    autoStart: false,
    healthTcpAddress: null,
    healthTcpPort: null,
  };
  const service = success(await runtime("create_service", { input }));
  let stopped = false;
  try {
    const operationId = randomUUID();
    const start = success(await control("start_service", { id: service.id }, operationId));
    assert.deepEqual(success(await control("start_service", { id: service.id }, operationId)), start);
    const [pid, port] = await until(async () => {
      const value = wsl([
        "/usr/bin/python3",
        "-c",
        `import os; p='${root}/ready'; print(open(p).read() if os.path.exists(p) else '')`,
      ]);
      return value && value.split(" ").map(Number);
    }, "WSL child did not listen");
    const run = success(await runtime("get_active_run", { id: service.id }));
    assert.ok(run);
    ...
    if (wslVersion === 1) {
      ...
      const selection = await until(async () => {
        ...
        const row = rows.find(
          (row) => row.source === "wsl" && row.wsl_distro === distro && row.pid === pid && row.port === port,
        );
      ...
      const endpoint = {
        proto: selection.row.proto,
        local_addr: selection.row.local_addr,
        port: selection.row.port,
      ...
      assert.deepEqual(
        success(
          await call(
            ...
            "kill_listener",
            { request: { endpoint, identity: selection.row.identity } },
            29000,
          ),
        ),
        { kind: "ownedTask", taskId: service.id },
      );
      const stale = { ...selection.row.identity, start_tick: selection.row.identity.start_tick + 1 };
      assert.equal(
        (await call("workspace.process-actions", "kill_listener", { request: { endpoint, identity: stale } }, 29000))
          .operation.outcome.state,
        "failed",
      );
      listenerCorrelation = {
        state: "passed",
        groupDescendantOwnership: true,
        startTickMismatch: true,
        declaredConfidencePreserved: true,
      };
    }
    ...
    const logs = success(
      await call(
        "workspace.logs",
        "read_sources",
        { sources: resolved.sources, cursors: [null], sequenceStarts: [0], generation: 1, operationId: randomUUID() },
        29000,
      ),
    );
    const serialized = JSON.stringify(logs);
    assert.ok(serialized.includes("synthetic-wsl-listener"));
    assert.ok(!serialized.includes("synthetic-wsl-private"), "Runtime leaked the execution secret into logs");
    ...
    await until(
      async () =>
        wsl([
          "/usr/bin/python3",
          "-c",
          `import os; print('gone' if not os.path.exists('/proc/${pid}') else open('/proc/${pid}/stat').read().split(') ',1)[1].split()[0])`,
        ]) === "gone",
      "WSL group stop left a descendant",
    );
    const stoppedInstance = success(await runtime("get_service_instance", { id: service.id }));
    assert.equal(stoppedInstance.state, "stopped");
    assert.equal(stoppedInstance.nextRetryAt, null);
    // A failing service reaches backoff; stop cancels the generation's retry.
    const failing = success(
      await runtime("create_service", {
        input: {
          ...input,
          name: "WSL backoff fixture",
          command: "exit 7",
    ...
    try {
      ...
      await until(async () => {
        const instance = success(await runtime("get_service_instance", { id: failing.id }));
        return instance.state === "retry_waiting" && instance;
      }, "WSL service did not reach retry backoff");
      ...
      await delay(1200);
      const stopped = success(await runtime("get_service_instance", { id: failing.id }));
      assert.equal(stopped.state, "stopped");
      assert.equal(stopped.generation, generation);
      assert.equal(stopped.nextRetryAt, null);
    } finally {
      await control("stop_service", { id: failing.id }).catch(() => {});
      await runtime("delete_service", { id: failing.id }).catch(() => {});
    }
    return {
      wslVersion,
      ownedDistroOnly: true,
      duplicateGenerationReceipt: true,
      listenerCorrelation,
      platformSecretRedaction: true,
      groupStop: true,
      backoffStop: true,
    };
  } finally {
