...
export async function exerciseWorkspaceDefinitions({ cdp, root, call, success, waitForRenderer }) {
  ...
  const manifest = {
    schemaVersion: 1,
    tasks: { dev: { kind: "package-script", source: "package.json", selector: "dev" } },
  };
  const empty = success(await definitions("load"));
  const creation = success(
    await definitions("preview_edit", {
      target: "project",
      content: JSON.stringify(manifest),
      editRevision: empty.editRevision,
    }),
  );
  ...
  const writeSource = (label) =>
    writeFileSync(
      source,
      JSON.stringify({
        scripts: { dev: `node -e "require('fs').writeFileSync('unexpected-execution.txt','${label}')"` },
      }),
    );
  writeSource("initial");
  ...
  await waitForRenderer(
    cdp,
    `!!document.querySelector('.workspace-definitions [aria-label="실행 정의 승인 확인"]')`,
    "Definition review missing",
  );
  ...
  await waitForRenderer(
    cdp,
    'document.querySelector(".workspace-definitions")?.textContent.includes("현재 실행 정의의 승인을 확인했습니다.")',
    "Definition approval did not finish",
  );
  ...
  const edit = success(
    await definitions("preview_edit", {
      target: "project",
      content: JSON.stringify({ ...current.project, expectedPorts: [8080] }),
  ...
  const cancelled = success(
    await definitions("preview_edit", {
      target: "local",
      content: JSON.stringify({ ...refreshed.local, expectedPorts: [7777] }),
  ...
  assert.equal(
    (
      await definitions("preview_edit", {
        target: "local",
        content: JSON.stringify({ ...refreshed.local, apiEnvironmentId: "unowned-environment" }),
        editRevision: refreshed.editRevision,
      })
  ...
  await waitForRenderer(
    cdp,
    '!!document.querySelector(".workspace-definition-editor textarea")',
    "Definition editor missing",
  );
  await cdp.evaluate(
    `(()=>{const input=document.querySelector(".workspace-definition-editor textarea");Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype,"value").set.call(input,${JSON.stringify(JSON.stringify({ ...refreshed.project, expectedPorts: [8080] }))});input.dispatchEvent(new Event("input",{bubbles:true}));})()`,
  );
  await click("저장 변경 검토");
  await waitForRenderer(
    cdp,
    `!!document.querySelector('.workspace-definition-editor [aria-label="설정 저장 확인"]')`,
    "Definition save review missing",
  );
  assert.equal(readFileSync(manifestPath, "utf8"), external);
  await click("검토한 설정 저장");
  await waitForRenderer(
    cdp,
    'document.querySelector(".workspace-definition-editor")?.textContent.includes("설정을 저장했습니다.")',
    "Definition UI save missing",
  );
  ...
  const localEdit = success(
    await definitions("preview_edit", {
      target: "local",
      content: JSON.stringify({ ...forLocal.local, expectedPorts: [7777] }),
      editRevision: forLocal.editRevision,
    }),
  );
  ...
  await click("설정 닫기");
  return {
    reviewedManifestCreation: true,
    changedManifestPreserved: true,
    cancelledLocalEditPreserved: true,
    ownerReferenceDenied: true,
    uiReviewedSave: true,
    localPrecedence: true,
    explicitReviewCancel: true,
    changedSourceDenied: true,
    approvalRetainsWorktreeContext: true,
    changedSourceInvalidatesDigest: true,
    explicitRevoke: true,
    noSourceExecution: true,
  };
}
