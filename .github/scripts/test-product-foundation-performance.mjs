...
const measured = {
  coldRendererReadyMs: 2000,
  firstKeyboardEventMs: 20,
  warmExistingWindowMs: 1000,
  idle,
  workload: null,
};
...
const cdp = (invoke) => ({
  evaluate: (expression) => runInNewContext(expression, { window: { __TAURI_INTERNALS__: { invoke } } }),
});
let profile;
const terminal = await measureWorkload(
  { id: "wsl-desktop" },
  cdp(async (command, args) => {
    if (command === "list_sessions") return [];
    if (command === "save_workspace_profile") {
      profile = { ...args.profile, id: "fixture-profile" };
      return profile;
    }
...
assert.equal(
  evaluateBudgets({ ...measured, workload: failedRun }, { ...config, baselineCommit: "f".repeat(40) }, "run-manager")
    .knownBaselineFailureObserved,
  false,
);
assert.equal(
  evaluateBudgets({ ...measured, workload: failedRun }, config, "workbench").knownBaselineFailureObserved,
  false,
);
assert.equal(
  evaluateBudgets({ ...measured, idle: reused, workload: failedRun }, config, "run-manager")
    .knownBaselineFailureObserved,
  false,
);
assert.equal(
  evaluateBudgets(
    { ...measured, workload: { ...failedRun, failure: { ...failedRun.failure, runFailureCode: "storage-failed" } } },
    config,
    "run-manager",
  ).knownBaselineFailureObserved,
  false,
