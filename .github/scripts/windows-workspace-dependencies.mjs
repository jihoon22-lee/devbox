...
export async function exerciseWorkspaceDependencies({ cdp, root, call, success, waitForRenderer }) {
  const dependencies = (method, request = {}) =>
    call("workspace.dependencies", method, { request: { path: root, ...request } });
  ...
  const writeLock = (version) =>
    writeFileSync(
      ...
      JSON.stringify({
        ...
        lockfileVersion: 3,
        packages: {
          "": { name: "owned-fixture", version: "1.0.0", dependencies: { "fixture-dependency": "1.2.3" } },
          "node_modules/fixture-dependency": {
            version,
            resolved: `https://registry.npmjs.org/fixture-dependency/-/fixture-dependency-${version}.tgz`,
          },
        },
      }),
    );
  ...
  writeLock("1.2.3");
  await cdp.evaluate(
    `(async()=>{const d=await window.__TAURI_INTERNALS__.invoke("plugin:product-shell|describe");const label=d.features.find(f=>f.route==="dependencies").label;Array.from(document.querySelectorAll('nav[aria-label="제품 화면"] button')).find(b=>b.textContent.trim()===label).click();})()`,
  );
  ...
  await waitForRenderer(
    cdp,
    'document.querySelector(".dependency-lens-panel")?.textContent.includes("fixture-dependency")',
    "Dependency inventory missing",
  );
  await click("전송 내용 검토");
  await waitForRenderer(
    cdp,
    '!!document.querySelector(".dependency-enrichment-preview")',
    "Dependency preview missing",
  );
  assert.equal(
    await cdp.evaluate(
      'Array.from(document.querySelectorAll(".workspace-registry button")).find(b=>b.textContent.trim()==="프로젝트 선택 해제").disabled',
    ),
    true,
  );
  ...
  await waitForRenderer(
    cdp,
    '!document.querySelector(".dependency-enrichment-preview") && !Array.from(document.querySelectorAll(".workspace-registry button")).find(b=>b.textContent.trim()==="프로젝트 선택 해제").disabled',
    "Dependency cancellation did not release selection",
  );
  ...
  return {
    plainFolderOfflineAnalysis: true,
    rendererPathRejected: true,
    reviewCancelledAndReplayRejected: true,
    changedLockRejectedBeforeNetwork: true,
    uiAnalyzeReviewCancel: true,
    selectionLockedDuringReview: true,
    sourceScriptsNeverExecuted: true,
    boundary:
      "Native parser and approval/cancellation only; public OSV/deps.dev requests are not issued by this fixture",
  };
}
