...
export const nativeFileSnapshot = (doc) => ({
  path: doc.path,
  nativeRevision: doc.nativeRevision,
  expectedMtimeNanos: doc.mtimeNanos,
  expectedSize: doc.size,
  expectedContentHash: doc.contentHash,
});
export const nativeFileSave = (doc, text) => ({
  ...nativeFileSnapshot(doc),
  text,
  encoding: doc.encoding,
  lineEnding: doc.lineEnding,
  sourceLossy: doc.lossy,
});
...
export async function nativeFileDialog({ processId, executable, directory, action, selectedFile, selectedFiles }) {
  assert.ok(
    ...
    "An owned product process is required for dialog acceptance",
  );
  const args = [
    "-NoProfile",
    "-NonInteractive",
  ...
  try {
    ...
    result.state = "failed";
    result.diagnostic = String(error.stderr ?? error.message).slice(0, 4000);
    throw error;
  } finally {
    result.elapsedMs = Math.round(performance.now() - started);
    writeFileSync(
      `product-foundation-evidence/workspace-native-dialog-${Date.now()}.json`,
      JSON.stringify(result, null, 2),
    );
  }
}

export async function exerciseWorkspaceFiles({
  cdp,
  root,
  directory,
  call,
  ...
  const [cancelResult] = await Promise.all([
    cancelledChoice,
    nativeFileDialog({ processId, executable, directory, action: "Cancel" }),
  ]);
  assert.deepEqual(success(cancelResult), []);
  rejected(await files("open_file", { request: { path: outside, encoding: null } }));
  const approvedChoice = files("pick_files");
  const [choiceResult] = await Promise.all([
    approvedChoice,
    nativeFileDialog({ processId, executable, directory, action: "Open", selectedFile: outside }),
  ]);
  const chosen = success(choiceResult);
  ...
  const renamed = success(
    await files("rename_file_action", { request: { ...snapshot(fresh), newName: "renamed.txt" } }),
  );
  assert.equal(existsSync(file), false);
  assert.equal(readFileSync(renamed.path, "utf8"), "changed\r\n");
  const recoverable = await open(renamed.path);
  const recoveryRevision = success(
    await files("save_recovery", {
      nativeRevision: success(await files("load_recovery")).nativeRevision,
      entries: [
        {
          path: recoverable.path,
          content: "recovered\n",
          base_hash: recoverable.contentHash,
  ...
  const navigate = async (route) => {
    await cdp.evaluate(
      `(async () => {const d=await window.__TAURI_INTERNALS__.invoke("plugin:product-shell|describe"); const label=d.features.find(f=>f.route===${JSON.stringify(route)}).label;Array.from(document.querySelectorAll('nav[aria-label="제품 화면"] button')).find(b=>b.textContent.trim()===label).click();})()`,
    );
  };
  await navigate("files");
  await waitForRenderer(
    cdp,
    '!!document.querySelector(".workspace-feature-files:not([hidden]) #path-input:not(:disabled)")',
    "Native Files did not hydrate",
  );
  const uiFile = path.join(root, "ui-draft.txt");
  writeFileSync(uiFile, "ui original", { flag: "wx" });
  await cdp.evaluate(
    `(() => {const input=document.getElementById("path-input");Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"value").set.call(input,${JSON.stringify(uiFile)});input.dispatchEvent(new Event("input",{bubbles:true}));})()`,
  );
  await cdp.evaluate(
    'Array.from(document.querySelectorAll(".workspace-feature-files button")).find(b=>b.textContent.trim()==="파일 열기").click()',
  );
  await waitForRenderer(
    cdp,
    '!!document.querySelector(".workspace-feature-files .cm-content")',
    "Native file did not open in CodeMirror",
  );
  ...
  const dirty =
    'Array.from(document.querySelectorAll(".workspace-feature-files [role=tab]")).some(t=>t.textContent.includes("●"))';
  ...
  await waitForRenderer(
    cdp,
    'Array.from(document.querySelectorAll(".workspace-registry button")).some(b=>b.textContent.trim()==="프로젝트 선택 해제" && b.disabled)',
    "Dirty native buffer did not block context change",
  );
  await cdp.evaluate(
  ...
  await cdp.evaluate(
    'Array.from(document.querySelectorAll(".workspace-feature-files button")).find(b=>b.textContent.trim()==="저장").click()',
  );
  ...
  await waitForRenderer(
    ...
    "Saved native tab did not close",
  );
  await navigate("overview");
  await waitForRenderer(
    cdp,
    'Array.from(document.querySelectorAll(".workspace-registry button")).some(b=>b.textContent.trim()==="프로젝트 선택 해제" && !b.disabled)',
    "Clean Files did not release context selection",
  );
  return {
    projectScopeDeniedOutside: true,
    nativeDialogCancelGrantsNothing: true,
    nativeDialogSelectionGrantsOnlyChosenFile: true,
    opaqueRevisionAndReplacementChecked: true,
    crlfSaveRenameDelete: true,
    recoveryCancelAndOneTimeApply: true,
    actualEditorSave: true,
    retainedDirtyDraftAndContextLock: true,
    hiddenShortcutIgnored: true,
  };
}
