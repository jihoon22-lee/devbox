...
export function workspaceRequestExpression(
  component,
  method,
  args = {},
  budgetMs = ["workspace.dependencies", "workspace.source", "workspace.lsp"].includes(component) ? 29000 : 5000,
) {
  assert.ok(
    Number.isInteger(budgetMs) && budgetMs >= 100 && budgetMs <= 29000,
    "fixture deadline outside native bounds",
  );
  const route =
...
export async function exerciseWorkspaceRegistration({
  cdp,
  directory,
  waitForRenderer,
  suffix,
  ...
  const call = async (component, method, args = {}, budgetMs) => {
    ...
    const record = () =>
      writeFileSync(
        `product-foundation-evidence/workspace-native-requests-${suffix}.json`,
        JSON.stringify(requests, null, 2),
      );
    record();
    try {
      const result = await cdp.evaluate(workspaceRequestExpression(component, method, args, budgetMs), {
        timeoutMs: method === "lsp_install" ? 660_000 : 35_000,
      });
      row.state = result?.operation?.outcome?.state ?? "invalid";
      if (typeof result?.value?.issue === "string" && /^[a-z_]{1,80}$/.test(result.value.issue))
        row.issue = result.value.issue;
      return result;
    } catch (error) {
      row.state = "transport-failed";
      throw error;
    } finally {
  ...
  await waitForRenderer(
    ...
    "Workspace registry did not activate",
  );
  ...
  await waitForRenderer(
    cdp,
    '!!Array.from(document.querySelectorAll(".workspace-registry button")).find(button => button.textContent.trim() === "프로젝트 선택 해제")',
    "Workspace selected context did not refresh",
  );
  ...
  assert.equal(
    (
      await call("workspace.registry", "select_project", {
        context: { ...selected.context, revision: selected.context.revision + 1 },
      })
  ...
  const record = (feature, checks) =>
    writeFileSync(
      `product-foundation-evidence/workspace-${feature}-${suffix}.json`,
      JSON.stringify(
        { source: process.env.GITHUB_SHA, environment: "github-hosted-windows", result: "pass", checks },
        null,
        2,
      ),
    );
  ...
  const dependencies = await exerciseWorkspaceDependencies({
    cdp,
    root: canonicalRoot,
    call,
    success,
  ...
  const source = await exerciseWorkspaceSource({
    cdp,
    directory,
    call,
    success,
  ...
  registry = success(await call("workspace.registry", "snapshot"));
  const files = await exerciseWorkspaceFiles({
    cdp,
    root: canonicalRoot,
    directory,
    call,
  ...
  record("files", files);
  const runtime = await exerciseWorkspaceRuntime({
    cdp,
    directory,
    call,
    success,
    waitForRenderer,
    measurePerformance: suffix === "a" && process.env.DEVBOX_FIXTURE_PROFILE === "release",
  });
  ...
  await waitForRenderer(
    ...
    "Workspace context did not reload after native Session cleanup",
  );
  const lspInstaller = await exerciseWorkspaceLspInstaller({
    cdp,
    root: canonicalRoot,
    directory,
    call,
  ...
  await cdp.evaluate(
    `(async()=>{const d=await window.__TAURI_INTERNALS__.invoke("plugin:product-shell|describe");const label=d.features.find(f=>f.route==="overview").label;Array.from(document.querySelectorAll('nav[aria-label="제품 화면"] button')).find(b=>b.textContent.trim()===label).click();})()`,
  );
  await click("프로젝트 선택 해제");
  await waitForRenderer(
    cdp,
    '!Array.from(document.querySelectorAll(".workspace-registry button")).find(button => button.textContent.trim() === "프로젝트 선택 해제")',
    "Workspace context did not clear",
  );
  assert.equal(
  ...
  registry = success(await call("workspace.registry", "snapshot"));
  registry = success(
    await call("workspace.registry", "rename", {
  ...
  assert.equal(registry.worktrees.length, 0);
  assert.equal(readFileSync(marker, "utf8"), "synthetic project bytes\r\n");
  await click("목록 새로 고침");
  await waitForRenderer(
    cdp,
    '(document.querySelector(".workspace-registry")?.textContent ?? "").includes("등록한 프로젝트가 없습니다.")',
    "Workspace empty registry did not refresh",
  );
  ...
  return {
    ...
    previewCancelDidNotRegister: true,
    explicitRegistrationUntrusted: true,
    selectedContextAndStaleHeaderChecked: true,
    cancelledPreviewRejected: true,
    renameRemovePreservedProjectFiles: true,
    boundary:
