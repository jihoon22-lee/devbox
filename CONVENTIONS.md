...
- 선행 묶음의 계획한 구현과 발견된 차단 결함을 먼저 마무리한다. 미완료 기능이나
  미해결 결함을 후속 묶음으로 덮지 않는다. 구현·네이티브 수용이 끝나 최종 CI·머지만
  남은 경우, 선행 소스를 고정한 기존 후속 브랜치에서 승인된 개발을 계속할 수 있다.
  병합·이슈 완료는 선행 순서와 최종 CI 통과 조건을 지킨다. CI 대기 자체를 개발 중단의
  이유로 만들거나 반복 검증 축소를 완료 조건의 생략으로 해석하지 않는다.
