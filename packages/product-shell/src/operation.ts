...
export type OperationOutcome =
  | { state: "running" | "succeeded" | "cancelled" | "stale" }
  | { state: "failed"; code: ProblemCode };
...
export function matchesProvenance(value: unknown, expected: Provenance): value is Provenance {
  return (
    record(value) &&
    Object.keys(value).sort().join(",") === "component,product,requestId,revision" &&
...
export function isOperation(value: unknown, expected: Provenance): value is Operation {
  if (
    !record(value) ||
    Object.keys(value).sort().join(",") !== "outcome,provenance" ||
...
export function operationMessage(operation: Operation): string {
  switch (operation.outcome.state) {
    ...
    case "cancelled":
      return "작업이 취소되었습니다.";
    case "stale":
      return "이 결과는 이전 상태의 결과입니다. 화면을 다시 열어 주세요.";
    case "failed":
      return messages[operation.outcome.code];
  }
}
