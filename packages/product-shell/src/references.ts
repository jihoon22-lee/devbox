...
export function isApiArtifactReference(value: unknown, expected: Provenance): boolean {
  if (
    expected.product !== "api-studio" ||
    expected.component !== "api-studio.api" ||
    !shape(value, "link,provenance,recipient") ||
    !matchesProvenance(value.provenance, expected) ||
    value.recipient !== "developer-toolbox" ||
    !shape(value.link, "from,target") ||
    value.link.from !== "api-playground" ||
    !shape(value.link.target, "handoffKind,id,kind")
  )
    return false;
  return (
    value.link.target.kind === "handoff" &&
    value.link.target.handoffKind === "toolbox-text/v1" &&
    typeof value.link.target.id === "string" &&
    /^[a-f0-9]{32}$/.test(value.link.target.id)
  );
}
...
export function isOwnedSecretReference(value: unknown, expected: Provenance, owner: SecretOwner): boolean {
  ...
  return (
    ...
    shape(value, "owner,provenance,reference") &&
    value.owner === owner &&
    matchesProvenance(value.provenance, expected) &&
    shape(value.reference, "kind,name") &&
    value.reference.kind === "secret-ref/v1" &&
