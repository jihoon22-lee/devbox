...
const phases = {
  review: "검토 대기",
  running: "진행 중",
  cancelRequested: "취소 요청됨 · 종료 확인 중",
  cancelled: "취소 완료",
  uncancellable: "취소할 수 없는 단계",
  succeeded: "완료",
  failed: "실패",
  unknown: "담당 화면에서 상태 확인 필요",
} as const;
interface Row {
  id: string;
  owner: string;
...
function parse(value: unknown, owner: string): Row[] {
  ...
  for (const row of value) {
    if (
      !row ||
      typeof row !== "object" ||
      Object.keys(row).sort().join(",") !== "component,id,label,owner,phase,revision,route" ||
      ...
      /[\u0000-\u001f\u007f]/.test(row.label) ||
      !Object.prototype.hasOwnProperty.call(phases, row.phase) ||
      typeof row.revision !== "string" ||
      !/^[a-f0-9]{64}$/.test(row.revision) ||
      !catalog.features.some((feature) => feature.owner === owner && feature.route === row.route) ||
      !catalog.components.some((component) => component.owner === owner && component.id === row.component)
    )
...
export default function Operations({ description, route }: { description: Description; route: string }) {
  ...
  const call = useCallback(
    async <T,>(method: object): Promise<T> => {
      ...
      const response = await invoke<{ operation: unknown; value: T }>("plugin:suite|connection", {
        request: { header, method },
      });
      if (!isOperation(response.operation, provenance) || response.operation.outcome.state !== "succeeded")
        throw new Error("operation source unavailable");
  ...
  useEffect(() => {
    ...
    let active = true,
      ...
      inFlight = false;
    const products =
      description.product.id === "control-center"
    ...
    const refresh = async () => {
      ...
      const results = await Promise.allSettled(
        products.map(async (product) => parse(await call({ kind: "readOperations", product }), product)),
      );
      if (active) {
        ...
        results.forEach((result, index) => {
          ...
          next[product] = {
            ...
            unavailable: result.status !== "fulfilled",
          };
          if (result.status === "fulfilled")
            for (const row of result.value) {
              const key = product + ":" + row.id;
              const previous = seen.current.get(key);
              if (previous && previous !== row.revision && ["succeeded", "failed", "cancelled"].includes(row.phase))
                notifications.push(`${row.label}: ${phases[row.phase]}`);
              seen.current.set(key, row.revision);
            }
        });
        while (seen.current.size > 512) seen.current.delete(seen.current.keys().next().value!);
        setSources(next);
        if (notifications.length) setNotice(notifications.slice(0, 3).join(" · "));
      }
      inFlight = false;
  ...
  const review = async (row: Row) => {
    ...
    try {
      ...
      setNotice("담당 제품에 화면 열기 검토를 요청했습니다.");
    } catch {
      setIssue("작업 상태가 바뀌었거나 제품 연결을 확인하지 못했습니다. 새로고침 후 다시 확인해 주세요.");
    } finally {
      setBusy(false);
    }
  };
  return (
    <section aria-label="작업 상태">
      <h2>작업 상태</h2>
      <p>각 제품이 보고한 작업입니다. 승인과 취소는 담당 화면에서 확인해 주세요.</p>
      <button onClick={() => setRevision((value) => value + 1)}>새로고침</button>
      {!nativeMode && <p>데스크톱 제품에서 작업 상태를 확인할 수 있습니다.</p>}
      {catalog.products
        .filter((product) => description.product.id === "control-center" || product.id === description.product.id)
        .map((product) => {
          const source = sources[product.id];
          return (
            <section key={product.id} aria-label={product.label}>
              <h3>{product.label}</h3>
              {!source ? (
                ...
                <ul>
                  {source.rows.map((row) => (
                    <li key={row.id}>
                      <strong>{row.label}</strong> · {phases[row.phase]} <small>{row.id.slice(0, 8)}</small>{" "}
