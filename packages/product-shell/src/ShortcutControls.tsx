...
export default function ShortcutControls({
  status,
  busy,
  onChange,
  ...
  const config: ShortcutConfig = {
    ...
    ...(status.terminal === undefined
      ? {}
      : { terminal: status.terminal, capture: status.capture, project: status.project }),
  };
  return (
    <>
      {!hideAccelerator && (
        <label>
          ...
          <select
            disabled={busy}
            value={status.accelerator}
            onChange={(event) =>
              onChange({ ...config, accelerator: event.target.value as ShortcutConfig["accelerator"] })
            }
          >
      ...
      {typeof status.terminal === "boolean" && (
        <fieldset disabled={busy}>
          ...
          {(
            [
              ["terminal", "터미널 표시·숨김 · Ctrl+Alt+T"],
              ["capture", "빠른 캡처 · Ctrl+Alt+N"],
              ["project", "현재 프로젝트 · Ctrl+Alt+P"],
            ] as const
          ).map(([key, label]) => (
            <label key={key}>
              <input
                type="checkbox"
                checked={status[key] === true}
