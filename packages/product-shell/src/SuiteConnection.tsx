...
interface Review {
  ...
  generation: string;
  products: { product: string; available: boolean }[];
}
...
const labels: Record<string, string> = Object.fromEntries(
  catalog.products.map((product) => [product.id, product.label]),
);
const issues: Record<string, string> = {
  suite_package_unavailable:
    "설치형 Suite로 실행할 때만 제품을 연결할 수 있습니다. 개발 빌드나 따로 푼 ZIP은 연결하지 않습니다.",
...
export default function SuiteConnection({ description, route }: { description: Description; route: string }) {
  ...
  const call = useCallback(
    async <T,>(method: object): Promise<T> => {
      ...
      const response = await invoke<{ operation: Operation; value: T }>("plugin:suite|connection", {
        request: { header, method },
      });
      if (!isOperation(response.operation, provenance) || response.operation.outcome.state !== "succeeded")
        throw new Error("제품 연결 응답을 확인할 수 없습니다.");
  ...
  const perform = async (action: () => Promise<void>) => {
    ...
    try {
      ...
      setIssue("제품 연결을 완료하지 못했습니다. 설치 상태를 확인한 뒤 다시 시도해 주세요.");
    } finally {
      statusRevision.current++;
      setBusy(false);
    }
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        /* The initial status remains available if listening fails. */
      }
      if (active) refresh();
    })();
    return () => {
  ...
  return (
    <section aria-label="제품 연결">
      ...
      {status?.connected ? (
        <>
          ...
          <button
            ...
            onClick={() =>
              void perform(async () => {
                setStatus(await call<ConnectionStatus>({ kind: "disconnect" }));
                setReview(null);
              })
            }
          >
        ...
        <>
          {status?.issue && (
            <p role="status">{issues[status.issue] ?? "이 실행 파일은 제품 연결을 사용할 수 없습니다."}</p>
          )}
          ...
          <button
            ...
            onClick={() =>
              void perform(async () => {
                const current = await call<ConnectionStatus>({ kind: "status" });
                setStatus(current);
      ...
      {review && !status?.connected && (
        <div>
          <p>Devbox {review.version}</p>
          <p>{review.root}</p>
          <ul>
            {review.products.map((product) => (
              <li key={product.product}>
                {labels[product.product] ?? product.product} · {product.available ? "연결 가능" : "파일 확인 필요"}
              </li>
            ))}
          </ul>
          <button
            ...
            onClick={() =>
              void perform(async () => {
                setStatus(await call<ConnectionStatus>({ kind: "approve", token: review.token, remember: true }));
                setReview(null);
              })
            }
          >
