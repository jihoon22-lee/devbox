...
class RouteBoundary extends Component<{ children: ReactNode }, { error: boolean }> {
  ...
  render() {
    return this.state.error ? (
      <p role="alert">화면을 불러오지 못했습니다. 앱을 다시 열어 주세요.</p>
    ) : (
      this.props.children
    );
  }
...
function ReadyShell({
  ...
  const initial = description.features.some((f) => f.route === queryRoute)
    ? queryRoute!
    : description.product.defaultRoute;
  ...
  const open = useCallback(
    (route: string) => {
      ...
      setVisited((v) => new Set([...v, route]));
      setHistory((h) => navigate(h, route));
    },
    [description],
  );
  return (
    <IncomingReviewContext.Provider value={{ review: incomingReview, clear: clearIncomingReview }}>
      <div
      ...
      >
        <a className="shell-skip" href="#product-content" onClick={() => content.current?.focus()}>
          본문으로 이동
        </a>
        <header>
          <strong>{description.product.label}</strong>
          {!nativeMode && <span className="shell-badge">브라우저 미리보기 · 모의 데이터</span>}
        </header>
        {description.deliveryState && !["direct", "committed"].includes(description.deliveryState) && (
          <p role="status" className="shell-delivery-notice">
            {description.deliveryState === "import"
              ? "설치를 마무리하고 있습니다. Control Center의 데이터 및 복구 화면에서 활성화를 완료해 주세요. 일반 작업과 자동 실행은 대기합니다."
              : description.deliveryState === "health"
                ? "Suite 활성화 전 제품 상태를 확인하고 있습니다. 일반 작업은 아직 시작할 수 없습니다."
                : "Suite 전환 상태를 확인해야 합니다. Control Center의 설치·데이터 복구 화면에서 기록을 확인해 주세요."}
          </p>
        )}
        <aside>
          <nav aria-label="제품 화면">
            {description.features.map((feature) => (
              <button
                key={feature.id}
                aria-current={current === feature.route ? "page" : undefined}
                onClick={() => open(feature.route)}
              >
                {feature.label}
              </button>
            ))}
        ...
        <main id="product-content" ref={content} tabIndex={-1}>
          <div className="shell-toolbar">
            <button
              ...
              onClick={() => setHistory((h) => traverse(h, -1))}
            >
              ←
            </button>
            <button
            ...
            <span>{description.context ? "프로젝트 연결됨" : "프로젝트 선택 없이 사용"}</span>
          ...
          {connectionOpen && (
            <Suspense fallback={<p role="status">연결 설정을 불러오고 있습니다…</p>}>
              <SuiteConnection description={description} route={current} />
            </Suspense>
          )}
          {!online && <p role="status">오프라인입니다. 로컬 화면은 계속 사용할 수 있습니다.</p>}
          {queryRoute && !description.features.some((f) => f.route === queryRoute) && (
            <p role="status">요청한 화면이 없어 기본 화면을 열었습니다.</p>
          )}
          {renderContent
            ? renderContent({ description, route: current, navigate: open, refreshContext })
            : description.features
                .filter((f) => visited.has(f.route))
                .map((feature) => (
                  <div key={feature.id} hidden={feature.route !== current}>
                    <RouteBoundary>
                      <Suspense fallback={<p role="status">화면을 불러오고 있습니다…</p>}>
                        <RouteView description={description} feature={feature} />
                      </Suspense>
                    </RouteBoundary>
                  </div>
                ))}
...
export function ProductShell({ product, renderContent }: { product: ProductId; renderContent?: ShellContent }) {
  ...
  useEffect(() => {
    ...
    setDescription(null);
    setError(false);
    void currentDescription(product).then(
      (value) => {
        if (active && id === loadId.current) {
          publishDescription(value);
          setDescription(value);
        }
      },
      () => {
        if (active && id === loadId.current) setError(true);
      },
    );
    return () => {
      active = false;
      loadId.current += 1;
      invalidateDescription(product);
    };
  }, [product, attempt]);
  const refreshContext = useCallback(async () => {
    ...
    if (
      !description ||
      next.handshake.sessionId !== description.handshake.sessionId ||
      next.handshake.installationId !== description.handshake.installationId
    )
      throw new Error("제품 연결이 변경되었습니다. 앱을 다시 열어 주세요.");
  ...
  if (error)
    return (
      <main role="alert">
        <h1>제품 연결을 확인할 수 없습니다</h1>
        <button onClick={() => setAttempt((v) => v + 1)}>다시 시도</button>
      </main>
    );
  if (!description || description.product.id !== product)
    return (
      <main>
        <p role="status">제품을 열고 있습니다…</p>
      </main>
    );
  return (
