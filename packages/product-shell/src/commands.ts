...
export interface Command {
  ...
  reviewRoute?: string;
  requiredContext: "none" | "project" | "selection";
...
export interface CommandSearch {
  ...
  state?: string;
  partial?: boolean;
  freshness?: Record<string, { availability: string; indexStale: boolean }>;
}
...
function command(value: unknown): value is Command {
  ...
  if (
    ...
    !row.id.startsWith(row.owner + ".") ||
    typeof row.component !== "string" ||
    !row.component.startsWith(row.owner + ".") ||
    ...
    typeof row.requiresReview !== "boolean" ||
    (row.reviewRoute !== undefined &&
      (typeof row.reviewRoute !== "string" || !/^[a-z0-9-]{1,96}$/.test(row.reviewRoute))) ||
  ...
  return target.kind === "route"
    ? typeof target.route === "string" && /^[a-z0-9-]{1,96}$/.test(target.route)
    : target.kind === "entity" &&
...
async function call(
  ...
  if (["command_preview", "command_open", "command_trigger_shortcut"].includes(method))
    header.deadlineMs = Date.now() + 29000;
  const cancel = () => {
    if (method === "command_source" && typeof args.product === "string")
      void call(description, route, "command_cancel", { product: args.product, queryId: header.requestId }).catch(
        () => undefined,
      );
  };
  ...
  const provenance = {
    ...
    component: description.product.id + ".commands",
    requestId: header.requestId,
    revision: catalog.catalogRevision,
  };
  try {
    const response = await invoke<{ operation: unknown; value: unknown }>("plugin:commands|" + method, {
      request: { header, ...args },
    });
    ...
    throw new Error(problemMessage(error, provenance));
  } finally {
    signal?.removeEventListener("abort", cancel);
  }
}
export async function searchCommands(description: Description, route: string, query: string): Promise<CommandSearch> {
  if (new TextEncoder().encode(query).length > 512 || /[\x00-\x1f\x7f]/.test(query))
    throw new Error("검색어 길이와 내용을 확인해 주세요.");
  if (!nativeMode) {
    const needle = query.trim().toLocaleLowerCase();
    return {
      truncated: false,
      results: catalog.features
        .filter((feature) => feature.label.toLocaleLowerCase().includes(needle) || feature.command.includes(needle))
        .map((feature) => ({
          id: feature.command,
          owner: feature.owner,
          ...
          target: { kind: "route", route: feature.route },
  ...
  if (
    ...
    value.results.length > 256 ||
    !value.results.every(command) ||
...
export async function previewCommand(
  ...
  const value = await call(description, route, "command_preview", {
    command: {
      operationId,
      commandId: item.id,
  ...
  if (!command(value) || value.id !== item.id || value.revision !== item.revision)
    throw new Error("명령의 현재 버전이 바뀌었습니다.");
  return value;
}
export interface CommandReceipt {
  operationId: string;
  phase: "awaitingReview" | "opening" | "opened" | "rejected" | "expired";
}
function receipt(value: unknown, id: string): CommandReceipt {
  if (
    ...
    !("phase" in value) ||
    !["awaitingReview", "opening", "opened", "rejected", "expired"].includes(String(value.phase))
  )
    throw new Error("명령 전달 결과를 확인하지 못했습니다.");
  return value as CommandReceipt;
}
export async function searchCommandSource(
  description: Description,
  route: string,
  product: string,
  query: string,
  ...
  if (
    ...
    throw new Error("검색 출처가 일치하지 않습니다.");
  const result = value.result;
  if (
    !("results" in result) ||
    !Array.isArray(result.results) ||
    result.results.length > 256 ||
    !result.results.every((row) => command(row) && row.owner === product) ||
    !("truncated" in result) ||
  ...
  if (extra.freshness && typeof extra.freshness === "object" && !Array.isArray(extra.freshness)) {
    for (const item of result.results) {
      ...
      if (
        ...
        !["available", "unverified", "stale"].includes(row.availability) ||
        !("indexStale" in row) ||
        typeof row.indexStale !== "boolean"
      )
        throw new Error("검색 결과의 최신 상태를 확인하지 못했습니다.");
      freshness[item.id] = { availability: row.availability, indexStale: row.indexStale };
    }
  }
  const state =
    typeof extra.state === "string" &&
    ["complete", "timed_out", "unavailable", "unsupported", "cancelled"].includes(extra.state)
      ? extra.state
      : undefined;
...
export async function openCommand(
  ...
  return receipt(
    await call(description, route, "command_open", {
      command: { operationId, commandId: item.id, revision: item.revision, context: item.context, selectionId: null },
    }),
    operationId,
  );
}
...
export async function launcherPreferences(
  description: Description,
  route: string,
  action: object = { kind: "read" },
): Promise<LauncherPreferences> {
  ...
  if (
    ...
    !("favorites" in value) ||
    !("recents" in value) ||
    ![value.favorites, value.recents].every(
      (ids) =>
        Array.isArray(ids) &&
        ids.length <= 64 &&
        ids.every((id) => typeof id === "string" && /^[A-Za-z0-9_./:-]{1,256}$/.test(id)),
    )
  )
    throw new Error("Launcher 설정을 확인하지 못했습니다.");
  return value as LauncherPreferences;
}
export async function launcherShortcut(
  description: Description,
  route: string,
  config: object | null = null,
): Promise<import("./launcher/types").ShortcutStatus> {
  if (!nativeMode)
    return {
  ...
  if (
    ...
    !["Ctrl+Alt+Space", "Ctrl+Alt+L", "Ctrl+Alt+J"].includes(String(value.accelerator)) ||
    !("enabled" in value) ||
    typeof value.enabled !== "boolean" ||
    !("registration" in value) ||
    !["registered", "unavailable", "unsupported", "disabled", "pending"].includes(String(value.registration)) ||
    !("alternatives" in value) ||
    !Array.isArray(value.alternatives) ||
    value.alternatives.some((key) => !["Ctrl+Alt+Space", "Ctrl+Alt+L", "Ctrl+Alt+J"].includes(key))
  )
    throw new Error("단축키 상태를 확인하지 못했습니다.");
  return value as import("./launcher/types").ShortcutStatus;
}

export async function onShortcut(
  callback: (event: { command: string; wasFocused: boolean }) => void,
): Promise<() => void> {
  ...
  return listen<unknown>("suite-shortcut", (event) => {
    ...
    if (
      ...
      "wasFocused" in value &&
      typeof value.wasFocused === "boolean" &&
      [
      ...
      callback({ command: value.command, wasFocused: value.wasFocused });
  });
}
...
export async function onConnectionChanged(callback: () => void): Promise<() => void> {
  ...
  const results = await Promise.allSettled([
    listen("suite-connected", callback),
    listen("suite-disconnected", callback),
  ]);
...
export interface LauncherImport {
  ...
  plan: {
    ...
    unresolvedRecents: string[];
    capacityFavorites: string[];
    capacityRecents: string[];
    legacyShortcut: { accelerator: string; enabled: boolean } | null;
    proposedShortcut: import("./launcher/types").ShortcutConfig | null;
    launcherTerminalConflict: boolean;
  };
}
export async function importLauncher(
  description: Description,
  route: string,
  method: "status" | "preview" | "apply" | "resume",
  id: string | null = null,
): Promise<LauncherImport | null> {
  ...
  if (
    ...
    throw new Error("가져오기 계획을 확인하지 못했습니다.");
  const plan = value.plan as Record<string, unknown>;
  if (
    !["unresolvedFavorites", "unresolvedRecents", "capacityFavorites", "capacityRecents"].every(
      (key) =>
        Array.isArray(plan[key]) &&
        (plan[key] as unknown[]).length <= 64 &&
        (plan[key] as unknown[]).every((id) => typeof id === "string" && /^[A-Za-z0-9_./:-]{1,256}$/.test(id)),
    )
  )
    throw new Error("가져오기 항목을 확인하지 못했습니다.");
...
export async function sendSessionSummary(
  ...
  if (!nativeMode || description.product.id !== "workspace")
    throw new Error("Workspace에서 요약을 전달할 수 있습니다.");
  const header = makeRequest(description.handshake, route, Date.now(), description.context);
  header.deadlineMs = Date.now() + 29000;
  const provenance = {
    product: description.product.id,
  ...
  const response = await invoke<{ operation: unknown; value: unknown }>("plugin:suite|connection", {
    request: { header, method: { kind: "sendSessionSummary", sourceId, operationId } },
  });
  if (!isOperation(response.operation, provenance) || response.operation.outcome.state !== "succeeded")
    throw new Error("요약 전달 결과를 확인하지 못했습니다.");
...
export async function openReceivedFile(
  ...
  route: string,
  reference: string,
): Promise<{ path: string; context: ProjectContext | null }> {
  if (!nativeMode || description.product.id !== "workspace")
    throw new Error("Workspace에서 파일을 확인할 수 있습니다.");
  const header = makeRequest(description.handshake, route, Date.now(), description.context);
  header.deadlineMs = Date.now() + 29000;
  const provenance = {
    product: "workspace",
  ...
  const response = await invoke<{ operation: unknown; value: unknown }>("plugin:suite|connection", {
    request: { header, method: { kind: "openReceivedFile", reference } },
  });
  if (!isOperation(response.operation, provenance) || response.operation.outcome.state !== "succeeded")
    throw new Error("파일 열기 요청을 확인하지 못했습니다.");
  ...
  if (
    ...
    !("context" in value) ||
    (value.context !== null && !isProjectContext(value.context))
  )
    throw new Error("파일 응답을 확인하지 못했습니다.");
