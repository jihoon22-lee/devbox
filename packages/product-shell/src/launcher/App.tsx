...
const SHORTCUT_ISSUES:Record<string,string>={shortcut_other_installation:"다른 Devbox 설치가 전역 단축키를 사용하고 있습니다. 그 설치에서 단축키를 끈 뒤 다시 설정해 주세요.",shortcut_registration_failed_previous_restored:"새 단축키를 등록하지 못해 이전 조합을 복원했습니다.",shortcut_previous_restore_failed:"이전 단축키도 복원하지 못했습니다. 다른 조합을 선택해 주세요.",shortcut_registration_failed:"단축키를 등록하지 못했습니다. 다른 조합을 선택해 주세요.",shortcut_save_failed:"단축키 설정을 저장하지 못했습니다."};

const SOURCE_NAMES: Record<string, string> = {
  "workspace":"Workspace", "api-studio":"API Studio", "knowledge":"Knowledge", "control-center":"Control Center",
  "workbench": "Workbench",
...
export default function App({adapter,embedded=false,description="앱과 검증된 snapshot을 빠르게 엽니다. 클립보드는 명시적 미리보기에서만 읽습니다."}:{adapter:LauncherAdapter;description?:string;embedded?:boolean}) {
  const Root=embedded?"section":"main";
  const {clearRecents,getShortcut,launchResult,performTextAction,previewTextAction,readCurrentText,search,setFavorite,setShortcut,hide:hideWindow}=adapter;
  ...
  useEffect(() => adapter.subscribe?.((next) => {
    if (mounted.current) { setResponse(next); setSelected(0); }
  }), [adapter]);
  ...
  const executeResult = async (result: SearchResult, allowStale = false) => {
    ...
    try {
      ...
      if (outcome.status === "reviewPending") { if(mounted.current) setError("요청한 제품에서 열기를 확인해 주세요."); }
      else if (outcome.status === "installRequired" && mounted.current) setError("대상 앱이 없어 Devbox Manager 설치 화면을 열었습니다.");
      else {
        void refresh(query);
        await hideWindow();
      }
        ...
        <ul id="launcher-results" className="result-list" role="listbox" aria-label="Launcher 결과" aria-busy={busy}>
          {response.results.map((result, index) => (
            <li key={result.id} role="presentation">
              <button id={`result-${index}`} type="button" role="option" aria-selected={selected === index} className={`result ${selected === index ? "selected" : ""}`} disabled={Boolean(result.disabledReason)} onMouseEnter={() => setSelected(index)} onClick={() => { setSelected(index); void runSelected(index); }}>
                ...
                <span className="result-detail">{result.detail ?? result.source}{result.disabledReason ? " · " + result.disabledReason : ""}{result.stale ? " · 오래됨" : ""}{result.explicitPreview ? " · 미리보기 필요" : ""}</span>
              </button>
            </li>
          ))}
        ...
        <footer className="launcher-footer">
          ...
          <div className="footer-actions">
            <button
              ...
              disabled={busy || favoriteBusyId !== null || !response.results[selected] || Boolean(response.results[selected]?.disabledReason && !response.results[selected]?.favorite)}
