...
const CLIPBOARD_PREVIEW_ID = "builtin/clipboard-preview";
...
const SHORTCUT_ISSUES: Record<string, string> = {
  shortcut_other_installation:
    "다른 Devbox 설치가 전역 단축키를 사용하고 있습니다. 그 설치에서 단축키를 끈 뒤 다시 설정해 주세요.",
  shortcut_registration_failed_previous_restored: "새 단축키를 등록하지 못해 이전 조합을 복원했습니다.",
  shortcut_previous_restore_failed: "이전 단축키도 복원하지 못했습니다. 다른 조합을 선택해 주세요.",
  shortcut_registration_failed: "단축키를 등록하지 못했습니다. 다른 조합을 선택해 주세요.",
  shortcut_save_failed: "단축키 설정을 저장하지 못했습니다.",
};
...
export default function App({
  adapter,
  embedded = false,
  description = "앱과 검증된 snapshot을 빠르게 엽니다. 클립보드는 명시적 미리보기에서만 읽습니다.",
}: {
  ...
  useEffect(() => {
    ...
    void getShortcut()
      .then((config) => {
        if (mounted.current) setShortcutState(config);
      })
      .catch(() => undefined);
  ...
  }, [preview, staleResult, hideWindow]);
  ...
  const executeResult = async (result: SearchResult, allowStale = false) => {
    ...
    try {
      ...
      if (outcome.status === "reviewPending") {
        if (mounted.current) setError("요청한 제품에서 열기를 확인해 주세요.");
      } else if (outcome.status === "installRequired" && mounted.current)
        setError("대상 앱이 없어 Devbox Manager 설치 화면을 열었습니다.");
      else {
        void refresh(query);
        await hideWindow();
      }
  ...
  const runSelected = async (index = selected) => {
    ...
    if (!result || busy || result.disabledReason) return;
    if (result.stale) {
  ...
  const saveShortcutConfig = async (next: ShortcutConfig) => {
    ...
    try {
      setShortcutState(await setShortcut(next));
    } catch {
      setError(safeError());
    } finally {
      if (mounted.current) setShortcutBusy(false);
    }
  };
  const saveShortcut = (value: ShortcutConfig["accelerator"]) =>
    saveShortcutConfig({
      accelerator: value,
      enabled: true,
      ...(shortcut?.terminal === undefined
        ? {}
        : { terminal: shortcut.terminal, capture: shortcut.capture, project: shortcut.project }),
    });
    ...
    <Root className="launcher-shell">
      <section className="launcher-panel" aria-labelledby="launcher-title">
        <header className="launcher-header">
          ...
          <button type="button" className="icon-button" aria-label="Launcher 닫기" onClick={() => void hideWindow()}>
            ×
          </button>
        </header>
        ...
        <input
          ...
          onChange={(event) => {
            ...
            if (!composition.current) void refresh(event.target.value);
          }}
          onKeyDown={(event) => {
            ...
            if (event.key === "ArrowDown") {
              event.preventDefault();
              setSelected((value) =>
                response.results.length === 0 ? 0 : Math.min(value + 1, response.results.length - 1),
              );
            }
        ...
        <div className="status" aria-live="polite" aria-atomic="true">
          {busy ? "확인 중…" : (error ?? `${response.results.length}개 결과`)}
        </div>
            ...
            <li key={result.id} role="presentation">
              ...
              >
                ...
                <span className="result-detail">
                  {result.detail ?? result.source}
                  {result.disabledReason ? " · " + result.disabledReason : ""}
                  {result.stale ? " · 오래됨" : ""}
                  {result.explicitPreview ? " · 미리보기 필요" : ""}
                </span>
              </button>
            </li>
          ))}
            ...
            <button
              ...
              aria-label={
                response.results[selected]
                  ? response.results[selected].favorite
                    ? `${response.results[selected].label} 즐겨찾기 해제`
                    : `${response.results[selected].label} 즐겨찾기 추가`
                  : "선택 결과 즐겨찾기"
              }
              aria-pressed={response.results[selected]?.favorite ?? false}
              onClick={() => {
                const result = response.results[selected];
              ...
              disabled={
                ...
                favoriteBusyId !== null ||
                !response.results[selected] ||
                Boolean(response.results[selected]?.disabledReason && !response.results[selected]?.favorite)
              }
            >
              {response.results[selected]?.favorite ? "★ 즐겨찾기 해제" : "☆ 즐겨찾기"}
            </button>
            <button
              type="button"
              className="clear-recents"
              onClick={() => void clearRecentHistory()}
              disabled={busy || recentsBusy}
            >
              최근 기록 초기화
            </button>
            <label>
              단축키{" "}
              <select
                aria-label="Launcher 단축키"
                value={shortcut?.accelerator ?? "Ctrl+Alt+Space"}
                disabled={shortcutBusy}
                onChange={(event) => void saveShortcut(event.target.value as ShortcutConfig["accelerator"])}
              >
                <option>Ctrl+Alt+Space</option>
                <option>Ctrl+Alt+L</option>
                <option>Ctrl+Alt+J</option>
              </select>
              <small>즉시 적용</small>
            </label>
          </div>
        ...
        {shortcut && (
          <ShortcutControls
            status={shortcut}
            busy={shortcutBusy}
            onChange={(config) => void saveShortcutConfig(config)}
            hideAccelerator
          />
        )}
        {shortcut?.issue && (
          <p className="shortcut-status" role="status">
            {SHORTCUT_ISSUES[shortcut.issue] ?? "단축키 상태를 확인해 주세요."}
          </p>
        )}
        {shortcut && !shortcut.issue && shortcut.registration !== "registered" && (
          <p className="shortcut-status" role="status">
            {shortcut.registration === "unavailable"
              ? `전역 단축키를 등록하지 못했습니다. ${shortcut.alternatives.join(" 또는 ")} 중 하나를 선택해 다시 시도하세요.`
              : shortcut.registration === "unsupported"
                ? "이 환경에서는 전역 단축키를 사용할 수 없습니다. 앱 메뉴나 다시 실행으로 Launcher를 여세요."
                : shortcut.registration === "pending"
                  ? "전역 단축키를 확인하는 중입니다…"
                  : "전역 단축키가 꺼져 있습니다. 위 선택에서 다시 켤 수 있습니다."}
          </p>
        )}
        <details className="sources">
          <summary>snapshot source 상태</summary>
          <ul className="source-list">
            {response.sources.map((source) => {
              const status = SOURCE_STATUS_COPY[source.status];
              return (
                <li key={`${source.producer}:${source.view}`}>
                  <span className="source-name">
                    {SOURCE_NAMES[source.producer] ?? source.producer} · {SOURCE_VIEW_NAMES[source.view] ?? source.view}
                  </span>
                  <span className={`source-status source-${source.status}`}>{status.label}</span>
      ...
      {staleResult && (
        <div className="modal-backdrop" role="presentation">
          <section
            className="preview-modal confirmation-modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="stale-title"
            aria-describedby="stale-description"
            tabIndex={-1}
            onKeyDown={handleDialogKeyDown}
          >
            <h2 id="stale-title">오래된 snapshot입니다</h2>
            <p id="stale-description">
              {staleResult.label}의 저장된 상태가 오래되었습니다. 계속하면 현재 catalog와 snapshot을 다시 확인한 뒤 대상
              앱에 전달합니다.
            </p>
            <div className="modal-actions">
              <button ref={staleCancelRef} type="button" onClick={() => setStaleResult(null)} disabled={busy}>
                취소
              </button>
              <button type="button" onClick={() => void confirmStale()} disabled={busy}>
                계속 열기
              </button>
            </div>
          </section>
      ...
      {preview && (
        <div className="modal-backdrop" role="presentation">
          ...
          >
            <h2 id="preview-title">
              {preview.kind === "clipboard-preview/v1" ? "클립보드 미리보기" : "텍스트 handoff 미리보기"}
            </h2>
            <p id="preview-description">
              {preview.kind === "clipboard-preview/v1"
                ? "명시적으로 요청한 현재 텍스트만 표시하며 전달하거나 저장하지 않습니다."
                : "이 내용은 확인 후 한 번만 전달되며 Launcher에 저장하지 않습니다."}
            </p>
            <pre>{preview.text}</pre>
            <div className="modal-actions">
              <button ref={previewCancelRef} type="button" onClick={() => setPreview(null)} disabled={busy}>
                {preview.kind === "clipboard-preview/v1" ? "닫기" : "취소"}
              </button>
              {preview.kind !== "clipboard-preview/v1" && (
                <button type="button" onClick={() => void confirmPreview()} disabled={busy}>
                  전달
                </button>
              )}
            </div>
