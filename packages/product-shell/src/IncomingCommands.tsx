...
export default function IncomingCommands({
  description,
  route,
  navigate,
  onReview,
      ...
      void call<Review[]>({ kind: "pending" })
        .then((value) => {
          if (
            ...
            value.some(
              (row) =>
                typeof row.commandRevision !== "string" ||
                !/^[a-f0-9]{64}$/.test(row.commandRevision) ||
                !description.features.some((feature) => feature.route === row.route) ||
                !row.target ||
                !["route", "entity"].includes(row.target.kind) ||
  ...
  const decide = async (review: Review, accept: boolean) => {
    ...
    setIssue("");
    try {
      const result = await call<{ operationId: string; route: string | null }>({
        kind: "decide",
        id: review.operationId,
        revision: review.revision,
        accept,
      });
      if (result.operationId !== review.operationId || result.route !== (accept ? review.route : null))
        throw new Error("invalid navigation");
      setReviews((values) => values.filter((value) => value.operationId !== review.operationId));
      if (result.route) {
        onReview(review);
        setOpening({ id: review.operationId, route: result.route });
        navigate(result.route);
      }
    } catch {
      setIssue("요청이 변경되었거나 만료되었습니다. 요청 제품에서 다시 확인해 주세요.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <section aria-label="다른 제품의 열기 요청">
      {reviews.map((review) => (
        <div key={review.operationId}>
          <strong>{review.label}</strong>
          <dl>
            <dt>담당 제품</dt>
            <dd>{description.product.label}</dd>
            <dt>대상</dt>
            <dd>
              {review.target.kind === "route" ? review.target.route : `${review.target.entity} · ${review.target.id}`}
            </dd>
            <dt>변경 내용</dt>
            <dd>담당 화면을 열고 현재 대상을 확인합니다. 실행·저장·삭제는 해당 화면에서 별도로 검토합니다.</dd>
            <dt>확인한 버전</dt>
            <dd>{review.commandRevision.slice(0, 12)}</dd>
          </dl>
          <button disabled={busy} onClick={() => void decide(review, true)}>
            화면 열기
          </button>
          <button disabled={busy} onClick={() => void decide(review, false)}>
            거절
          </button>
        </div>
      ))}
