...
describe("note editor ordering and native revisions", () => {
  it("keeps edits made during save dirty and serializes duplicate saves", async () => {
    ...
    const pending = deferred<NoteSnapshot>();
    write.mockReturnValueOnce(pending.promise);
    note.edit("A");
    const first = note.save();
    const duplicate = note.save();
    expect(duplicate).toBe(first);
    note.edit("B");
    pending.resolve(disk("A", "saved-A"));
  ...
  it("does not clear another document when an earlier save finishes", async () => {
    ...
    note.edit("B unsaved");
    pending.resolve(disk("A saved", "saved-A"));
    await save;
  ...
  it("applies only the latest open even when responses arrive in reverse", async () => {
    ...
    const first = deferred<NoteSnapshot>(),
      second = deferred<NoteSnapshot>();
    read.mockReturnValueOnce(first.promise).mockReturnValueOnce(second.promise);
    const a = note.openPath("first.md", () => true),
      b = note.openPath("latest.md", () => true);
  ...
  it("protects edits made while an inbound link is read, even if they were saved", async () => {
    ...
    const pending = deferred<{ path: string; content: string; revision: string }>();
    const opening = note.open(
      () => pending.promise,
      () => true,
    );
    note.edit("typed while waiting");
    await note.save();
  ...
  it("protects a draft without a watcher event and requires the reviewed disk revision to overwrite", async () => {
    ...
    note.edit("my draft");
    read.mockResolvedValue(disk("external", "external-1"));
    ...
    expect(note.snapshot()).toMatchObject({
      content: "my draft",
      dirty: true,
      conflict: disk("external", "external-1"),
    });
  ...
  it("shows external deletion and never automatically reloads a dirty buffer", async () => {
    const { note, read } = await fixture();
    note.edit("keep");
    read.mockResolvedValue(disk(null, "missing"));
    await note.inspect();
...
describe("approved deletion ownership", () => {
  ...
  it.each([false, true])("preserves a save overlapping deletion (%s)", async (finishSaveFirst) => {
    ...
    const complete = note.approveRemoval("A.md");
    if (finishSaveFirst) {
      pending.resolve(disk("submitted", "saved"));
      await saving;
    }
    expect(complete()).toBe(false);
...
it.each(["appliedWithConflict", "unknown"] as const)(
  "retains the draft and blocks successful quit for a %s write",
  async (state) => {
    const { note, write } = await fixture();
    note.edit("my unsaved draft");
    write.mockResolvedValueOnce({
      content: null,
      revision: "",
      saveOutcome: { state, recoveryDirectory: ".devbox-save-fixture", warning: "note_commit_unknown" },
    });
    expect(await note.saveBeforeQuit()).toBe(false);
...
it("records a committed write with a durability warning as saved", async () => {
  ...
  write.mockResolvedValueOnce({
    content: "committed",
    revision: "saved",
    saveOutcome: { state: "applied", recoveryDirectory: null, warning: "note_durability_warning" },
  });
...
it("preserves the newest open intent while pre-switch saves are waiting", async () => {
  const { note } = await fixture(),
    ...
    newer = deferred<void>();
  note.edit("dirty");
  note.setBeforeSwitch(vi.fn().mockReturnValueOnce(older.promise).mockReturnValueOnce(newer.promise));
  const first = note.open(
    async () => ({ path: "first.md", content: "first", revision: "r" }),
    () => true,
  );
  const second = note.open(
    async () => ({ path: "second.md", content: "second", revision: "r" }),
    () => true,
  );
  newer.resolve();
