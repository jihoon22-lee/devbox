...
it("schedules another save for edits made during an in-flight save", async () => {
  vi.useFakeTimers();
  try {
    const doc=new FakeDocument(); let finish!:()=>void;
    doc.save.mockImplementationOnce(async()=> { doc.set({saving:true}); await new Promise<void>(resolve=>{finish=resolve;}); doc.set({saving:false}); return false; });
    const autosave=new NoteAutosave(doc,true,undefined,timers,1500);
    doc.set({content:"first",dirty:true}); await vi.advanceTimersByTimeAsync(1500);
    doc.set({content:"second",dirty:true}); await vi.advanceTimersByTimeAsync(2000);
    expect(doc.save).toHaveBeenCalledTimes(1); finish(); await vi.advanceTimersByTimeAsync(0);
    await vi.advanceTimersByTimeAsync(1500); expect(doc.save).toHaveBeenCalledTimes(2); autosave.dispose();
  } finally { vi.useRealTimers(); }
});
it("does not retry a failed save forever without a new edit", async()=> {
  vi.useFakeTimers();
  try {
    const doc=new FakeDocument(); doc.save.mockImplementation(async()=> {doc.set({saving:true}); doc.set({saving:false,error:"failed"}); return false;});
    const autosave=new NoteAutosave(doc,true,undefined,timers,1500);
    doc.set({content:"first",dirty:true}); await vi.advanceTimersByTimeAsync(10000);
    expect(doc.save).toHaveBeenCalledTimes(1); autosave.dispose();
  } finally {vi.useRealTimers();}
});
it("keeps restored content paused across a view remount", async()=> {
  vi.useFakeTimers();
  try {
    const doc=new FakeDocument(); const first=new NoteAutosave(doc,true,undefined,timers,1500);
    first.pause(); doc.set({content:"restored",dirty:true}); first.dispose();
    const second=new NoteAutosave(doc,true,undefined,timers,1500); doc.set({content:"restored edit",dirty:true});
    await vi.advanceTimersByTimeAsync(5000); expect(doc.save).not.toHaveBeenCalled(); second.dispose();
  } finally {vi.useRealTimers();}
});
