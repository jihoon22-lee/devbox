...
const host = vi.hoisted(() => ({ enabled: false }));
vi.mock("../transport", () => ({ componentInvoke: () => invokeMock, isProductHosted: () => host.enabled }));

vi.mock("@tauri-apps/api/core", () => ({ invoke: invokeMock }));
vi.mock("@tauri-apps/plugin-clipboard-manager", () => ({ readText: vi.fn() }));
vi.mock("./lib/isTauri", () => ({ isTauri: () => true }));

import { captureNote, quickCaptureShortcutStatus } from "./api";
...
describe("quick capture IPC error boundary", () => {
  ...
  it("defers product shortcuts to the shell without claiming OS registration", async () => {
    host.enabled = true;
    await expect(quickCaptureShortcutStatus()).resolves.toEqual({ shortcut: "Ctrl+Alt+N", state: "managed" });
  ...
  it("preserves an allowlisted string rejection from the Tauri runtime", async () => {
    ...
    await expect(captureNote({ title: "", body: "body", tags: [] })).rejects.toThrow(
  ...
  it("redacts an unexpected native string instead of reflecting it", async () => {
    ...
    await expect(captureNote({ title: "", body: "body", tags: [] })).rejects.toThrow("빠른 캡처를 저장하지 못했습니다");
  });
