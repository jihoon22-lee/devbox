...
export class NoteDocument {
  ...
  clear() {
    ...
    this.document++;
    this.edits++;
    this.publish({ ...empty, saving: !!this.writing });
  }
  ...
  approveRemoval(target: string): () => boolean {
    ...
    return () => {
      ...
      if (edits !== this.edits || opening !== this.opening || saves !== this.saves || saving || this.writing) {
        ...
        this.publish({
          dirty: true,
          error: "파일은 삭제했지만 삭제 승인 이후의 노트 내용은 유지했습니다. 디스크 상태를 확인해 주세요.",
        });
        return false;
      }
      this.clear();
  ...
  async open(load: () => Promise<InboundNote>, discard: () => boolean): Promise<boolean> {
    ...
    if ((this.view.dirty || this.view.saving) && this.beforeSwitch) {
      try {
        ...
        /* The remaining dirty check preserves user choice. */
      }
    ...
    try {
      ...
      if (request === this.opening)
        this.publish({ error: error instanceof Error ? error.message : "노트를 열지 못했습니다." });
      return false;
    }
  }
  ...
  async inspect(): Promise<void> {
    const { path, revision } = this.view,
      document = this.document;
    const inspection = ++this.inspecting;
    const current = () =>
      inspection === this.inspecting &&
      document === this.document &&
      path === this.view.path &&
    ...
    try {
      ...
      if (current()) this.publish({ error: "파일의 현재 상태를 확인하지 못했습니다. 편집 내용은 유지됩니다." });
    }
  }
  save(overwriteRevision?: string): Promise<boolean> {
    ...
    const task = (async () => {
      try {
        ...
        const recovery = outcome?.recoveryDirectory
          ? ` 같은 폴더의 ${outcome.recoveryDirectory}에 보존된 파일을 확인해 주세요.`
          : "";
        if (outcome && outcome.state !== "applied") {
          const message =
            (outcome.state === "appliedWithConflict"
        ...
        if (document === this.document) {
          this.publish({
            dirty: true,
            error: error instanceof Error ? error.message : "저장하지 못했습니다. 편집 내용은 유지됩니다.",
          });
          await this.inspect();
        }
        return false;
