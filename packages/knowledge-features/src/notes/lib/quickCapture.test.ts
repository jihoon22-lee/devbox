...
describe("quick capture input policy", () => {
  ...
  it("uses the fixed title only when the title is blank", () => {
    expect(normalizeQuickCapture({ title: " ", body: "note", tags: [] }).title).toBe(QUICK_CAPTURE_DEFAULT_TITLE);
  ...
  it("rejects line breaks in title and tags before the native call", () => {
    expect(() => normalizeQuickCapture({ title: "bad\nname", body: "note", tags: [] })).toThrow(
      "빠른 캡처 입력이 올바르지 않습니다",
    );
    expect(() => normalizeQuickCapture({ title: "title\n", body: "note", tags: [] })).toThrow(
      "빠른 캡처 입력이 올바르지 않습니다",
    );
    expect(() => normalizeQuickCapture({ title: "title", body: "note", tags: ["bad\n tag"] })).toThrow(
      "태그에 줄바꿈·쉼표·대괄호·따옴표를 사용할 수 없습니다",
    );
    expect(() => normalizeQuickCapture({ title: "title", body: "bad\u0085text", tags: [] })).toThrow(
      "빠른 캡처 입력이 올바르지 않습니다",
    );
    expect(() => normalizeQuickCapture({ title: "title\u2028name", body: "note", tags: [] })).toThrow(
      "빠른 캡처 입력이 올바르지 않습니다",
    );
    expect(() => normalizeQuickCapture({ title: "title", body: "bad\ud800text", tags: [] })).toThrow(
      "빠른 캡처 입력이 올바르지 않습니다",
  ...
  it("keeps Rust and browser byte/scalar bounds aligned", () => {
    ...
    const rawAtLimit = `${"\r\n".repeat(65_535)}x`;
    expect(normalizeQuickCapture({ title: "title", body: rawAtLimit, tags: [] }).body).toHaveLength(65_536);
    const rawOverLimit = `${"\r\n".repeat(65_536)}x`;
    expect(() => normalizeQuickCapture({ title: "title", body: rawOverLimit, tags: [] })).toThrow(
      "본문은 LF 기준 64 KiB(원문 128 KiB) 이내로 입력하세요",
    );
    expect(normalizeQuickCapture({ title: "title", body: "note", tags: ["😀".repeat(48)] }).tags).toHaveLength(1);
    expect(() =>
      normalizeQuickCapture({
        title: "title",
        body: "note",
  ...
  it("scans all credential prefix occurrences and blocks header-shaped secrets", () => {
    for (const body of [
      ...
      expect(() => normalizeQuickCapture({ title: "title", body, tags: [] })).toThrow(
