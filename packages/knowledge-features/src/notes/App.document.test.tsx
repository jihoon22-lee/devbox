...
vi.mock("./components/MarkdownEditor", () => ({ default: ({value, onChange}: {value: string; onChange: (text: string) => void}) => <textarea aria-label="note editor" value={value} onChange={event => onChange(event.target.value)}/> }));
...
it("preserves edits after the save request and does not submit duplicate writes", async () => {
  const saving = pending<NoteSnapshot>(); vi.mocked(writeFile).mockReturnValueOnce(saving.promise);
  render(<App/>); fireEvent.click(await screen.findByText("note.md"));
  ...
  await act(async () => { saving.resolve({content: "submitted", revision: "saved-1"}); await saving.promise; });
...
it("keeps text typed while another file is loading", async () => {
  ...
  await act(async () => { reading.resolve({content: "other", revision: "other"}); await reading.promise; });
...
it("ignores an older file response after an external openRequest selected a newer note", async () => {
  const first = pending<NoteSnapshot>(), second = pending<NoteSnapshot>();
  vi.mocked(readFile).mockReturnValueOnce(first.promise).mockReturnValueOnce(second.promise);
  const app = render(<App/>); fireEvent.click(await screen.findByText("note.md"));
  app.rerender(<App openRequest={{id:1,path:"Notes/nested.md"}}/>);
  await act(async () => { second.resolve({content:"latest",revision:"latest"}); await second.promise; });
  await act(async () => { first.resolve({content:"older",revision:"older"}); await first.promise; });
