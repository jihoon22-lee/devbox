...
import type { QuickCaptureInput, QuickCaptureSaved } from "../types";
import {
...
type BusyOperation = "clipboard" | "save" | null;
...
export default function QuickCaptureDialog({ open, active = true, onClose, onSaved, restoreFocusRef }: Props) {
  ...
  const close = () => {
    // Keep the dialog open while native creation settles. Clipboard reads may be cancelled.
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      const saved = await captureNote(input());
