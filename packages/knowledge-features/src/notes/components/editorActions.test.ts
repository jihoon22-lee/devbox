...
describe("Markdown editor context actions", () => {
  ...
  it("wraps selected text and inserts a visible label for an empty selection", () => {
    ...
    expect(selected.update(insertMarkdownLink(selected, "https://example.com")).state.doc.toString()).toBe(
      "[docs](https://example.com)",
    );

    const empty = EditorState.create({ doc: "start ", selection: { anchor: 6 } });
    expect(empty.update(insertMarkdownLink(empty, "note.md")).state.doc.toString()).toBe("start [링크](note.md)");
  });
});
