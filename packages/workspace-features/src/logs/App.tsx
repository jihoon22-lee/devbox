...
function handoffRetryMessage(action: HandoffRecoveryAction, attempts: number): string {
  ...
  const operation =
    action === "discard" ? "취소" : action === "accept" ? "추가" : action === "renew" ? "lease 갱신" : "미리보기";
...
function labelForKind(kind: SourceKind): string {
  switch (kind) {
    ...
    case "wslJournal":
      return "WSL 저널";
    case "run":
      return "Run Manager 연결";
    case "runtimeRun":
      return "Workspace 실행 로그";
    case "webhookCapture":
      return "Webhook 캡처";
    case "container":
      return "컨테이너 로그";
...
function mergeClientRecords(
  ...
  return [...byKey.values()]
    .sort(
      (left, right) =>
        compareSafeNumbers(left.timestampMillis, right.timestampMillis) ||
...
function highlightMessage(message: string, filter: FilterSpec): ReactNode {
  ...
  const matcher = filter.regex ? createSafeRegex(filter.text, "g") : createLiteralRegex(filter.text, "g");
...
function App({
  ...
  active?: boolean;
  openRequest?: RuntimeLogOpenRequest | null;
  onOpenConsumed?: (id: string) => void;
  ...
  const [sources, setSources] = useState<SourceSpec[]>(() =>
    isTauri() ? [] : [{ kind: "localFile", path: "fixture.log" }],
  );
  ...
  const refreshRef = useRef<(nextSources?: SourceSpec[], nextCursors?: Array<FileCursor | null>) => Promise<void>>(
    async () => undefined,
  );
  ...
  const prepareLogContext = useCallback(
    (_reason: "pointer" | "keyboard", target: HTMLElement) => {
      const element = target as HTMLElement | null;
      const key = element?.dataset?.logKey ?? element?.closest?.<HTMLElement>("[data-log-key]")?.dataset.logKey;
      const record = key
  ...
  const refresh = useCallback(
    async (nextSources = sources, nextCursors = cursors) => {
      ...
      try {
        ...
        if (!mounted.current || !activeRef.current || generation.current !== currentGeneration || pausedRef.current)
          return;
        setSnapshot(next);
        setRecords(
          nextSources === sources
  ...
  const startLogSourcePreview = useCallback(
    async (handoffKind: HandoffOpenTarget["handoffKind"], id: string, attempts = 0) => {
      ...
      try {
        ...
        if (classifyHandoffError(error) === "retryable") {
          ...
          const action: HandoffRecoveryAction =
            code === "handoff-response-invalid" || code === "handoff-restore-failed" ? "discard" : "preview";
          ...
          clearHandoffState();
          setError(handoffFailureMessage(error, "Log Lens source handoff를 미리볼 수 없습니다. 다시 보내 주세요."));
        }
      } finally {
  ...
  const openLogSourcePreview = useCallback(
    (target: HandoffOpenTarget) => {
      if (!/^[0-9a-f]{32}$/.test(target.id) || !["log-source/v1", "webhook-log/v1"].includes(target.handoffKind))
        return;
  ...
  const restoreHandoffClaim = useCallback(
    async (kind: HandoffOpenTarget["handoffKind"], id: string, attempts = 0) => {
      const actionGeneration = ++handoffGeneration.current;
      handoffBusyRef.current = true;
      setHandoffBusy(true);
      updateHandoffRecovery(null);
  ...
  const applyAcceptedSource = useCallback(
    async (kind: HandoffOpenTarget["handoffKind"], id: string, attempts = 0) => {
      ...
      try {
        ...
        if (classifyHandoffError(error) === "retryable" && handoffErrorCode(error) !== "handoff-response-invalid") {
          const nextAttempts = Math.min(attempts + 1, MAX_HANDOFF_RECOVERY_ATTEMPTS);
          updateHandoffRecovery({ id, kind, action: "accept", attempts: nextAttempts });
          setError(null);
        } else {
          clearHandoffState();
          setError(handoffFailureMessage(error, "Log Lens source handoff를 적용하지 못했습니다. 다시 보내 주세요."));
        }
      } finally {
        if (handoffGeneration.current === actionGeneration) {
          handoffBusyRef.current = false;
  ...
  const acceptLogSourcePreview = useCallback(() => {
    const preview = handoffPreviewRef.current;
    if (!preview || handoffBusyRef.current) return;
    const attempts = handoffRecoveryRef.current?.action === "accept" ? handoffRecoveryRef.current.attempts : 0;
    void applyAcceptedSource(preview.kind, preview.id, attempts);
  }, [applyAcceptedSource]);

  const renewPreviewLease = useCallback(
    async (kind: HandoffOpenTarget["handoffKind"], id: string, attempts = 0) => {
      const actionGeneration = ++handoffGeneration.current;
      handoffBusyRef.current = true;
      setHandoffBusy(true);
      ...
      try {
        ...
        if (code === "handoff-response-invalid") {
          ...
          clearHandoffState();
          setError(handoffFailureMessage(error, "Log Lens source handoff lease를 갱신하지 못했습니다."));
        }
      } finally {
        if (handoffGeneration.current === actionGeneration) {
          handoffBusyRef.current = false;
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const focusable = [handoffCancelRef.current, handoffAcceptRef.current, handoffRetryRef.current].filter(
    ...
    return () => {
      ...
      if (handoffPreviewRef.current?.id !== id && handoffRecoveryRef.current?.id !== id && opener?.isConnected)
        opener.focus();
    };
  ...
  useEffect(() => {
    ...
    const retained = sources.filter((candidate) =>
      source.kind === "runtimeRun"
    ...
    if (openRequest.offset != null) {
      if (
        source.kind !== "runtimeRun" ||
        !/^(0|[1-9][0-9]{0,19})$/.test(openRequest.offset) ||
    ...
    setNotice(
      source.kind === "runtimeRun"
        ? "선택한 Workspace 실행의 로그를 불러옵니다."
        : source.kind === "webhookCapture"
          ? "검토한 Webhook 로그를 불러옵니다."
          : "선택한 WSL 로그를 불러옵니다.",
    );
    void refresh(nextSources, nextCursors);
  }, [active, openRequest, onOpenConsumed, sources, refresh]);

  const visibleRecords = useMemo(() => filterRecords(records, filter).slice(-MAX_RENDERED_ROWS), [filter, records]);
  ...
  const sendSelectedLogs = useCallback(async () => {
    ...
    try {
      ...
      const dispatch = isProductHosted()
        ? await sendNativeLogSelection(actionSnapshotGeneration, targets)
        : await sendSelectionToToolbox(exported.text);
      if (!mounted.current) return;
      if (!isCurrentSelection()) {
        setError(STALE_SELECTION_ERROR);
        setNotice(null);
      ...
      setNotice(
        isProductHosted()
          ? "API Studio에 검토를 요청했습니다." + (dispatch.redacted ? " 민감한 값은 마스킹되었습니다." : "")
          : dispatch.redacted
            ? TOOLBOX_SEND_REDACTED
            : TOOLBOX_SEND_SUCCESS,
      );
    } catch {
      if (!mounted.current) return;
  ...
  const buildSource = (): SourceSpec | null => {
    switch (kind) {
      ...
      case "directory":
        return path && pattern ? { kind, path, pattern } : null;
      case "wslFile":
        return distro && path ? { kind, distro, path } : null;
      case "wslJournal":
        return distro ? (unit ? { kind, distro, unit } : { kind, distro }) : null;
      case "run":
      ...
      case "webhookCapture":
        return null;
      case "container":
        return containerId ? { kind, engine, containerId } : null;
    }
  };
  ...
  const reconnectSources = async () => {
    ...
    try {
      ...
      setSnapshot(null);
      setNotice(
        resolved.unavailableSources
          ? `연결할 수 없는 Runtime source ${resolved.unavailableSources}개가 있습니다. 나머지 source를 연결합니다.`
          : "source에 다시 연결하는 중입니다.",
      );
      await refresh(resolved.sources, nextCursors);
    } catch {
      if (mounted.current && generation.current === requestedGeneration)
        setError("source 재연결을 완료하지 못했습니다.");
    } finally {
  ...
  const onLogContextSelect = useCallback(
    (id: string) => {
  ...
  return (
    <main className="app-shell">
      {(handoffPreview || handoffRecovery) && (
        <div className="handoff-backdrop" role="presentation">
          ...
          >
            <h2 id="log-source-handoff-title">
              {handoffPreview ? "Log Lens source 미리보기" : "Log Lens source handoff 복구"}
            </h2>
            ...
            {handoffPreview && (
              <dl className="handoff-details">
                <div>
                  <dt>보낸 앱</dt>
                  <dd>{handoffPreview.sourceApp}</dd>
                </div>
                <div>
                  <dt>어댑터</dt>
                  <dd>{labelForKind(handoffPreview.source.kind)}</dd>
                </div>
                <div>
                  <dt>비공개 source ID</dt>
                  <dd>
                    <code>{handoffPreview.source.sourceId}</code>
                  </dd>
                </div>
              </dl>
            )}
            ...
            <div className="handoff-actions">
              {handoffPreview && (
                <button
                  ref={handoffCancelRef}
                  type="button"
                  className="button"
                  onClick={() => cancelLogSourcePreview()}
                  disabled={handoffBusy || Boolean(handoffRecovery)}
                >
                  취소
                </button>
              )}
              {handoffPreview && (
                <button
                  ref={handoffAcceptRef}
                  type="button"
                  className="button primary"
                  onClick={() => acceptLogSourcePreview()}
                  disabled={handoffBusy || Boolean(handoffRecovery)}
                >
                  읽기 전용 source 추가
                </button>
      ...
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <label className="toggle">
            <input type="checkbox" checked={paused} onChange={(event) => setPaused(event.target.checked)} /> 화면 갱신
            일시정지
          </label>
          <label className="toggle">
            <input type="checkbox" checked={wrapLines} onChange={(event) => setWrapLines(event.target.checked)} /> 줄
            바꿈
          </label>
          <button
            className="button"
            ...
            disabled={busy || !connected || !sources.length}
          >
            새로고침
          </button>
          {!connected && (
      ...
      <section className="source-panel" aria-labelledby="sources-heading">
        <div className="section-heading">
          <h2 id="sources-heading">로그 source</h2>
          <span className="muted">
            {sources.length}/{MAX_SOURCES}개 선택 · 읽기 전용 · 네트워크 수집 없음 · 아카이브 없음
          </span>
        </div>
        <form
        ...
        >
          <label>
            ...
            <select value={kind} onChange={(event) => setKind(event.target.value as SourceKind)}>
              {(["localFile", "directory", "wslFile", "wslJournal", "run", "container"] as SourceKind[]).map(
                (value) => (
          ...
          {(kind === "localFile" || kind === "directory" || kind === "wslFile") && (
            <label>
              경로
              <input
                value={path}
                onChange={(event) => setPath(truncateUtf8(event.target.value, 4 * 1024))}
                placeholder={kind === "wslFile" ? "/var/log/app.log" : "C:\\logs\\app.log"}
              />
            </label>
          )}
          {kind === "directory" && (
            <label>
              ...
              <input
                value={pattern}
                onChange={(event) => setPattern(truncateUtf8(event.target.value, 128))}
                placeholder="*.log"
              />
            </label>
          )}
          {(kind === "wslFile" || kind === "wslJournal") && (
            <label>
              배포판
              <input
                value={distro}
                onChange={(event) => setDistro(truncateUtf8(event.target.value, 128))}
                placeholder="Ubuntu"
              />
            </label>
          )}
          {kind === "wslJournal" && (
            <label>
              ...
              <input
                value={unit}
                onChange={(event) => setUnit(truncateUtf8(event.target.value, 128))}
          ...
          {kind === "run" && (
            <label>
              ...
              <input
                value={sourceId}
                onChange={(event) => setSourceId(truncateUtf8(event.target.value, 192))}
                placeholder="run-manager:run-1:stdout"
              />
            </label>
          )}
          {kind === "container" && (
            <>
              <label>
                엔진
                <select value={engine} onChange={(event) => setEngine(event.target.value as ContainerEngine)}>
              ...
              <label>
                ...
                <input
                  value={containerId}
                  onChange={(event) => setContainerId(truncateUtf8(event.target.value, 128))}
                />
              </label>
            </>
          )}
        ...
        <div className="source-list" aria-live="polite">
          {sources.length ? (
            sources.map((source, index) => {
              ...
              return (
                <div className="source-chip" key={`${source.kind}-${index}`}>
                  <span>{labelForKind(source.kind)}</span>
                  <span
                    className={`source-status source-status-${status ?? "pending"}`}
                    aria-label={`상태: ${labelForStatus(status ?? "pending")}`}
                  >
                    {labelForStatus(status ?? "pending")}
                  </span>
                  <button
                    type="button"
                    disabled={busy}
                    aria-label={`${labelForKind(source.kind)} 제거`}
            ...
            <span className="muted">선택한 source가 없습니다. 브라우저 모드에서는 제한된 예시를 표시합니다.</span>
          )}
        </div>
      </section>
      ...
      <section className="filter-panel" aria-labelledby="filter-heading">
        <div className="section-heading">
          <h2 id="filter-heading">필터</h2>
          <span className="muted">
            {visibleRecords.length}개 표시 · {records.length}개 보관
          </span>
        </div>
        <div className="filter-row">
          <label className="filter-grow">
            ...
            <input
              value={filter.text}
              onChange={(event) =>
                setFilter((current) => ({ ...current, text: truncateUtf8(event.target.value, 512) }))
              }
              placeholder="메시지 또는 필드 값"
            />
          </label>
          <label className="toggle">
            <input
              ...
              onChange={(event) => setFilter((current) => ({ ...current, regex: event.target.checked }))}
            />{" "}
            정규식
          </label>
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setFilter((current) => ({
                  ...current,
                  level: (event.target.value || undefined) as LogLevel | undefined,
                }))
              }
            >
              <option value="">모든 레벨</option>
              {(["trace", "debug", "info", "warn", "error", "fatal"] as LogLevel[]).map((value) => (
                <option key={value} value={value}>
                  {value}
                </option>
              ))}
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) => setFilter((current) => ({ ...current, sourceId: event.target.value || undefined }))}
            >
          ...
          <button
            className="button"
            type="button"
            onClick={() => void exportVisible(false)}
          ...
          <button
            className="button"
            type="button"
            onClick={() => void exportVisible(true)}
            disabled={!visibleRecords.length}
          >
            복사
          </button>
          <button
            className="button primary"
            type="button"
            onClick={() => void sendSelectedLogs()}
            disabled={busy || toolboxBusy || !selectedRecords.length}
          >
            {isProductHosted() ? "선택 로그를 API Studio에서 변환" : "선택 로그를 Developer Toolbox로 보내기"}
          </button>
        </div>
        <div className="filter-row">
          <label>
            ...
            <input
              ...
              onChange={(event) =>
                setFilter((current) => ({
                  ...current,
                  field: event.target.value ? truncateUtf8(event.target.value, 4 * 1024) : undefined,
                }))
              }
              placeholder="필드 이름"
            />
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) =>
                setFilter((current) => ({
                  ...current,
                  fieldValue: event.target.value ? truncateUtf8(event.target.value, 4 * 1024) : undefined,
                }))
              }
              placeholder="값"
            />
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) =>
                setFilter((current) => ({
                  ...current,
                  endAt: event.target.value ? Number(event.target.value) : undefined,
                }))
              }
            />
          </label>
          <label>
            ...
            <input
              value={viewName}
              onChange={(event) => setViewName(truncateUtf8(event.target.value, 128))}
              placeholder="뷰 이름"
            />
          </label>
          <button
            ...
            disabled={
              savedViewsBusy ||
              !sources.length ||
              sources.some((source) => source.kind === "wslFile" || source.kind === "webhookCapture")
            }
          >
            저장
          </button>
          ...
          <button
            ...
            disabled={savedViewsBusy || !selectedViewName}
          >
            뷰 삭제
          </button>
        </div>
      ...
      <section className="log-panel" aria-labelledby="log-heading">
        <div className="section-heading">
          <h2 id="log-heading">병합 결과</h2>
          <span className="muted" aria-live="polite">
            {busy ? "읽는 중…" : snapshot?.truncated ? "안전 제한 도달" : "준비됨"}
          </span>
        </div>
        <div className="log-table" role="table" aria-label="병합된 로그 레코드" aria-busy={busy}>
          <div className="log-row log-head" role="row">
            ...
            <span role="columnheader">북마크</span>
          </div>
          {visibleRecords.map((record) => {
            ...
            return (
              <div
                ...
                key={key}
                data-log-key={key}
                aria-label={`로그 줄 ${record.sequence}: ${record.message}`}
                onContextMenu={(event) => {
                  contextRecordRef.current = record;
                  setContextRecord(record);
                  logContextTrigger.onContextMenu?.(event);
                }}
                onKeyDown={(event) => {
                  contextRecordRef.current = record;
                  setContextRecord(record);
                  logContextTrigger.onKeyDown?.(event);
                }}
              >
                <span role="cell" className="row-select">
                  <input
                    type="checkbox"
                    aria-label={`로그 줄 ${record.sequence} 선택`}
                    checked={selected.has(key)}
                    onChange={() => toggleSelection(record)}
                  />
                </span>
                <time role="cell" dateTime={toIsoTimestamp(record.timestampMillis)}>
                  {formatTimestamp(record.timestampMillis)}
                </time>
                <span role="cell" className={`level level-${record.level ?? "unknown"}`}>
                  {record.level ?? "—"}
                </span>
                <code role="cell" title={record.sourceId}>
                  {record.sourceId.slice(-12)}
                </code>
                <span role="cell" className={`message ${wrapLines ? "" : "nowrap"}`}>
                  {highlightMessage(record.message, filter)}
                </span>
                <span role="cell" className="bookmark-cell">
                  <button
                    className={`bookmark ${bookmarks.has(key) ? "active" : ""}`}
                    type="button"
                    aria-label={`${bookmarks.has(key) ? "북마크 제거" : "북마크 추가"}`}
      ...
      <footer className="statusbar" aria-live="polite">
        <span>{connected ? "연결됨" : "재연결 필요"}</span>
        <span>source {snapshot?.sources.length ?? 0}개</span>
        <span>링 버퍼 제외 {snapshot?.droppedRecords ?? 0}개</span>
        <span>북마크 {bookmarks.size}개</span>
        <span>
          저장된 뷰 {savedViews.length}/{MAX_SAVED_VIEWS}
        </span>
      </footer>
    </main>
  );
