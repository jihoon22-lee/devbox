...
import { createSafeRegex, filterRecords, recordKey, truncateUtf8, utf8ByteLength } from "./filter";
...
describe("Log Lens filter", () => {
  ...
  it("keeps browser fixture source IDs distinct for multi-source views", () => {
    ...
    expect(snapshot.sources.map((source) => source.sourceId)).toEqual(["log-source:fixture-0", "log-source:fixture-1"]);
    expect(snapshot.records).toHaveLength(4);
    expect(
      filterRecords(snapshot.records, {
        text: "",
        regex: false,
