...
export function configureTerminalStorage(installation: string, session: string): void {
  if (owner || ![installation, session].every((value) => /^[A-Za-z0-9_-]{1,128}$/.test(value)))
    throw new Error("터미널 저장 위치를 확인하지 못했습니다.");
...
export async function initializeTerminalPreferences(): Promise<void> {
  ...
  for (const key of [
    ...
    if (preferences[key] === undefined && previous !== null) {
      try {
        await invoke("set_terminal_preference", { key, expected: null, value: previous });
...
export function writeTerminalPreference(key: string, value: string): void {
  ...
  pending = pending.then(async () => {
    try {
      await invoke("set_terminal_preference", { key, expected: persisted[key] ?? null, value });
      persisted[key] = value;
    } catch {
      // A stale companion cannot overwrite a newer import or another window.
      try {
        persisted = await invoke<Record<string, string>>("terminal_preferences", {});
        preferences = { ...persisted };
      } catch {
        /* keep the last confirmed values */
      }
