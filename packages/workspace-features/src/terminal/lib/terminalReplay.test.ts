...
const batch = (sequence: number, data: string, closed = false): OutputBatch => ({
...
describe("owned terminal replay", () => {
  it("waits for xterm before reading more and reports closure only after final drain", async () => {
    ...
    const read = vi
      .fn()
      .mockResolvedValueOnce(batch(1, "first"))
      .mockResolvedValueOnce(batch(2, "last", true));
    const write = vi
      .fn()
    ...
    expect(write).toHaveBeenLastCalledWith("last", false);
  ...
  it("a detached renderer never consumes a late response or closes a PTY", async () => {
    ...
    const read = vi.fn(
      () =>
        new Promise<OutputBatch>((resolve) => {
          complete = resolve;
        }),
    );
    const write = vi.fn();
  ...
  it("requires an explicit loss marker for skipped output sequences", async () => {
    ...
    expect(write).not.toHaveBeenCalled();
    invalid();
    const valid = followTerminalOutput(
      async () => ({ ...batch(5, "retained", true), truncated: true }),
      write,
      closed,
      failed,
    );
    await settle();
    expect(write).toHaveBeenCalledWith("retained", true);
    expect(closed).toHaveBeenCalledTimes(1);
    valid();
  });
});
