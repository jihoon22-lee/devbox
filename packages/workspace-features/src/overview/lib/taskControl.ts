import type { WorkspaceTaskControl, WorkspaceTaskControlReceipt, WorkspaceTaskControlReceiptStatus } from "../api";
...
const FIXED_TASK_CONTROL_ERRORS: Record<string, string> = {
  ...
  "task-control-source-changed":
    "task snapshot 이후 원본 revision이 변경되었습니다. 목록을 새로고침하고 다시 확인하세요.",
  ...
  "workspace-task-source-untrusted":
    "이 workspace task의 소스가 아직 승인되지 않았습니다. Run Manager에서 현재 revision을 승인하세요.",
  "workspace-task-shell-untrusted":
    "이 셸 task의 실행이 아직 승인되지 않았습니다. Run Manager에서 셸 실행을 별도로 승인하세요.",
  "workspace-task-unavailable": "이 workspace task는 현재 사용할 수 없습니다. Run Manager에서 원본을 다시 확인하세요.",
  "workspace-task-source-changed":
    "원본 tasks.json이 변경되어 승인이 무효화되었습니다. Run Manager에서 다시 확인하세요.",
  "workspace-task-dependency-selection-incomplete":
    "선택한 task의 선행 dependency가 빠졌습니다. Run Manager에서 dependency를 함께 선택하세요.",
  ...
  "workspace-task-orchestration-required":
    "이 workspace task는 dependency가 있어 Run Manager orchestration이 필요합니다.",
...
export function taskControlErrorMessage(cause: unknown): string {
  ...
  return FIXED_TASK_CONTROL_ERRORS[code] ?? "Run Manager task 요청을 완료하지 못했습니다.";
}

export function canStartWorkspaceTask(task: WorkspaceTaskControl): boolean {
  return !task.operationActive && task.trusted && task.available && (task.taskKind === "process" || task.shellTrusted);
}
...
export function isTerminalTaskControlReceipt(status: WorkspaceTaskControlReceiptStatus): boolean {
  return status === "rejected" || status === "started" || status === "stopped" || status === "failed";
}
