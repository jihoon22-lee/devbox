        ...
        <div className="dependency-remote-value">
          <div>
            ...
            <span className={`dependency-remote-state ${remoteStateClass(entry.osv.state)}`}>
              {VALUE_STATE_LABEL[entry.osv.state]}
              {osvAge ? ` · ${osvAge}` : ""}
            </span>
          </div>
          {entry.osv.state !== "failed" &&
            (entry.osv.advisoryIds.length > 0 ? (
              <p>
                권고: <span className="mono">{entry.osv.advisoryIds.join(" · ")}</span>
              </p>
            ) : (
              <p className="dim">조회 시점에 반환된 advisory가 없습니다.</p>
            ))}
          {entry.osv.truncated && (
            <p className="dependency-lens-warning">OSV 결과에 다음 페이지가 있어 일부만 표시합니다.</p>
          )}
        </div>
      )}
        ...
        <div className="dependency-remote-value">
          <div>
            ...
            <span className={`dependency-remote-state ${remoteStateClass(entry.depsDev.state)}`}>
              {VALUE_STATE_LABEL[entry.depsDev.state]}
              {depsAge ? ` · ${depsAge}` : ""}
            </span>
          </div>
          {entry.depsDev.state !== "failed" && (
            <>
              <p>
                라이선스(참고용): {entry.depsDev.licenses.length ? entry.depsDev.licenses.join(" · ") : "확인되지 않음"}
              </p>
              <p>
                서비스 기본 버전: <span className="mono">{entry.depsDev.defaultVersion ?? "확인되지 않음"}</span>{" "}
                <span className="dim">(안전한 업데이트 보장 아님)</span>
              </p>
              {entry.depsDev.deprecated && (
                <p className="dependency-remote-deprecated">deps.dev에서 deprecated로 표시했습니다.</p>
              )}
              {entry.depsDev.advisoryIds.length > 0 && (
                <p>
                  권고: <span className="mono">{entry.depsDev.advisoryIds.join(" · ")}</span>
                </p>
              )}
              {(!entry.depsDev.versionFound || !entry.depsDev.packageFound) && (
                <p className="dim">
                  서비스에 {!entry.depsDev.versionFound ? "해당 버전" : "패키지 기본 버전 정보"}가 없습니다.
                </p>
              )}
            </>
          )}
...
export default function DependencyLensPanel({
  repo,
  onBusyChange,
}: {
  repo: RepoEntry | null;
  ...
  const serviceTransmissionCount =
    preview?.services.reduce((total, service) => total + service.transmitted.length, 0) ?? 0;
  ...
  return (
    <section className="dependency-lens-panel" aria-busy={busy}>
      ...
      {loading && (
        <div className="dependency-lens-status" role="status">
          제한된 lock graph를 분석하고 있습니다…
        </div>
      )}
      {error && (
      ...
      {!loading && !report && !error && (
        <div className="dependency-lens-empty dim">
          선택한 저장소의 Cargo, pnpm, npm, uv lockfile을 분석할 수 있습니다.
        </div>
      )}
      {report && (
        <>
          <dl className="dependency-lens-metrics">
            <div>
              <dt>전체</dt>
              <dd>{report.packageCount}</dd>
            </div>
            <div>
              <dt>직접</dt>
              <dd>{report.directCount}</dd>
            </div>
            <div>
              <dt>전이</dt>
              <dd>{report.transitiveCount}</dd>
            </div>
            <div>
              <dt>중복 이름</dt>
              <dd>{report.duplicates.length}</dd>
            </div>
            <div>
              <dt>미해결 edge</dt>
              <dd>{report.unresolvedDependencyCount}</dd>
            </div>
          </dl>
          ...
          <div className="dependency-lens-section dependency-enrichment-section">
            ...
            {remoteActivity === "execute" && (
              <div className="dependency-lens-status" role="status">
                검토한 좌표의 원격 정보를 불러오고 있습니다…
              </div>
            )}
            {remoteError && (
              <div className="error dependency-lens-error" role="alert">
            ...
            {preview && (
              <div className="dependency-enrichment-preview" aria-label="원격 전송 검토">
                <div className="dependency-enrichment-disclosure">
                  ...
                  <span>
                    서비스별 전송 좌표 합계 {serviceTransmissionCount}개 · 로컬 package {preview.localPackageCount}개
                    분석 기준
                  </span>
                  <span className="dim">
                    저장소 경로, lockfile 내용·경로, graph edge, registry·Git URL, checksum, credential, 환경·사용자
                    식별 정보는 보내지 않습니다.
                  </span>
                </div>
                {preview.services.map((service) => (
                  <div className="dependency-enrichment-service" key={service.service}>
                    <div className="dependency-enrichment-service-head">
                      ...
                      <span className="dim">
                        요청 {service.requestCount} · 캐시 {service.cachedCount} · 오래된 캐시 대체{" "}
                        {service.staleFallbackCount} · 상한 생략 {service.omittedCount}
                      </span>
                    </div>
                      ...
                      <ul className="dependency-enrichment-coordinates">
                        {service.transmitted.map((coordinate) => (
                          <li key={`${coordinate.ecosystem}:${coordinate.name}@${coordinate.version}`}>
                            ...
                            {coordinate.localPackageCount > 1 && (
                              <span className="dim">로컬 {coordinate.localPackageCount}개와 매핑</span>
                            )}
                          </li>
                        ))}
                      </ul>
                ...
                <div className="dependency-enrichment-confirm">
                  <button type="button" className="btn" disabled={busy} onClick={resetRemote}>
                    전송 검토 취소
                  </button>
                  ...
                  <button
                    ...
                    disabled={busy}
                    onClick={() => void executeEnrichment()}
                  >
                ...
                <div className="dependency-enrichment-summaries">
                  {enrichment.services.map((service) => (
                    <span key={service.service}>
                      <strong>{SERVICE_LABEL[service.service]}</strong> 대상 {service.targetCount} · 전송{" "}
                      {service.transmittedCount} · 캐시 {service.cachedCount} · stale {service.staleCount} · 실패{" "}
                      {service.failedCount} · 생략 {service.omittedCount}
                    </span>
                  ))}
                </div>
                {!enrichment.cachePersisted && (
                  <span className="dependency-lens-warning">이번 결과의 로컬 캐시를 저장하지 못했습니다.</span>
                )}
              </div>
            )}
          </div>
            ...
            {report.sources.length ? (
              <div className="dependency-source-list">
                {report.sources.map((source) => (
                  <div
                    className={`dependency-source status-${source.status}`}
                    key={`${source.ecosystem}:${source.path}`}
                  >
                    ...
                    <span className="dim">
                      {source.packageCount} packages · {source.directCount} direct
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div className="dependency-lens-section">
                ...
                return (
                  <details className="dependency-package" key={dependency.id}>
                    <summary>
                      ...
                      {remote && remote.osv.advisoryIds.length > 0 && (
                        <span className="dependency-remote-badge danger">OSV {remote.osv.advisoryIds.length}</span>
                      )}
                      {remote?.depsDev.deprecated && <span className="dependency-remote-badge warn">deprecated</span>}
                      <span className="dim">
                        {ECOSYSTEM_LABEL[dependency.ecosystem]} · edge {dependency.dependencies.length}
                      </span>
                    </summary>
            ...
            {filteredPackages.total > filteredPackages.visible.length && (
              <div className="dim">
                렌더링 상한으로 {filteredPackages.visible.length}개만 표시합니다. 필터를 좁혀 주세요.
              </div>
            )}
          </div>
        </>
