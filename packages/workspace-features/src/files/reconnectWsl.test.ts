...
function fixture() {
  ...
  const editor = {
    current: () => docs,
    active: vi.fn(() => true),
    replace: vi.fn((doc: Doc) => {
      docs = docs.map((old) => (old.id === doc.id ? doc : old));
    }),
    conflict: vi.fn(),
  };
  const connection = {
...
describe("WSL file reconnection", () => {
  it("refreshes unchanged native snapshots while preserving edits, selection and bookmarks", async () => {
    const f = fixture();
    f.connection.open.mockImplementation(async () => {
      f.change({ text: "newer edit", revision: 2 });
      return f.opened;
    });
    await reconnectWsl(f.editor, f.connection);
    expect(f.editor.current()[0]).toMatchObject({
      text: "newer edit",
      dirty: true,
      revision: 2,
      nativeRevision: "new",
  ...
  it("reloads a clean buffer but preserves edits made during the read", async () => {
    const f = fixture();
    f.change({ dirty: false });
    f.opened.contentHash = "changed";
    await reconnectWsl(f.editor, f.connection);
    expect(f.editor.current()[0]).toMatchObject({ text: "disk", nativeRevision: "new", dirty: false });
    const racing = fixture();
    racing.change({ dirty: false });
    racing.opened.contentHash = "changed";
    racing.connection.open.mockImplementation(async () => {
      racing.change({ text: "typed", revision: 2, dirty: true });
      return racing.opened;
    });
  ...
  it("preserves unavailable files and stops publication after context changes", async () => {
    const f = fixture();
    f.connection.open.mockRejectedValue(new Error("missing"));
    ...
    const racing = fixture();
    racing.connection.open.mockImplementation(async () => {
      racing.editor.active.mockReturnValue(false);
      return racing.opened;
    });
    await reconnectWsl(racing.editor, racing.connection);
  ...
  it("does not read documents when retirement or native admission fails", async () => {
    const f = fixture();
    f.connection.reconnect.mockRejectedValue(new Error("retirement unconfirmed"));
