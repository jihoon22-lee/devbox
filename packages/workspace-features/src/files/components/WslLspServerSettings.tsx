...
function withoutLanguage(config: LspConfig, language: string): LspConfig {
  ...
  const custom_servers = config.custom_servers
    .map((server) => ({
      ...server,
      language_ids: server.language_ids.filter((id) => id !== language),
    }))
...
export default function WslLspServerSettings({ config, disabled, onApply, onDirty }: Props) {
  ...
  return (
    <>
      ...
      {Object.values(config.server_by_language).some((server) => server.kind === "managed") && (
        <p className="lsp-warning" role="status">
          Windows 관리형 서버 설정이 남아 있습니다. 해당 언어를 Linux 서버로 바꾸거나 설정을 제거한 뒤 실행을
          검토하세요.
        </p>
      )}
      <label>
      ...
      />
      <p className="lsp-warning" role="status">
        WSL에서는 설치된 서버만 사용하며 자동 설치는 제공하지 않습니다. 이름 변경과 여러 파일에 걸친 편집 적용은 아직
        지원하지 않습니다.
      </p>
    </>
  );
...
function ServerForm({ config, language, disabled, onApply, onDirty }: Props & { language: string }) {
  ...
  const [executable, setExecutable] = useState(
    custom?.executable ??
      (direct?.kind === "local"
        ? direct.executable || direct.installed_path
        : direct?.kind === "custom"
          ? direct.executable
          : ""),
  );
      ...
      <div className="lsp-config-grid">
        <label>
          ...
          <select
            ...
            value={kind}
            onChange={(event) => {
              setKind(event.currentTarget.value as "native" | "node");
              onDirty(true);
            }}
          >
        ...
        <label className="lsp-wide-field">
          ...
          <input
            ...
            placeholder={
              kind === "node"
                ? "/opt/lsp/node_modules/typescript-language-server/lib/cli.mjs"
                : "/opt/lsp/rust-analyzer"
            }
          />
        </label>
        {kind === "node" && (
          <label className="lsp-wide-field">
            ...
            <input
              ...
              value={runtime}
              onChange={(event) => {
                setRuntime(event.currentTarget.value);
                onDirty(true);
              }}
      ...
      <div className="lsp-config-actions">
        <button
          ...
          onClick={() => {
            ...
            setArgs("");
            setError(null);
            onApply(withoutLanguage(config, language));
            onDirty(false);
          }}
        >
          이 언어 설정 제거
        </button>
        <button type="button" className="toolbar-button selected" disabled={disabled} onClick={apply}>
          이 언어 설정 적용
        </button>
      </div>
    </>
