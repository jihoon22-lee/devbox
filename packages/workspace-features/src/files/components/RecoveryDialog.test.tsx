...
const api = vi.hoisted(() => ({
  ...
  applyRecoveryPreview: vi.fn(),
  cancelRecoveryPreview: vi.fn(),
...
beforeEach(() => {
  vi.resetAllMocks();
  api.loadRecoveryState.mockResolvedValue({
    nativeRevision: "r1",
    entries: [{ path: "C:/fixture.txt", content: "unsaved", baseHash: "hash", snapshotAtMs: 1 }],
  });
  api.prepareRecovery.mockResolvedValue({
    previewId: "native-preview",
    path: "C:/fixture.txt",
    before: "disk",
    after: "unsaved",
  });
...
it("applies only the native token and disables duplicate approval until it completes", async () => {
  let finish!: () => void;
  api.applyRecoveryPreview.mockImplementation(
    () =>
...
it("retires a preview that arrives after the dialog unmounts", async () => {
  ...
  api.prepareRecovery.mockImplementation(
    () =>
      new Promise((resolve) => {
        finish = resolve;
      }),
...
it("preserves unavailable entries while applying another entry with the loaded revision", async () => {
  api.loadRecoveryState.mockResolvedValue({
    nativeRevision: "r1",
    entries: [
      { path: "missing", content: "keep", baseHash: null, snapshotAtMs: 1 },
      { path: "available", content: "unsaved", baseHash: null, snapshotAtMs: 2 },
    ],
  });
  api.prepareRecovery
    .mockRejectedValueOnce(new Error("file unavailable"))
    .mockResolvedValueOnce({ previewId: "available-token", before: "disk", after: "unsaved" });
  const done = vi.fn(),
    view = render(<RecoveryDialog onDone={done} />);
...
it("chains discard revisions and retains backups when a later discard conflicts", async () => {
  api.loadRecoveryState.mockResolvedValue({
    nativeRevision: "r1",
    entries: [
      { path: "a", content: "unsaved", baseHash: null, snapshotAtMs: 1 },
      { path: "b", content: "unsaved", baseHash: null, snapshotAtMs: 2 },
    ],
  });
  api.prepareRecovery.mockImplementation(async (path: string) => ({
    previewId: path,
    before: "disk",
    after: "unsaved",
  }));
  ...
  expect(api.discardRecovery.mock.calls).toEqual([
    ["a", "r1"],
    ["b", "r2"],
  ]);
