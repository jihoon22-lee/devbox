...
export default function RecoveryDialog({ onDone }: Props) {
    ...
    void (async () => {
      ...
      for (const e of entries) {
        try {
          ...
          if (preview) {
            if (!active) {
              void cancelRecoveryPreview(preview.previewId).catch(() => {});
              return;
            }
          ...
          failed.push({ path: e.path, issue: cause instanceof Error ? cause.message : String(cause) });
        }
      }
      if (active) {
        setPreviews(approvals);
        setItems(built);
        setUnavailable(failed);
      }
    })().catch((cause) => {
      if (active) setError(cause instanceof Error ? cause.message : "복구 내용을 확인하지 못했습니다.");
    });
    return () => {
      active = false;
  ...
  return (
    <div className="recovery-dialog">
      <h2>복구할 미저장 변경</h2>
      <p className="recovery-note">이전에 저장하지 못한 버퍼입니다. 적용하려면 복구, 버리려면 폐기를 선택하세요.</p>
      {error && (
      ...
      {unavailable.length > 0 && (
        <section aria-label="확인하지 못한 복구 문서">
          <p>다음 버퍼는 보존했습니다. 파일 접근을 확인한 뒤 다시 검토해 주세요.</p>
          <ul>
            {unavailable.map((item) => (
              <li key={item.path}>
                {item.path} · {item.issue}
              </li>
            ))}
          </ul>
          <button type="button" disabled={busy} onClick={() => setRevision((value) => value + 1)}>
      ...
      {items.length === 0 ? (
        <>
          <p>현재 적용할 수 있는 변경이 없습니다. 보관된 버퍼를 유지합니다.</p>
          <button type="button" onClick={() => onDone([])}>
            닫기
          </button>
        </>
