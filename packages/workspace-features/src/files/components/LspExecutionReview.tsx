...
import {
  approveLspExecution,
  cancelLspExecutionReview,
  previewLspExecution,
  revokeLspExecution,
  type LspExecutionPreview,
} from "../api";

export default function LspExecutionReview({
  disabled,
  nativeRevision,
}: {
  disabled: boolean;
  ...
  const revoke = () =>
    run(async (current) => {
      ...
      if (current()) {
        setPreview(null);
        setMessage("언어 서버를 종료하고 실행 승인을 해제했습니다.");
      }
    });

  return (
    <section className="lsp-status-section" aria-label="언어 서버 실행 승인">
      <h3>언어 서버 실행 승인</h3>
      ...
      {!preview && (
        <button
          ...
          disabled={disabled || busy || !nativeRevision}
          onClick={review}
        >
          실행 설정 검토
        </button>
      )}
      <button type="button" className="toolbar-button" disabled={busy || !nativeRevision} onClick={revoke}>
        실행 승인 해제
      </button>
      {preview && (
        <div className="lsp-execution-review">
          <p>
            작업 폴더: <code>{preview.workspaceRoot}</code>
          </p>
          {preview.commands.map((command) => (
            <article key={command.languageId}>
              <h4>{command.languageId}</h4>
              <p>
                실행 파일: <code>{command.executable}</code>
              </p>
              {command.runtime && (
                <p>
                  런타임: <code>{command.runtime}</code>
                </p>
              )}
              <p>인자</p>
              <pre>{command.args.length ? command.args.join("\n") : "없음"}</pre>
            </article>
          ))}
          <details>
            <summary>프로세스에 전달할 환경 변수</summary>
            <dl>
              {Object.entries(preview.environment ?? {}).map(([name, value]) => (
                <div key={name}>
                  <dt>{name}</dt>
                  <dd>
                    <code>{value}</code>
          ...
          <button type="button" className="toolbar-button selected" disabled={disabled || busy} onClick={approve}>
            이 설정의 실행 승인
          </button>
