...
vi.mock("../api", () => ({
  applyLspRecovery: vi.fn(),
  cancelLspRecovery: vi.fn(),
  listLspRecovery: vi.fn(),
  previewLspRecovery: vi.fn(),
}));
const preview: LspRecoveryPreview = {
  previewId: "once",
  journalId: "journal",
  files: [
    { path: "src/fixture.rs", restore: true, current: "after", original: "before", currentSize: 5, originalSize: 6 },
  ],
};
beforeEach(() => {
  vi.resetAllMocks();
  vi.mocked(listLspRecovery).mockResolvedValue({
    records: [{ journalId: "journal", files: 1, available: true }],
    truncated: false,
  });
  vi.mocked(previewLspRecovery).mockResolvedValue(preview);
  vi.mocked(cancelLspRecovery).mockResolvedValue();
  vi.mocked(applyLspRecovery).mockResolvedValue({
    complete: true,
    restored: ["src/fixture.rs"],
...
it("retires cancelled and late unmounted reviews without applying", async () => {
  ...
  vi.mocked(previewLspRecovery).mockReturnValue(
    new Promise((done) => {
      resolve = done;
    }),
  );
  ...
  view.unmount();
  await act(async () => {
    resolve({ ...preview, previewId: "late" });
  });
...
it("consumes a failed apply and reports partial recovery with retained backups", async () => {
  ...
  vi.mocked(applyLspRecovery).mockResolvedValueOnce({
    complete: false,
    restored: ["first.rs"],
    cleanupPending: true,
    error: "권한 변경",
...
it("does not offer malformed journals for recovery", async () => {
  vi.mocked(listLspRecovery).mockResolvedValue({
    records: [{ journalId: "bad", files: 0, available: false }],
    truncated: true,
  });
  const view = render(<LspRecoveryReview disabled={false} />);
  fireEvent.click(view.getByText("복구 기록 확인"));
