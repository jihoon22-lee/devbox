...
vi.mock("../api", () => ({
  approveLspExecution: vi.fn(),
  cancelLspExecutionReview: vi.fn(),
  previewLspExecution: vi.fn(),
  revokeLspExecution: vi.fn(),
}));
const preview: LspExecutionPreview = {
  previewId: "review-1",
  approved: false,
  ...
  commands: [
    {
      ...
      runtime: "C:/tools/node.exe",
      args: ["C:/tools/server.mjs", "--stdio"],
    },
  ],
  environment: { PATH: "C:/tools", SystemRoot: "C:/Windows" },
  definitionsDigest: "digest",
};
beforeEach(() => {
  vi.resetAllMocks();
...
it("retires a late preview after unmount and does not approve it", async () => {
  ...
  vi.mocked(previewLspExecution).mockReturnValue(
    new Promise((done) => {
      resolve = done;
    }),
  );
...
it("shows native WSL review with environment names and no values", async () => {
  vi.mocked(previewLspExecution).mockResolvedValue({
    ...preview,
    workspaceRoot: "/home/fixture",
    environment: undefined,
    environmentKeys: ["HOME", "PATH"],
    commands: [{ languageId: "rust", executable: "/usr/bin/server", args: ["--stdio"], runtime: null }],
  });
  const view = render(<LspExecutionReview disabled={false} nativeRevision="revision-1" />);
  fireEvent.click(view.getByText("실행 설정 검토"));
