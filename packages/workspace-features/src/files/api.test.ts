...
it("encodes native file action snapshots without losing timestamp precision or the native revision", async () => {
  const snapshot = {
    path: "C:/한글/file.txt",
    mtimeNanos: "9223372036854775807",
    size: 6,
    contentHash: "hash",
    nativeRevision: "native-open",
  };
  invoke.mockResolvedValue(null);
  await renameFileAction(snapshot, "renamed.txt");
  expect(invoke).toHaveBeenLastCalledWith("rename_file_action", {
    request: {
      path: snapshot.path,
      expectedMtimeNanos: snapshot.mtimeNanos,
      expectedSize: 6,
  ...
  expect(invoke).toHaveBeenLastCalledWith("delete_file_action", {
    request: {
      path: snapshot.path,
      expectedMtimeNanos: snapshot.mtimeNanos,
      expectedSize: 6,
