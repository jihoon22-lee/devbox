...
export async function reconnectWsl(editor: Editor, connection = native): Promise<void> {
  ...
  for (const before of documents) {
    ...
    try {
      opened = await connection.open(before.path, before.encoding);
    } catch {
      if (!editor.active()) return;
      unavailable = true;
    ...
    if (
      latest.nativeRevision !== before.nativeRevision ||
      (!sameDisk && (latest.dirty || latest.revision !== before.revision))
    ) {
      editor.conflict(before.path);
    } else {
      editor.replace(
    ...
    try {
      await connection.watch(before.path);
    } catch {
      unavailable = true;
    }
  }
  if (unavailable && editor.active())
    throw new WorkspaceOperationError(
      "일부 WSL 파일을 다시 연결하지 못했습니다. 편집 내용은 유지됩니다. 파일과 배포판 상태를 확인한 뒤 다시 시도해 주세요.",
    );
}
