...
function context(value: unknown): Context | null {
  ...
  if (
    ...
    return null;
  if (
    candidate.target.kind !== "windows" &&
    !(candidate.target.kind === "wsl" && typeof candidate.target.distroId === "string")
  )
    return null;
...
export function matchesLspEventContext(contextKey: string, nativeContext: unknown): boolean {
  ...
  try {
    selected = context(JSON.parse(contextKey));
  } catch {
    return false;
  }
  ...
  return Boolean(
    selected &&
      ...
      selected.worktreeId === event.worktreeId &&
      selected.revision === event.revision &&
      selected.target.kind === event.target.kind &&
      (selected.target.kind !== "wsl" ||
        (event.target.kind === "wsl" && selected.target.distroId === event.target.distroId)),
  );
}
