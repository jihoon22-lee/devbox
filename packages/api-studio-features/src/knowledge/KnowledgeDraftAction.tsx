...
export function KnowledgeDraftAction({ value, owner, source, disabled = false }: {
  ...
  return <div className="studio-knowledge-action">
    ...
    {saved && <div role="status"><p>API Studio에 마스킹한 초안을 보관했습니다. History & Console에서 다시 열 수 있습니다.{saved.redacted ? " 민감한 값은 마스킹되었습니다." : ""}</p>
      <SendDraft draft={saved} disabled={busy}/><button className="copy-btn" onClick={() => { try { exportDraft(saved); } catch { setError("보관한 초안을 내보내지 못했습니다."); } }}>보관한 초안 내보내기</button></div>}
    ...
    {open && <div className="studio-knowledge-backdrop"><div ref={dialog} className="studio-knowledge-dialog" role="dialog" aria-modal="true" aria-label="Knowledge 초안 보관 확인" aria-busy={busy}
      ...
      <h2>Knowledge 초안 보관</h2><p>현재 결과를 확인하세요. 마스킹한 사본을 API Studio에 보관합니다. 보관한 사본을 Knowledge에 전달해 검토하거나 이곳에서 직접 내보낼 수 있습니다.</p>
