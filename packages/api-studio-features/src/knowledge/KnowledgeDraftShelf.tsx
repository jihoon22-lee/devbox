...
export function KnowledgeDraftShelf() {
  ...
  const refresh = async () => {
    ...
    if (mounted.current && refreshRevision.current === revision)
      setItems(groups.flat().sort((a, b) => b.createdAtMs - a.createdAtMs));
  };
  ...
  useEffect(() => {
    ...
    if (native)
      void refresh().catch((error) => {
        if (current && mounted.current) setError(draftError(error));
      });
    return () => {
      current = false;
  ...
  const run = async (action: () => Promise<void>) => {
    ...
    try {
      ...
      if (mounted.current) setError(draftError(error));
    } finally {
  ...
  return (
    <section className="studio-knowledge-shelf" aria-label="보관한 Knowledge 초안">
      <h2>보관한 Knowledge 초안</h2>
      <p>명시적으로 보관한 마스킹 사본입니다. Requests와 Transforms가 각각 소유하며, 직접 삭제할 때까지 유지됩니다.</p>
      {!native ? (
        <p>초안 보관함은 데스크톱 앱에서 사용할 수 있습니다.</p>
      ) : (
        <>
          <button className="btn" disabled={busy} onClick={() => void run(refresh)}>
            보관함 새로고침
          </button>
          {error && <p role="alert">{error}</p>}
          <div className="studio-knowledge-items">
            {items.map((item) => (
              <button
                className="btn"
                key={item.artifact.id}
                disabled={busy}
                aria-pressed={selected?.artifact.id === item.artifact.id}
                onClick={() =>
                  void run(async () => {
              ...
              >
                {item.title} · {new Date(item.createdAtMs).toLocaleString()}
              </button>
            ))}
          </div>
          {!items.length && !error && <p>보관한 초안이 없습니다.</p>}
          {selected && (
            <div aria-label="보관한 초안 미리보기">
              <h3>{selected.title}</h3>
              <pre>{selected.body}</pre>
              <SendDraft key={selected.artifact.id} draft={selected} disabled={busy} />
              <button
                className="btn"
                disabled={busy}
                onClick={() =>
                  void run(async () => {
                    exportDraft(selected);
                  })
                }
              >
                초안 내보내기
              ...
              {confirmDelete ? (
                <>
                  <p>이 보관 사본을 삭제합니다. 내보낸 파일에는 영향을 주지 않습니다.</p>
