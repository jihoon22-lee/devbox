...
export function parseSummary(value: unknown, owner: DraftOwner): DraftSummary {
  ...
  if (
    artifact.kind !== "knowledge-draft/v1" ||
    typeof artifact.id !== "string" ||
    !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/u.test(artifact.id) ||
    provenance.product !== "api-studio" ||
    provenance.component !== owner ||
    typeof provenance.requestId !== "string" ||
    !/^[a-zA-Z0-9-]{1,64}$/u.test(provenance.requestId) ||
    !Number.isSafeInteger(provenance.revision) ||
    (provenance.revision as number) < 1 ||
    !Number.isSafeInteger(value.createdAtMs) ||
    (value.createdAtMs as number) < 1 ||
    typeof value.redacted !== "boolean" ||
    value.title !== `API Studio · ${owner === "api-studio.api" ? "Requests" : "Transforms"} 결과`
  )
    throw new Error(ERROR);
  return {
    artifact: artifact as unknown as DraftSummary["artifact"],
...
export function parseDraft(value: unknown, owner: DraftOwner): KnowledgeDraft {
  ...
  if (
    ...
    new TextEncoder().encode(body).length > 512 * 1024
  )
    throw new Error(ERROR);
  return { ...summary, body };
}
...
export function exportDraft(draft: KnowledgeDraft): void {
  ...
  try {
    ...
    link.href = url;
    link.download = `api-studio-knowledge-${draft.artifact.id}.txt`;
    link.click();
  } finally {
...
export async function sendDraft(owner: DraftOwner, id: string): Promise<void> {
  ...
  if (
    ...
    !["awaitingReview", "opening", "opened"].includes(String(result.phase))
  )
    throw new Error(ERROR);
}
