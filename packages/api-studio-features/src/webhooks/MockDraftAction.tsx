...
export function MockDraftAction({
  value,
  owner,
  source,
  status = 200,
  ...
  useEffect(() => {
    revision.current += 1;
    setNotice("");
  }, [value, status, mediaType, requestTarget, requestMethod]);
  async function send() {
    ...
    try {
      ...
      if (mounted.current && revision.current === version)
        setNotice(
          error instanceof Error && error.message === "mock_draft_busy"
  ...
  return (
    <span className="mock-draft-action">
      <button type="button" className="copy-btn" disabled={disabled || busy} onClick={() => void send()}>
        {busy ? "전달 중…" : label}
      </button>
