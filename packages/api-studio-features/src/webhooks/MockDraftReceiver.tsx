...
interface Preview {
  id: string;
  producer: "api-playground" | "developer-toolbox";
  expiresAtMs: number;
  rule: ResponseRule;
  redacted: boolean;
}
...
export function parseMockRule(value: unknown): ResponseRule {
  if (
    ...
    Object.keys(value).some(
      (key) => !["id", "priority", "method", "path", "status", "headers", "body", "delayMs", "sequence"].includes(key),
    ) ||
    (value.sequence !== undefined && (!Array.isArray(value.sequence) || value.sequence.length !== 0)) ||
    !Array.isArray(value.headers) ||
    value.headers.length !== 1 ||
    !Array.isArray(value.headers[0]) ||
    value.headers[0][0] !== "Content-Type" ||
    !["application/json", "text/plain; charset=utf-8"].includes(value.headers[0][1]) ||
    validateRule(value as unknown as ResponseRule).length
  )
...
export function parseMockPreview(value: unknown): Preview | null {
  ...
  return {
    id: value.id,
    producer: value.producer as Preview["producer"],
    expiresAtMs: value.expiresAtMs as number,
    redacted: value.redacted,
    rule: parseMockRule(value.rule),
  };
}
export function MockDraftReceiver({
  onApply,
  disabled,
  active = true,
}: {
  onApply: (rule: ResponseRule) => void;
  disabled: boolean;
  active?: boolean;
}) {
  ...
  return (
    <>
      ...
      {preview && (
        <div className="mock-draft-backdrop">
          <div
            ref={dialog}
            className="mock-draft-dialog"
            role="dialog"
            aria-modal="true"
            aria-label="Mock 규칙 초안 미리보기"
            aria-busy={busy}
            onKeyDown={(event) => {
              if (dialog.current)
                trapDialogKeyDown(event, dialog.current, () => {
                  if (!running.current) void finish(false);
                });
            }}
          >
            <h2>Mock 규칙 초안 미리보기</h2>
            <p>
              {preview.producer === "api-playground" ? "Requests" : "Transforms"}에서 전달한 결과입니다. 적용하면 현재
              규칙 초안을 바꿉니다. 일치 경로와 메서드를 확인한 뒤 별도로 규칙을 저장하세요.
            </p>
            <p>
              서버를 시작하거나 실행 중인 규칙을 자동 변경하지 않습니다.
              {preview.redacted ? " 민감한 값은 마스킹되었습니다." : ""}
            </p>
            <dl>
              <div>
                <dt>일치 조건</dt>
                <dd>
                  {preview.rule.method ?? "모든 메서드"} {preview.rule.path}
                </dd>
              </div>
              <div>
                <dt>응답</dt>
                <dd>
                  {preview.rule.status} · {preview.rule.headers[0][1]}
                </dd>
              </div>
            </dl>
            <pre aria-label="Mock 응답 본문">{preview.rule.body || "(빈 본문)"}</pre>
            {error && <p role="alert">{error}</p>}
            <button ref={cancel} className="btn" disabled={busy} onClick={() => void finish(false)}>
              취소
            </button>
            <button className="btn" disabled={busy || disabled} onClick={() => void finish(true)}>
              {busy ? "처리 중…" : "현재 규칙 초안 대신 적용"}
            </button>
          </div>
        </div>
      )}
