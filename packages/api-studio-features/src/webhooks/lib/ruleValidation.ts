...
export type RuleValidationInput = Pick<ResponseRule, "path" | "status" | "delayMs"> &
...
function addIssue(issues: RuleValidationIssue[], field: RuleValidationField, message: string): void {
 ...
 */
export function validateRule(rule: RuleValidationInput): RuleValidationIssue[] {
  const issues: RuleValidationIssue[] = [];
  const normalized = normalizedStrings(rule);

  if (
    typeof normalized.id !== "string" ||
    (!within(normalized.id, MAX_RULE_ID_CHARS, MAX_RULE_ID_BYTES) && normalized.id !== "") ||
  ...
  if (typeof normalized.body !== "string" || !within(normalized.body, MAX_BODY_CHARS, MAX_BODY_BYTES)) {
  ...
  if (metrics && (metrics.chars > MAX_RULE_COLLECTION_CHARS || metrics.bytes > MAX_RULE_COLLECTION_BYTES)) {
