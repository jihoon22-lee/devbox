...
`,
  ...
  it("isolates invalid operations and rejects unsupported documents", () => {
    const preview = previewOpenApiRules(
      JSON.stringify({
        openapi: "3.0.0",
    ...
    if (preview.ok) {
      expect(preview.preview.operations.map((operation) => operation.reason)).toEqual([
        "operationInvalid",
        "referenceUnsupported",
      ]);
    }
    expect(previewOpenApiRules("openapi: 2.0\npaths: {}", "yaml", "api.yaml")).toMatchObject({
      ok: false,
      code: "VERSION_UNSUPPORTED",
    });
    expect(previewOpenApiRules("{", "json", "api.json")).toMatchObject({ ok: false, code: "DOCUMENT_INVALID" });
  });
