...
const definition = {
  ...
  name: "Fixture",
  openApiVersion: "3.1",
  operations: [{ label: "GET /users", method: "GET", requestTarget: "/users", mockStatus: 200, request }],
};
...
it("opens a stored projection as a preview and applies only after explicit replacement", async () => {
  ...
  const onApply = vi.fn();
  render(<OpenApiDefinitions revision={0} linkedIds={null} onSummaries={vi.fn()} onApply={onApply} disabled={false} />);
  ...
  expect(invoke).toHaveBeenLastCalledWith("get_openapi_definition", { id });
  fireEvent.click(apply);
  expect(onApply).toHaveBeenCalledWith(expect.objectContaining({ method: "GET", url: request.url }));
...
it("requires explicit delete and removes the native-deleted record even when the list refresh fails", async () => {
  ...
  render(
    <OpenApiDefinitions revision={0} linkedIds={[id]} onSummaries={onSummaries} onApply={vi.fn()} disabled={false} />,
  );
  fireEvent.click(screen.getByText(/보관한 OpenAPI 작업 \(/));
  fireEvent.click(await screen.findByRole("button", { name: "Fixture · 1개" }));
  fireEvent.click(await screen.findByRole("button", { name: "보관한 작업 삭제…" }));
...
it("does not apply a late result after unmount", async () => {
  ...
  invoke.mockResolvedValueOnce(summaries).mockImplementationOnce(
    () =>
      new Promise((done) => {
        resolve = done;
      }),
  ...
  const onApply = vi.fn();
  const view = render(
    <OpenApiDefinitions revision={0} linkedIds={null} onSummaries={vi.fn()} onApply={onApply} disabled={false} />,
  );
  fireEvent.click(screen.getByText(/보관한 OpenAPI 작업 \(/));
  fireEvent.click(await screen.findByRole("button", { name: "Fixture · 1개" }));
  view.unmount();
