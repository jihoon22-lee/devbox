  ...
  return (
    <div className="handoff-backdrop">
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="saved-request-preview-title"
        onKeyDown={(event) => {
          if (dialog.current) trapDialogKeyDown(event, dialog.current, () => close.current());
        }}
      >
        <h2 id="saved-request-preview-title">저장된 요청 미리보기</h2>
        <p>{title}</p>
        ...
        <div className="handoff-dialog-actions">
          ...
          <button className="btn" disabled={!canApply} onClick={onApply}>
            현재 초안 대신 열기
          </button>
        </div>
      </section>
