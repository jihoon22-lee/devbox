...
import { type CollectionEntry, type CollectionStore, COLLECTION_VERSION } from "./collections";
import type { Environment, EnvironmentStore, EnvVariable } from "./environments";
import { safeMultipartFileName } from "./multipart";
import { isSensitiveName, REDACTED, sanitizeRequestForPersistence, toRequestTemplate } from "./persistence";
...
function safeMetadata(value: unknown, maxBytes = MAX_TRANSFER_FIELD_BYTES): value is string {
  return boundedString(value, maxBytes) && !hasUnsafeMetadataChars(value) && !looksLikeSecret(value);
}
...
function boundedName(value: unknown): value is string {
  return (
    typeof value === "string" &&
    value.length <= MAX_TRANSFER_NAME_CHARS &&
...
function isKeyValue(value: unknown): value is KeyValue {
  return (
    isRecord(value) &&
    onlyKeys(value, ["key", "value"]) &&
    safeMetadata(value.key) &&
...
function isHeader(value: unknown): value is RequestHeader {
  return (
    isRecord(value) &&
    onlyKeys(value, ["key", "value"], ["enabled"]) &&
    safeMetadata(value.key, 256) &&
    boundedString(value.value) &&
    (!isSensitiveName(value.key) || protectedValue(value.value)) &&
    !hasKnownSecret(value.value) &&
...
function isCookie(value: unknown): value is RequestCookie {
  return (
    isRecord(value) &&
    onlyKeys(value, ["name", "value"], ["enabled"]) &&
    safeMetadata(value.name, 256) &&
    protectedValue(value.value) &&
    (value.enabled === undefined || typeof value.enabled === "boolean")
  );
}
...
function isMultipart(value: unknown): value is MultipartPart {
  if (
    !isRecord(value) ||
    !onlyKeys(value, ["kind", "name", "value", "file_path", "file_name", "content_type"], ["enabled"])
  )
    return false;
  if (
    !safeMetadata(value.kind, 16) ||
    !safeMetadata(value.name, 256) ||
    !boundedString(value.value) ||
    !boundedString(value.file_path) ||
    !safeMetadata(value.file_name) ||
    !safeMetadata(value.content_type, 256) ||
    (value.enabled !== undefined && typeof value.enabled !== "boolean")
  )
    return false;
  if (value.kind === "file") {
    return (
      value.value === "" &&
      value.file_path === "" &&
      !/[\\/]/u.test(value.file_name) &&
      !hasKnownSecret(value.file_name)
    );
  }
  return (
    value.file_path === "" &&
...
function isGraphql(value: unknown): value is GraphqlRequest | null | undefined {
  ...
  if (
    !isRecord(value) ||
    !onlyKeys(value, ["query", "variables", "operation_name"]) ||
    boundedString(value.query) === false ||
    boundedString(value.variables) === false ||
    hasUnsafeMetadataChars(value.query) ||
    hasUnsafeMetadataChars(value.variables) ||
    !safeMetadata(value.operation_name, 256) ||
    hasKnownSecret(value.query)
  )
    return false;
...
function isSafeJsonValue(value: unknown, key: string): boolean {
  ...
  if (typeof value === "string")
    return boundedString(value) && !hasUnsafeMetadataChars(value) && !hasKnownSecret(value);
...
function isPersistedRequest(value: unknown): value is PersistedHistoryRequest {
  if (
    ...
    return false;
  return (
    safeMetadata(value.method, 32) &&
    value.method.length > 0 &&
    boundedString(value.url) &&
    !hasUnsafeMetadataChars(value.url) &&
    !hasKnownSecret(value.url) &&
    Array.isArray(value.headers) &&
    value.headers.length <= 100 &&
    value.headers.every(isHeader) &&
    Array.isArray(value.cookies) &&
    value.cookies.length <= 100 &&
    value.cookies.every(isCookie) &&
    Array.isArray(value.multipart) &&
    value.multipart.length <= 50 &&
    value.multipart.every(isMultipart) &&
    Array.isArray(value.params) &&
    value.params.length <= 100 &&
    value.params.every(isKeyValue) &&
    safeMetadata(value.body_kind, 32) &&
    boundedString(value.body) &&
...
function safeTransferMetadata(value: unknown, maxChars: number, maxBytes: number, fallback: string): string {
...
function cleanCollectionEntry(value: unknown): CollectionEntry | null {
  ...
  if (
    !boundedString(value.id, MAX_TRANSFER_ID_CHARS) ||
    !boundedName(value.name) ||
    !boundedName(value.folder) ||
    hasUnsafeMetadataChars(value.id) ||
    hasKnownMetadataSecret(value.id) ||
    hasKnownMetadataSecret(value.name) ||
    hasKnownMetadataSecret(value.folder) ||
    !Number.isSafeInteger(value.saved_at) ||
    Number(value.saved_at) < 0 ||
    typeof value.requiresSecretReview !== "boolean" ||
...
export function parseCollectionExport(raw: string): CollectionStore | null {
  ...
  if (
    !isRecord(value) ||
    !onlyKeys(value, ["schema", "schema_version", "collections"]) ||
    value.schema !== COLLECTION_EXPORT_SCHEMA ||
    value.schema_version !== TRANSFER_VERSION ||
    !Array.isArray(value.collections) ||
    value.collections.length > MAX_EXPORTED_COLLECTIONS
  )
    return null;
...
function isEnvironmentKey(value: string): boolean {
  return value.length > 0 && value.length <= 128 && /^[A-Za-z0-9_.-]+$/u.test(value) && !hasKnownSecret(value);
}

function exportEnvironmentVariable(variable: EnvVariable): ExportEnvironmentVariable | null {
  if (!isEnvironmentKey(variable.key) || !boundedString(variable.value) || hasUnsafeMetadataChars(variable.value))
    return null;
...
function cleanEnvironment(value: unknown): ExportEnvironment | null {
  if (
    !isRecord(value) ||
    !onlyKeys(value, ["id", "name", "variables"]) ||
    !boundedString(value.id, MAX_TRANSFER_ID_CHARS) ||
    !boundedName(value.name) ||
    hasUnsafeMetadataChars(value.id) ||
    hasKnownMetadataSecret(value.id) ||
    ...
    value.variables.length > MAX_EXPORTED_VARIABLES
  )
    return null;
  const variables = value.variables.map((candidate): ExportEnvironmentVariable | null => {
    if (
      !isRecord(candidate) ||
      !onlyKeys(candidate, ["key", "reference", "secret"], ["value"]) ||
      !boundedString(candidate.key, 128) ||
      !/^[A-Za-z0-9_.-]+$/u.test(candidate.key) ||
      candidate.reference !== `\${${candidate.key}}` ||
...
export function parseEnvironmentExport(raw: string): EnvironmentExportDocument | null {
  ...
  if (
    !isRecord(value) ||
    !onlyKeys(value, ["schema", "schema_version", "environments"]) ||
    value.schema !== ENVIRONMENT_EXPORT_SCHEMA ||
    value.schema_version !== TRANSFER_VERSION ||
    !Array.isArray(value.environments) ||
    value.environments.length > MAX_EXPORTED_ENVIRONMENTS
  )
    return null;
  ...
  return {
    ...
    environments: environments as ExportEnvironment[],
  };
}
...
export function mergeImportedCollections(
  ...
  if (
    current.collections.length > MAX_EXPORTED_COLLECTIONS ||
    imported.collections.length > MAX_EXPORTED_COLLECTIONS ||
    current.collections.length + imported.collections.length > MAX_EXPORTED_COLLECTIONS
  )
    return null;
...
export function mergeImportedEnvironments(
  ...
  if (
    current.environments.length > MAX_EXPORTED_ENVIRONMENTS ||
    imported.environments.length > MAX_EXPORTED_ENVIRONMENTS ||
    current.environments.length + imported.environments.length > MAX_EXPORTED_ENVIRONMENTS
  )
    return null;
...
function nextUniqueId(makeId: () => string, used: Set<string>): string | null {
  for (let attempt = 0; attempt < 1024; attempt += 1) {
    ...
    if (
      boundedString(candidate, MAX_TRANSFER_ID_CHARS) &&
      candidate.length > 0 &&
