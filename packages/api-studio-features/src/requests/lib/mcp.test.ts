...
describe("MCP frontend projections", () => {
  ...
  it("projects one list page and rejects duplicate identities across pages", () => {
    ...
    expect(() => appendMcpListPage(first.items, duplicate, "tools")).toThrow("mcp_message_invalid");
    expect(() => projectMcpListPage({ resources: [{ uri: "bad\nuri" }] }, "resources")).toThrow("mcp_message_invalid");
    expect(() => projectMcpListPage({ resources: [{ uri: "fixture://valid" }] }, "resources")).toThrow(
      "mcp_message_invalid",
    );
    expect(() => projectMcpListPage({ tools: [{ name: "bad", inputSchema: {} }] }, "tools")).toThrow(
      "mcp_message_invalid",
    );
    expect(() =>
      projectMcpListPage(
