...
describe("request cookie rows", () => {
  it("기존 enabled 누락은 활성으로 정규화하고 편집은 원본을 변경하지 않는다", () => {
    ...
    expect(updateCookie(source, 0, { enabled: false })).toEqual([{ name: "session", value: "abc", enabled: false }]);
  ...
  it("행 수를 100개로 제한한다", () => {
    ...
    expect(normalizeCookies([...full, { name: "overflow", value: "x" }])).toHaveLength(MAX_REQUEST_COOKIE_ROWS);
  ...
  it("활성 cookie를 순서대로 조립하고 직접 값만 마스킹한다", () => {
    ...
    expect(buildCookieHeader(rows)).toBe("session=plain; token=${COOKIE_TOKEN}; mixed=prefix-${COOKIE_TOKEN}; empty=");
    expect(buildCookieHeader(rows, true)).toBe("session=[REDACTED]; token=${COOKIE_TOKEN}; mixed=[REDACTED]; empty=");
  });
  ...
  it("구조화 cookie와 활성 raw Cookie header의 충돌만 감지한다", () => {
    ...
    expect(hasCookieSourceConflict(cookies, [{ key: "cookie", value: "legacy=x", enabled: false }])).toBe(false);
