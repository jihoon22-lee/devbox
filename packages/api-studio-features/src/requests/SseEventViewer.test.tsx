...
const events: SseEvent[] = [{ event: "message", data: "<masked> & safe", id: "event-1", retryMs: 1_000 }];
...
describe("SseEventViewer", () => {
  it("renders event payload as text and exposes pause state", () => {
    ...
    const { container } = render(
      <SseEventViewer events={events} dropped={3} paused={false} onPauseChange={onPauseChange} onError={errorMock} />,
    );
  ...
  it("copies only the explicit formatted masked range", async () => {
    render(<SseEventViewer events={events} dropped={0} paused={false} onPauseChange={vi.fn()} onError={errorMock} />);
