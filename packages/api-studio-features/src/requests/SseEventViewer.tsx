...
export function SseEventViewer({ events, dropped, paused, onPauseChange, onError }: SseEventViewerProps) {
  ...
  return (
    <section className="sse-viewer" aria-labelledby="sse-events-heading">
      <div className="sse-viewer-head">
        ...
        <span className="dim">
          {events.length}개 표시{dropped ? ` · ${dropped}개 제외됨` : ""}
        </span>
      ...
      <div className="sse-event-list" role="log" aria-live={paused ? "off" : "polite"} aria-label="SSE 이벤트 스트림">
        {events.length === 0 ? (
