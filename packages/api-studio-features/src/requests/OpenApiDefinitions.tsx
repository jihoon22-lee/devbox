...
export function parseDefinition(value: unknown): Definition {
  if (
    ...
    !["3.0", "3.1"].includes(value.openApiVersion as string) ||
  ...
  const operations = value.operations.map((op, index) => {
    if (
      ...
      typeof op.requestTarget !== "string" ||
      !op.requestTarget.startsWith("/") ||
      (op.mockStatus !== null &&
        (!Number.isInteger(op.mockStatus) || (op.mockStatus as number) < 100 || (op.mockStatus as number) > 599))
    )
      throw new Error(errorText);
    ...
    return {
      label: op.label,
      method: op.method,
      requestTarget: op.requestTarget,
      mockStatus: op.mockStatus as number | null,
      request: toRequestTemplate(request),
    };
  });
  return {
    schemaVersion: 1,
    id: value.id,
    name: value.name,
    openApiVersion: value.openApiVersion as Definition["openApiVersion"],
    operations,
  };
}
export function OpenApiDefinitions({
  revision,
  linkedIds,
  onSummaries,
  onApply,
  ...
  onSummaries: (items: DefinitionSummary[]) => void;
  onApply: (request: RequestTemplate) => void;
  disabled: boolean;
}) {
    ...
    if (
      ...
      result.some(
        (item) =>
          ...
          !Number.isSafeInteger(item.operationCount) ||
          (item.operationCount as number) < 1,
      )
    )
      throw new Error(errorText);
  ...
  useEffect(() => {
    ...
    if (isTauri())
      void list()
        .then((result) => {
          if (alive && generation.current === version) {
            setItems(result);
            summaries.current(result);
            setError("");
          }
        })
        .catch(() => {
          if (alive) setError(errorText);
        });
    return () => {
      alive = false;
  ...
  async function open(id: string) {
    ...
    try {
      ...
      if (mounted.current && generation.current === version) setError(errorText);
    } finally {
  ...
  async function remove() {
    ...
    try {
      ...
      if (mounted.current && generation.current === version)
        setError("삭제 결과 또는 목록 갱신을 확인하지 못했습니다. 다시 열어 보관 상태를 확인하세요.");
    } finally {
      running.current = false;
      if (mounted.current) setBusy(false);
    }
  ...
  return (
    <details className="api-workspace" aria-label="보관한 OpenAPI 작업">
      <summary>
        보관한 OpenAPI 작업 ({items.filter((item) => !linkedIds || linkedIds.includes(item.id)).length})
      </summary>
      <p className="dim">
        지원하는 요청과 Mock 초안을 보관합니다. 원문 전체와 지원하지 않는 스키마는 원래 문서에서 확인하세요.
      </p>
      {error && <p role="alert">{error}</p>}
      <div className="api-workspace-toolbar">
        {items
          .filter((item) => !linkedIds || linkedIds.includes(item.id))
          .map((item) => (
            <button className="btn mini" key={item.id} disabled={busy} onClick={() => void open(item.id)}>
              {item.name} · {item.operationCount}개
            </button>
          ))}
      </div>
      {definition && operation && (
        <div className="api-workspace-editor">
          <h2>
            {definition.name} · OpenAPI {definition.openApiVersion}
          </h2>
          <label>
            보관한 작업{" "}
            <select
            ...
            >
              {definition.operations.map((op, index) => (
          ...
          <p>
            {operation.method} {operation.requestTarget}
          </p>
          <pre>{operation.request.url}</pre>
          <div className="api-workspace-toolbar">
            <button
              className="btn"
              disabled={busy || disabled}
              onClick={() => onApply(structuredClone(operation.request))}
            >
              현재 요청 대신 적용
            </button>
            {operation.mockStatus !== null && (
              <MockDraftAction
                owner="api-studio.api"
                value=""
                status={operation.mockStatus}
                requestTarget={operation.requestTarget}
                requestMethod={operation.method}
                label="Mock 초안으로 전달"
                disabled={busy}
              />
            )}
            <button className="btn" disabled={busy} onClick={() => setDefinition(null)}>
              미리보기 닫기
            </button>
          </div>
          {confirmDelete ? (
            <div className="api-workspace-toolbar">
              <span>보관한 작업을 삭제합니다. Workspace의 연결 ID는 남으며 원본 문서는 유지됩니다.</span>
