...
const KNOWLEDGE_DRAFT_CREATE_ERROR = "Knowledge 초안을 만들거나 전달하지 못했습니다. 클립보드로 자동 전환하지 않습니다";
...
export function KnowledgeDraftAction(props: KnowledgeDraftActionProps) {
  ...
  return isProductHosted() ? (
    <ProductKnowledgeDraftAction {...props} owner="api-studio.transforms" source={source} />
  ) : (
        ...
        <div className="knowledge-draft-backdrop">
          ...
          >
            ...
            <p id="knowledge-draft-dialog-description">
              현재 결과를 확인한 뒤 명시적으로 Knowledge 초안 미리보기로 전달하세요. 저장은 Knowledge에서 별도로
              확인하며, 클립보드로 자동 전환하지 않습니다.
            </p>
            <dl className="knowledge-draft-meta">
              <div>
                <dt>원본</dt>
                <dd>Developer Toolbox</dd>
              </div>
              <div>
                <dt>대상</dt>
                <dd>Knowledge</dd>
              ...
              <div>
                <dt>형식</dt>
                <dd>
                  <code>knowledge-draft/v2</code>
                </dd>
              </div>
            </dl>
            <pre className="knowledge-draft-preview" aria-label="Knowledge 초안 결과">
              {preview}
            </pre>
            <p className="knowledge-draft-bounds" aria-label="Knowledge 초안 크기">
              {Array.from(preview).length.toLocaleString()} / {KNOWLEDGE_DRAFT_MAX_CHARS.toLocaleString()}자 ·{" "}
              {utf8ByteLength(preview).toLocaleString()} / {KNOWLEDGE_DRAFT_MAX_BYTES.toLocaleString()}바이트
            </p>
            {error ? (
