...
const chrome = [
  process.env.DEVBOX_TEST_CHROME,
...
try {
  ...
  await cdp.connect();
  const source = ts.transpileModule(readFileSync(path.join(root, "src/index.ts"), "utf8"), {
  ...
  assert.deepEqual(
    await cdp.evaluate("helpers.tabbableElements(document.getElementById('dialog')).map(node => node.id)"),
    expected,
  );
  ...
  await cdp.evaluate(
    "document.getElementById('dialog').addEventListener('keydown', event => helpers.trapDialogKeyDown(event, event.currentTarget))",
  );
