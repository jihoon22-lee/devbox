...
export const invoke = <T>(method: string, args?: Record<string, unknown>): Promise<T> =>
  product ? product(method, args ?? {}) : args === undefined ? legacyInvoke(method) : legacyInvoke(method, args);
