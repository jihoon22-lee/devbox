...
describe("parseBoundedOpenApiDocument", () => {
  ...
  it("rejects empty, oversized, dangerous, aliased, and unsafe-number input", () => {
    ...
    expect(parseBoundedOpenApiDocument("x".repeat(OPENAPI_DOCUMENT_LIMITS.maxBytes + 1), "yaml")).toEqual({
      ok: false,
      error: { code: "SOURCE_TOO_LARGE" },
    });
    expect(parseBoundedOpenApiDocument('{"constructor":{}}', "json")).toEqual({
      ok: false,
      error: { code: "DANGEROUS_KEY" },
    });
    expect(parseBoundedOpenApiDocument('{"value":9007199254740992}', "json")).toEqual({
      ok: false,
