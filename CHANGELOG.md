...
- **WSL Desktop 설정과 시작 동작** — 팬 하나 닫기 확인, 시작할 때 터미널 하나 열기, 글꼴,
  색 테마, 커서 모양·깜박임, 스크롤백 줄 수를 설정 대화상자에서 바꾸고 저장한다. 세션 유지
  방식과 사이드 패널 열림 상태도 창을 다시 열어도 유지되며, 저장된 유지 방식을 현재 배포판이
  제공하지 않으면 조용히 native로 되돌린다. 팬 머리글은 실제로 시작된 유지 방식과 기존 세션
  재연결 여부를 표시한다.

- **WSL Desktop 탐색과 방향 감각** — 탭 바가 `tablist`/`tab` 의미와 좌우·Home·End 이동을
  갖추고 활성 탭을 항상 시야 안으로 스크롤하며, 가운데 클릭·`Delete`로 닫고 더블 클릭으로
  이름을 바꾼다. `Alt+방향키`는 목록 순환 대신 현재 레이아웃의 실제 이웃 팬으로 이동한다.
  동시 입력 대상 팬은 터미널 격자에서 배지·테두리·접근성 이름으로 구분되고, 명령 팔레트가
  탭·레이아웃·클립보드·배포판 열기·설정까지 포함하도록 넓어지며 단축키를 함께 표시한다.
  실제 matcher 표에서 생성되는 단축키 안내 화면과 오류 배너 닫기 버튼을 추가했다.

- **WSL Desktop 팬 레이아웃과 렌더링** — 팬 사이 구분선을 끌어 크기를 조절하고(키보드 조절과
  균등 복원 포함) 활성 팬만 확대할 수 있다. 공식 `@xterm/addon-webgl`을 별도 chunk로 도입해
  대량 출력과 전체 화면 TUI를 GPU로 그리며, 로딩·컨텍스트 실패와 이후 컨텍스트 손실에서는
  조용히 DOM 렌더러로 남는다. 스크롤백 검색에 대/소문자·단어 단위·정규식 옵션과 버퍼 전체
  일치 강조가 붙고, 스크롤백 비우기·맨 아래로 이동·전체 선택 명령과 벨 표시를 추가했다.
