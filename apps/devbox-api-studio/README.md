...
`pnpm --filter devbox-api-studio dev` opens the explicitly labelled browser fixture. On Windows, `pnpm --filter devbox-api-studio tauri dev` runs the native shell. `?route=requests` selects a route for development. Native requests are restricted to the local main webview and validated by `product-shell-tauri`; route selection does not grant domain authority.
