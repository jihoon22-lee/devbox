...
vi.mock("@tauri-apps/api/core", () => ({
  isTauri: () => false,
  invoke: vi.fn(() => {
    throw new Error("native command in browser");
  }),
}));
afterEach(() => {
  cleanup();
...
it("keeps the actual request draft through protocol and webhook navigation without starting native work", async () => {
  ...
  await waitFor(() =>
    expect(within(nav).getByRole("button", { name: "프로토콜" }).getAttribute("aria-current")).toBe("page"),
  );
...
it("previews collections and History independently of the live request draft", async () => {
  ...
  localStorage.setItem(
    "apip-history-v2",
    JSON.stringify({ version: 2, history: [{ id: "history-fixture", saved_at: 1000, request, status: 200 }] }),
  );
  localStorage.setItem(
    "apip-collections-v2",
    JSON.stringify({
  ...
  fireEvent.click(within(nav).getByRole("button", { name: /^요청$/ }));
  expect(((await screen.findByPlaceholderText("https://api.example.com/users")) as HTMLInputElement).value).toBe(
    "https://fixture.test/unsaved-draft",
  );
  fireEvent.click(within(nav).getByRole("button", { name: "기록 및 콘솔" }));
  await act(async () => {
    await vi.dynamicImportSettled();
  ...
  fireEvent.click(
    within(screen.getByRole("region", { name: "HTTP 요청 기록" })).getByRole("button", { name: /fixture.test\/saved/ }),
  );
  fireEvent.click(screen.getByRole("button", { name: "현재 초안 대신 열기" }));
  expect(((await screen.findByPlaceholderText("https://api.example.com/users")) as HTMLInputElement).value).toBe(
    "https://fixture.test/saved",
  );
