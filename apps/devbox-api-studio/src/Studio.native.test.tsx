...
it("opens a native-owned pending transform preview through the actual hosted transport", async () => {
  ...
  native.invoke.mockImplementation(
    async (
      ...
      else if (request.method === "ack_pending_navigation") {
        value = null;
        navigation = null;
      ...
      else if (request.method === "preview_toolbox_text")
        value = {
          handoffId: id,
          producerId: "api-playground",
          expiresAtMs: Date.now() + 600_000,
          text: "fixture [REDACTED]",
          redacted: true,
        };
  ...
  await act(async () => {
    for (const callback of native.listeners.get("api-studio://navigate") ?? [])
      callback({ payload: "untrusted-event-is-not-a-route" });
  });
  await act(async () => {
    await vi.dynamicImportSettled();
  });
  ...
  await waitFor(() =>
    expect((screen.getByRole("textbox", { name: "스마트 워크플로 입력" }) as HTMLTextAreaElement).value).toBe(
      "fixture [REDACTED]",
    ),
  );
  fireEvent.click(screen.getByRole("button", { name: "비교의 이전 입력으로" }));
  expect((screen.getByRole("textbox", { name: "이전 버전 입력" }) as HTMLTextAreaElement).value).toBe(
    "fixture [REDACTED]",
  );
