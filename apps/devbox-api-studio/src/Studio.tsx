...
function Content({ route, navigate }: ShellContentProps) {
  useEffect(() => {
    ...
    const consume = async () => {
      ...
      if (
        ...
        (value.route !== "requests" && value.route !== "transforms" && value.route !== "webhooks")
      )
        return;
    ...
    void listen<unknown>("api-studio://navigate", wake)
      .then((stop) => {
        if (disposed) stop();
        else {
          unlisten = stop;
  ...
  return (
    <>
      {(visited.has("requests") || group === "requests") && (
        <div className="api-feature-requests" hidden={group !== "requests"}>
          <Suspense fallback={<p role="status">요청 화면을 불러오고 있습니다…</p>}>
            <Requests
              onNavigate={navigate}
              section={group === "requests" ? (route as typeof apiSection) : apiSection}
            />
          </Suspense>
        </div>
      )}
      {(visited.has("webhooks") || group === "webhooks") && (
        <div className="api-feature-webhooks studio-webhooks" hidden={group !== "webhooks"}>
          <Suspense fallback={<p role="status">Webhook 화면을 불러오고 있습니다…</p>}>
            {group === "webhooks" && <ListenerControls />}
            <Webhooks active={group === "webhooks"} />
          </Suspense>
        </div>
      )}
      {(visited.has("transforms") || group === "transforms") && (
        <div className="api-feature-transforms" hidden={group !== "transforms"}>
          <Suspense fallback={<p role="status">변환 도구를 불러오고 있습니다…</p>}>
            <IncomingSelection />
            <Transforms />
          </Suspense>
        </div>
