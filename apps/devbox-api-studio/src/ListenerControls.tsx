...
export function ListenerControls() {
    ...
    try {
      ...
      setError(
        code === "lifecycle_settings_changed"
          ...
          : code === "lifecycle_tray_unavailable"
            ? "알림 영역 아이콘을 사용할 수 없어 창을 숨길 수 없습니다."
  ...
  return (
    <section className="listener-controls" aria-label="Webhook 서버 종료 설정">
      ...
      {status?.policy === "keep-listening" && (
        <p>
          알림 영역 아이콘에서 창 열기·서버 중지·완전 종료를 선택할 수 있습니다.
          {!status.trayAvailable ? " 현재 아이콘을 사용할 수 없어 창을 닫으면 종료합니다." : ""}
        </p>
      )}
