...
function respond(foreign=false) {
 vi.mocked(invoke).mockImplementation(async(_command,args)=>{
  ...
  return {operation:{provenance:{product:foreign?"workspace":"control-center",component:"control-center.delivery",requestId:request.header.requestId,revision:catalog.catalogRevision},outcome:{state:"succeeded"}},value:request.method==="launch_suite_update"?{accepted:true}:{available:true,id,version:"0.9.0",bytes:1024,received:request.method==="check_suite_update"?0:1024,state:request.method==="check_suite_update"?"reviewed":"ready"}} as never;
 });
}
it("checks and downloads only on request and requires a separate install confirmation",async()=>{
 respond();render(<Updates description={fixtureDescription("control-center")} route="updates"/>);
 ...
 fireEvent.click(screen.getByRole("checkbox"));fireEvent.click(launch);
 await screen.findByRole("status");
 expect(invoke).toHaveBeenLastCalledWith("plugin:control-center|execute",expect.objectContaining({request:expect.objectContaining({method:"launch_suite_update",args:{id}})}));
});
it("rejects foreign native provenance before exposing executable actions",async()=>{
 respond(true);render(<Updates description={fixtureDescription("control-center")} route="updates"/>);
 fireEvent.click(screen.getByRole("button",{name:"업데이트 확인"}));
