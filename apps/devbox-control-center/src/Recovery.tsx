...
interface Status {state:"none"|"recorded";phase?:string;committed?:boolean;recovery?:string;backupCount?:number;dataCheckpointCount?:number;importCount?:number;cleanupPending?:number;failure?:string|null}
...
export default function Recovery({description,route}:ShellContentProps){
 ...
 return <section aria-label="Suite 복구 기록"><h2>설치·데이터 복구</h2>{error&&<p role="alert">{error}</p>}{status?.state==="none"?<p>이 설치에서 진행한 Suite 전환 기록이 없습니다.</p>:status?.state==="recorded"?<>
  ...
  <p>원본 백업 {status.backupCount}개 · 제품 데이터 보존본 {status.dataCheckpointCount??0}개 · 이전 기록 {status.importCount}개 · 구 설치 정리 대기 {status.cleanupPending}개</p>
