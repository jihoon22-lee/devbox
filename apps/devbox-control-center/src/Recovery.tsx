...
export default function Recovery({ description, route }: ShellContentProps) {
  ...
  useEffect(() => {
    ...
    setError("");
    if (!nativeMode) {
      setError("설치·복구 기록은 데스크톱 앱에서 확인할 수 있습니다.");
      return;
    }
    ...
    void deliveryCall(header, "suite_recovery", {})
      ...
      .catch(() => {
        if (active) setError("복구 기록을 읽지 못했습니다. 기존 기록과 백업은 보존됩니다.");
      });
    return () => {
      active = false;
    };
    ...
    <section aria-label="Suite 복구 기록">
      ...
      {status?.state === "none" ? (
        ...
        <>
          ...
          <p>
            원본 백업 {status.backupCount}개 · 제품 데이터 보존본 {status.dataCheckpointCount ?? 0}개 · 제품별 검증 기록{" "}
