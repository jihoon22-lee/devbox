...
vi.mock("@devbox/product-shell/commands", () => ({
  onConnectionChanged: vi.fn(async () => () => {}),
  searchCommands: vi.fn(),
  searchCommandSource: vi.fn().mockRejectedValue(new Error("unavailable")),
...
const item = {
  id: "control-center.open-tools",
  owner: "control-center",
  component: "control-center.shell",
  label: "도구",
  revision: "a".repeat(64),
  target: { kind: "route" as const, route: "tools" },
  requiredContext: "none" as const,
  context: null,
  destructive: false,
...
it("ignores an older search response and opens only the current native-reviewed route", async () => {
  ...
  vi.mocked(searchCommands).mockImplementation(async (_description, _route, query) =>
    query
      ? { results: [item], truncated: false }
      : new Promise((resolve) => {
...
it("keeps unavailable owners visible without invoking them", async () => {
  vi.mocked(searchCommands).mockResolvedValue({
    results: [
      {
        ...item,
    ...
    if (product === "knowledge")
      return {
        results: [
          {
            ...item,
            id: "knowledge.open-notes",
            owner: "knowledge",
            component: "knowledge.shell",
            label: "새 노트",
            target: { kind: "route", route: "notes" },
          },
        ],
        truncated: false,
      };
