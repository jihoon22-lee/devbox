...
export default function HostedLauncher({
  description,
  route,
  navigate,
  close,
}: ShellContentProps & { close: () => void }) {
  ...
  const adapter = useMemo<LauncherAdapter>(() => {
    ...
    const result = (sources: SourceDiagnostic[]): SearchResponse => ({
      sources,
      results: current
        .map((item) => ({
          id: item.id,
          ...
          detail: item.owner,
          source: item.owner,
          targetApp: item.owner,
          targetKind: item.target.kind,
          stale: false,
          explicitPreview: false,
          favorite: preferences.favorites.includes(item.id),
          recent: preferences.recents.includes(item.id),
    ...
    const command = (item: SearchResult) => {
      const found = current.find((row) => row.id === item.id && row.revision === item.revision);
      if (!found || found.disabledReason) throw new Error("stale command");
      return found;
    };
    const request = (item: Command, operationId: string) => ({
      operationId,
      commandId: item.id,
      revision: item.revision,
    ...
    return {
      async search(query) {
        ...
        try {
          preferences = await launcherPreferences(description, route);
        } catch {
          /* Search remains available; writes still reject corrupt preferences. */
        }
        if (ticket !== generation) return { results: [], sources: [] };
        current = base.results;
        const sources: SourceDiagnostic[] = providers.map((producer) => ({
          producer,
          view: "commands",
          status: "missing",
        }));
        for (const product of providers) {
          void searchCommandSource(description, route, product, query, ticket, "commands", signal)
            .then((remote: CommandSearch) => {
            ...
            .catch(() => {
              /* Per-source missing state remains separate from other results. */
            });
        }
      ...
      async launchResult(item) {
        ...
        const selected = await previewCommand(description, route, command(item), operationId);
        if (selected.owner === description.product.id && selected.target.kind === "route") {
          navigate(selected.target.route);
          await launcherPreferences(description, selected.target.route, {
            kind: "visit",
            command: request(selected, operationId),
          }).catch(() => undefined);
      ...
      async setFavorite(item, favorite) {
        const selected = favorite ? command(item) : current.find((row) => row.id === item.id);
        if (!selected) throw new Error("unknown command");
        preferences = await launcherPreferences(description, route, {
          kind: "favorite",
          command: request(selected, crypto.randomUUID()),
          favorite,
        });
      },
      async clearRecents() {
        preferences = await launcherPreferences(description, route, { kind: "clearRecents" });
      },
      ...
      previewTextAction: async () => {
        throw new Error("text action unavailable");
      },
      performTextAction: async () => {
        throw new Error("text action unavailable");
      },
  ...
  return (
    <dialog
      ref={dialog}
      className="hosted-launcher"
      onCancel={(event) => {
        event.preventDefault();
        close();
      }}
    >
      <Launcher adapter={adapter} embedded description="네 제품의 명령을 찾고 해당 제품에서 작업을 확인합니다." />
    </dialog>
  );
}
