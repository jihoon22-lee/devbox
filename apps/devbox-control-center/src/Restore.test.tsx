...
function respond(activeOperation:string|null=null, installation?:{phase:string;committed:boolean;recordedOwners:number;clean:boolean;freshHealth:boolean}) {
 vi.mocked(invoke).mockImplementation(async(_command,args)=>{
  ...
  return {operation:{provenance:{product:"control-center",component:"control-center.delivery",requestId:request.header.requestId,revision:catalog.catalogRevision},outcome:{state:"succeeded"}},
   value:request.method==="restore_inventory"?{installation,checkpoints:[{id:checkpoint,bytes:512,files:2}],operations:activeOperation?[{id:activeOperation,phase:"health"}]:[],activeOperation}:{accepted:true}} as never;
 });
}
