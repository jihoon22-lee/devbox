...
export default function Inventory({ description, route }: ShellContentProps) {
  ...
  const openFolder = async () => {
    ...
    try {
      ...
      setError("설치 폴더를 열지 못했습니다. 설치 상태를 새로고침한 뒤 다시 시도해 주세요.");
    } finally {
      setOpening(false);
    }
  };
  ...
  useEffect(() => {
    ...
    if (!nativeMode) {
      setBusy(false);
      setError("설치 상태는 데스크톱 앱에서 확인할 수 있습니다.");
      return;
    }
    const header = makeRequest(description.handshake, route, Date.now(), description.context);
    const provenance = {
    ...
    void deliveryCall(header, "suite_inventory", {})
      ...
      .catch(() => {
        if (active) setError("설치 상태를 확인하지 못했습니다. 마지막 결과가 있다면 그대로 표시합니다.");
      })
      .finally(() => {
  ...
  return (
    <section aria-label={route === "components" ? "구성 요소" : "제품 설치 상태"}>
      <h2>{route === "components" ? "구성 요소" : "제품 설치 상태"}</h2>
      <button onClick={reload} disabled={busy}>
        {busy ? "확인 중…" : "설치 상태 새로고침"}
      </button>
      {route === "products" && (
        <button
          onClick={() => void openFolder()}
          disabled={!nativeMode || busy || opening || snapshot?.declaration !== "verified"}
        >
          {opening ? "폴더 여는 중…" : "설치 폴더 열기"}
        </button>
      )}
      ...
      {snapshot && (
        <>
          <p>
            Suite {snapshot.suiteVersion ?? "버전 확인되지 않음"} · 패키지 {label(snapshot.declaration)}
          </p>
          {route === "components" ? (
            <table>
              <thead>
