...
interface Summary {schemaVersion:number;owner:string;suiteVersion:string;busy:boolean;setupSelected:boolean;reviewRequired:boolean;revision:string}
...
export default function MigrationOwners({description,route}:ShellContentProps){
 ...
 return <section aria-label="제품별 이전 상태"><h2>제품별 데이터 이전</h2>
  ...
  {rows.length>0&&<table><thead><tr><th>제품</th><th>상태</th></tr></thead><tbody>{rows.map(row=><tr key={row.owner}><td>{catalog.products.find(p=>p.id===row.owner)?.label??row.owner}</td><td>{!row.summary?"확인되지 않음":row.summary.busy?"이전 작업 진행 중":row.summary.reviewRequired?"이전 계획 검토 필요":row.summary.setupSelected?"저장소 선택 완료":"저장소 선택 필요"}</td></tr>)}</tbody></table>}
