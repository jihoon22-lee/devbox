...
복구 화면에서 제공한다. 화면 오류 복구는 실행 중인 프로세스의 메모리 보존이다. 별도의 노트 저널은
이미 기록한 편집 내용을 다음 실행에서 복원하는 데 사용한다. 화면 재시도는 해당 오류 경계의 lazy resource를 새로 만들어
loader를 다시 호출한다. 다른 기능의 resource와 노트 세션은 유지한다. 브라우저의 모듈 평가
실패 캐시나 잘못 배포된 청크를 우회한다고 보장하지 않으며, 계속 실패하면 복구 화면을 유지한다.
자동 새로고침으로 메모리 초안을 버리지 않는다.
미리보기는 문서 열기·내용 변경 세대와 원문 경로를 결과에 함께 보관하고, 현재 원문과 일치할
때만 표시한다. 같은 경로·같은 내용의 재열기도 새 원문이며 저장 상태 변경만으로는 재렌더하지
않는다. 새 미리보기 대기·실패 중에 이전 문서의 HTML이나 링크를 대신 표시하지 않는다.
...
트리는 DB mutex 아래에서 저장 위치와 요청 소유권만 가져온 뒤 blocking worker에서 순회한다.
파일 읽기·저장은 이 순회를 기다리며 같은 DB 잠금을 점유하지 않는다. 완료 시 저장 위치의
객체 identity와 요청 소유권을 다시 확인하며 A→B→A 위치 변경도 이전 요청을 무효화한다.
새로 접수된 탐색은 이전 탐색을 협력적으로 취소한다. 종료되지 않은 worker를 포함해 최대
2개만 허용하며, OS 호출이 멈춘 worker를 성공적으로 종료한 것으로 처리하지 않는다.

탐색 상한은 파일·폴더 항목 20,000개, 루트 포함 디렉터리 5,000개, 경로 깊이 64,
JSON 응답의 보수적 상한 4 MiB, 전체 조회 시간 5초다. 디렉터리는 스트리밍으로 읽고 빈
폴더도 작업량에 포함한다. 취소·시간 제한은 OS 호출 사이에 확인하므로 중단된 네트워크
파일시스템 호출 자체를 강제 선점하지는 않는다. 읽기 실패·상한 초과·취소는 명시적 오류이며
부분 목록을 정상 결과로 반환하지 않는다. 화면은 마지막 완전한 트리·태그를 유지하고 오류를
표시한다. 불완전한 목록의 누락을 파일 삭제 근거로 사용하지 않는다. 이름 변경의 전체 참조
탐색과 최초 위키링크 재색인도 같은 트리 상한에서 실패하며 부분 결과를 적용하지 않는다.

화면 갱신은 실행 중 1건과 후속 갱신 1건으로 합친다. 새 갱신 의도가 생긴 순간 이전 성공과
오류를 모두 무효화하며 두 조회가 종료된 뒤 후속 조회를 시작한다. unmount·StrictMode 재시작도
