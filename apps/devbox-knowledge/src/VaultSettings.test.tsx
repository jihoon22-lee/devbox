...
it("schedules a next-start folder review without switching the live binding", async () => {
  rpc.mockReset().mockImplementation(async (method: string, args: { path?: string } = {}) => ({
    schedule:
      method === "schedule_vault_change" ? { id: "native-plan", target: args.path, previousRoot: "C:/old" } : null,
  }));
