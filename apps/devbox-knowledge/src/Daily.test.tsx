...
it("requires a preview and explicit approval, consumes it once and opens only on request", async () => {
  invoke.mockImplementation(async (method: string) =>
    method === "preview_daily" ? preview : method === "save_daily" ? { path: preview.path, indexed: true } : null,
  );
  const { onOpen } = show();
  ...
  expect(invoke.mock.calls.filter((call) => call[0] === "save_daily")).toEqual([
    ["save_daily", { previewId: "daily-1" }],
  ]);
