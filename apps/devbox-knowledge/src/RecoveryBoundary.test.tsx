...
it("preserves a note and its quit guard when the entire product view fails", async () => {
  ...
  function Capture() { session = useNoteSession(); return null; }
  function Product({ failed }: { failed: boolean }) { if (failed) throw new Error("shell failed"); return <p>product</p>; }
  const content = (failed: boolean) => <NoteSessionProvider><Capture/><RecoveryBoundary name="Knowledge" recoverNote><Product failed={failed}/></RecoveryBoundary></NoteSessionProvider>;
  const view = render(content(false));
  await act(async () => { await session!.openPath("note.md", () => true); session!.edit("recover from shell error"); });
