...
const MigrationSettings = recoveryLazy(() => import("./MigrationSettings"));
...
function Content({ route, navigate }: ShellContentProps) {
  ...
  const captureRequest=incoming?.route==="notes"&&incoming.target.kind==="entity"&&incoming.target.entity==="capture"&&incoming.target.id==="inbox"?incoming.operationId:undefined;
  ...
  return <>
    {group === "notes" && showSettings && <div className="knowledge-settings-page"><button type="button" onClick={() => setShowSettings(null)}>노트로 돌아가기</button><RecoveryBoundary name="기능"><Suspense fallback={<p role="status">설정을 불러오고 있습니다…</p>}>{showSettings === "vault" ? <VaultSettings/> : <MigrationSettings/>}</Suspense></RecoveryBoundary></div>}
    {group === "daily" && <RecoveryBoundary name="기능"><Suspense fallback={<p role="status">일일 기록을 불러오고 있습니다…</p>}><IncomingSessionSummary onNotes={activateNotes}/><Daily date={date} onDateChange={setDate} onOpen={openNote} onActivity={activateActivity}/></Suspense></RecoveryBoundary>}
    {(visited.has("notes") || group === "notes") && <div className="knowledge-feature-notes" hidden={group !== "notes" || showSettings !== null}>
      <RecoveryBoundary name="노트" recoverNote><Suspense fallback={<p role="status">노트를 불러오고 있습니다…</p>}><IncomingResultDraft/><Notes active={group === "notes" && !showSettings} onActivate={activateNotes} onDaily={activateDaily} onImport={() => setShowSettings("import")} onVaultSettings={() => setShowSettings("vault")} openRequest={openRequest} captureRequest={captureRequest}/></Suspense></RecoveryBoundary>
    </div>}
    {(visited.has("activity") || group === "activity") && <div className="knowledge-feature-activity" hidden={group !== "activity"}>
      <RecoveryBoundary name="기능"><Suspense fallback={<p role="status">활동을 불러오고 있습니다…</p>}><Activity projectRevision={projectRevision} active={group === "activity"} selectedDate={date} onDateChange={setDate} onDaily={activateDaily} onDraft={activateNotes} lifecycleSettings={<RecoveryBoundary name="기능"><Suspense fallback={<p role="status">종료 설정을 불러오고 있습니다…</p>}><LifecycleSettings/></Suspense></RecoveryBoundary>}/></Suspense></RecoveryBoundary>
    </div>}
    {(visited.has("search") || group === "search") && <div className="knowledge-feature-search" hidden={group !== "search"}>
      <RecoveryBoundary name="기능"><Suspense fallback={<p role="status">검색을 불러오고 있습니다…</p>}><IncomingSearchReview onNoteOpen={activateNotes} onSaved={setSavedSearch}/><Search savedSearch={savedSearch} projectRevision={projectRevision} onNoteOpen={activateNotes}/></Suspense></RecoveryBoundary>
    </div>}
  </>;
}
export default function Knowledge() {
  return <NoteSessionProvider><QuitGuard/><RecoveryBoundary name="Knowledge" recoverNote><ProductShell product="knowledge" renderContent={props => {
    ...
    return <Startup>{available?<Content {...props}/>:<>{pending}<RecoveryBoundary name="기능"><Suspense fallback={null}><MigrationSettings/></Suspense></RecoveryBoundary></>}</Startup>;
  }}/></RecoveryBoundary></NoteSessionProvider>;
}
