...
function Content({ route, navigate }: ShellContentProps) {
  ...
  return <>
    ...
    {(visited.has("notes") || group === "notes") && <div className="knowledge-feature-notes" hidden={group !== "notes" || showSettings !== null}>
      <Suspense fallback={<p role="status">노트를 불러오고 있습니다…</p>}><IncomingResultDraft/><Notes active={group === "notes" && !showSettings} onActivate={activateNotes} onDaily={activateDaily} onImport={() => setShowSettings("import")} onVaultSettings={() => setShowSettings("vault")} openRequest={openRequest} captureRequest={captureRequest}/></Suspense>
    </div>}
...
export default function Knowledge() {
  return <><QuitGuard/><ProductShell product="knowledge" renderContent={props => {
