...
it.each([false, true])(
  "reinvokes only the failed boundary's loader and retains the dirty session (repeated failure: %s)",
  async (repeated) => {
        ...
        <>
          <button
            onClick={() => {
              void document
                .open(
                  async () => ({ path: "draft.md", content: "saved", revision: "r1" }),
                  () => true,
                )
                .then(() => {
                  document.edit("unsaved before failure");
                  setShow(true);
                });
            }}
          >
            open failing feature
          </button>
          <textarea
          ...
          />
          <RecoveryBoundary name="sibling">
            <Suspense>
              <Sibling />
            </Suspense>
          </RecoveryBoundary>
          {show && (
            <RecoveryBoundary name="failed" recoverNote>
              <Suspense>
                <Feature />
              </Suspense>
            </RecoveryBoundary>
