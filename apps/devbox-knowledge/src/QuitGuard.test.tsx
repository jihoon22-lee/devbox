...
beforeEach(() => {
  invoke.mockReset().mockImplementation((method: string) => Promise.resolve(method === "pending_quit" ? "quit-1" : null));
  listen.mockReset().mockResolvedValue(() => undefined);
});
afterEach(() => { cleanup(); unregister?.(); unregister = undefined; });
...
it("cancels a dirty exit without saving or terminating collection", async () => {
  ...
  render(<QuitGuard/>); await screen.findByRole("dialog");
...
it("waits for an in-flight write before explicit discard and ignores composing Escape", async () => {
  ...
  unregister=registerNoteEditor({unsaved:()=>true,saveBeforeQuit:async()=>false,settleBeforeQuit:()=>pending});
  render(<QuitGuard/>); const dialog=await screen.findByRole("dialog");
