...
afterEach(()=>{cleanup();native.invoke.mockReset();});
it.each([false,true])("automatically prepares a store, existing=%s",async(existing)=>{
 native.invoke.mockImplementation(async(method:string)=>method==="status"?{active:false,hasExisting:existing}:{active:true});
...
it("keeps a failed start idle until an explicit retry",async()=>{
 native.invoke.mockImplementation(async(method:string)=>{if(method==="status")return {active:false,hasExisting:false};throw new Error("저장소를 준비하지 못했습니다.");});
 render(<Startup><p>notes ready</p></Startup>);
...
it("accepts prepared readiness before Suite activation",async()=>{
 ...
 render(<Startup><p>prepared</p></Startup>);
