...
export default function QuitGuard() {
  ...
  useEffect(() => {
    ...
    const pending = async () => {
      ...
      try {
        ...
        else { setRequest(id); setError(""); }
      } catch { if (!disposed) setError("종료 요청을 확인하지 못했습니다. 다시 종료를 선택해 주세요."); }
      finally { handling.current = false; }
    };
    void import("@tauri-apps/api/event").then(async ({ listen }) => {
      const stop = await listen("knowledge://quit-request", () => { void pending(); });
      if (disposed) stop(); else { unlisten = stop; void pending(); }
    }).catch(() => { if (!disposed) setError("종료 알림에 연결하지 못했습니다. 편집 내용을 먼저 저장해 주세요."); });
  ...
  const decide = async (action: "save" | "discard" | "cancel") => {
    ...
    try {
      ...
      setRequest(null);
    } catch { setError("종료를 완료하지 못했습니다. 현재 편집 내용은 유지됩니다."); }
    finally { setBusy(false); }
  };
  if (!request) return error ? <p role="alert">{error}</p> : null;
  return <div className="modal-backdrop"><section ref={dialog} role="dialog" aria-modal="true" aria-labelledby="knowledge-quit-title" className="rename-dialog" onKeyDown={event => {
    if (isImeComposing(event)) return;
    if (event.key === "Escape" && !busy) { event.preventDefault(); void decide("cancel"); }
    ...
    <button disabled={busy} onClick={() => void decide("save")}>저장하고 종료</button>
    <button disabled={busy} onClick={() => void decide("discard")}>버리고 종료</button>
    <button disabled={busy} onClick={() => void decide("cancel")}>종료 취소</button>
  </section></div>;
}
