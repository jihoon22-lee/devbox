...
function RecoveryDraft() {
  ...
  return (
    <section aria-label="노트 복구">
      ...
      <button
        type="button"
        disabled={note.saving || !!note.conflict}
        onClick={() => {
          void document
            .saveBeforeQuit()
            .then((saved) =>
              setMessage(saved ? "노트를 저장했습니다." : "저장하지 못했습니다. 편집 내용은 유지됩니다."),
            );
        }}
      >
        보존된 노트 저장
...
export default class RecoveryBoundary extends Component<
  { children: ReactNode; name: string; recoverNote?: boolean },
  { failed: boolean; attempt: object }
> {
  ...
  render() {
    if (!this.state.failed)
      return <RecoveryAttempt.Provider value={this.state.attempt}>{this.props.children}</RecoveryAttempt.Provider>;
    return (
      <section aria-label={`${this.props.name} 복구`}>
        <p role="alert">{this.props.name} 화면을 표시하지 못했습니다.</p>
        {this.props.recoverNote && <RecoveryDraft />}
        <button type="button" onClick={() => this.setState({ failed: false, attempt: {} })}>
          화면 다시 시도
        </button>
      </section>
    );
