...
export async function vaultJob<T>(
  ...
  const failure = (issue?: string) =>
    committed ? new Error("폴더 연결은 저장되었습니다. 앱을 다시 시작해 마무리해 주세요.") : issueError(issue);
  for (let attempt = 0; attempt < 400 && !signal.aborted; attempt++) {
    const result = await vaultInvoke<{ state: string; value?: T; issue?: string; committed?: boolean }>(
      "vault_change_job",
      { id: jobId },
    );
