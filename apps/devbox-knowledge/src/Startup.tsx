...
export function Startup({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    let alive = true;
    void invoke<{ active: boolean; scheduled?: boolean; hasExisting?: boolean }>("status").then(value => { if (alive) { setActive(value.active === true); setHasExisting(value.hasExisting === true); setShowImport(value.scheduled === true && !value.active); } })
      .catch(error => { if (alive) { setError(error instanceof Error ? error.message : "저장소를 확인하지 못했습니다."); setBlocked(error instanceof Error && ["future_schema", "store_invalid", "restart_required", "import_schema_unsupported"].includes(error.name)); } })
  ...
  const start = async () => {
    ...
    catch (error) { setError(error instanceof Error ? error.message : "저장소를 준비하지 못했습니다."); setBlocked(error instanceof Error && ["future_schema", "store_invalid", "restart_required", "import_schema_unsupported"].includes(error.name)); }
