...
export default function VaultSettings({ onScheduled }: { onScheduled?: () => void } = {}) {
    ...
    void vaultInvoke("vault_change_status", {})
      .then((value) => {
        if (alive) {
          ...
          setPath(value.schedule?.target ?? "");
        }
      })
      .catch((error) => {
        if (alive) setError(error instanceof Error ? error.message : "노트 폴더 설정을 확인하지 못했습니다.");
      })
      .finally(() => {
  ...
  const change = async (cancel = false) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "노트 폴더 변경을 예약하지 못했습니다.");
    } finally {
      setBusy(false);
    }
  };
  return (
    <section className="knowledge-migration" aria-labelledby="vault-settings-title">
      <h2 id="vault-settings-title">노트 폴더 연결</h2>
      <p>
        {onScheduled
          ? "기존 폴더를 확인한 뒤 연결을 변경합니다."
          : "다음 시작에서 기존 폴더를 확인하고 연결을 변경합니다. 현재 노트를 저장한 뒤 앱을 다시 시작해 주세요."}
      </p>
      <p>기존 파일을 옮기거나 삭제하지 않습니다. 템플릿과 설정은 유지하며, 새 폴더의 노트를 다시 색인합니다.</p>
      <label className="knowledge-vault-choice">
        연결할 폴더{" "}
        <input
          aria-label="연결할 노트 폴더"
          value={path}
          maxLength={4096}
          disabled={busy || !nativeMode}
          onChange={(event) => setPath(event.currentTarget.value)}
          placeholder={"C:\\Notes 또는 WSL 폴더의 UNC 경로"}
        />
      </label>
      ...
      <div className="migration-actions">
        <button type="button" disabled={!nativeMode || busy || !path.trim()} onClick={() => void change()}>
          {onScheduled ? "선택한 폴더 확인" : "다음 시작에서 폴더 확인"}
        </button>
        {schedule && (
