...
export default function IncomingSearchReview({
  ...
  onNoteOpen: () => void;
  onSaved: (value: SavedSearchInput) => void;
}) {
  ...
  const id =
    review?.route === "search" &&
    review.target.kind === "entity" &&
    ["note", "file", "savedQuery"].includes(review.target.entity)
      ? review.target.id
      : null;
  useEffect(() => {
    let current = true;
    ...
    if (id && !savedTarget)
      void invoke<Reference>("source_reference", { reference: id })
        ...
        .catch(() => {
          if (current) setIssue("검색 결과가 만료되었거나 원본이 바뀌었습니다. 다시 검색해 주세요.");
        });
    return () => {
      current = false;
    };
  ...
  const open = async (editor = false) => {
    ...
    try {
      ...
      setIssue("원본을 확인하거나 열지 못했습니다. 다시 검색해 주세요.");
    } finally {
      setBusy(false);
    }
  };
  return (
    <section aria-label="받은 검색 결과 확인">
      <h2>검색 결과 열기</h2>
      {saved && (
        <>
          <p>{saved.name}</p>
          ...
          <button
            onClick={() => {
              onSaved({ ...saved, id: review!.operationId });
              clear();
            }}
          >
            저장한 검색 적용
      ...
      {reference ? (
        <>
          ...
          {reference.source === "files" && (
            <button disabled={busy} onClick={() => void open(true)}>
              Workspace Editor에서 열기
            </button>
          )}
          <button disabled={busy} onClick={() => void open()}>
            {reference.source === "notes" ? "노트에서 열기" : "기본 앱에서 열기"}
          </button>
        </>
      ) : (
        !issue && !saved && <p role="status">원본을 확인하고 있습니다…</p>
      )}
