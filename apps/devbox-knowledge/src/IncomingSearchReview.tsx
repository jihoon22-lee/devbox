...
export default function IncomingSearchReview({onNoteOpen,onSaved}:{onNoteOpen:()=>void;onSaved:(value:SavedSearchInput)=>void}){
 ...
 const open=async(editor=false)=>{
  ...
  try{await invoke(editor?"open_in":"open_file",editor?{appId:"devbox-workspace",reference:reference.reference}:{reference:reference.reference});clear();if(reference.source==="notes")onNoteOpen();}
 ...
 return <section aria-label="받은 검색 결과 확인">
  ...
  {reference?<><p>{reference.name}</p><p>{reference.path}</p>
   {reference.source==="files"&&<button disabled={busy} onClick={()=>void open(true)}>Workspace Editor에서 열기</button>}
