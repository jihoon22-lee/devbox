...
export default function Daily({ date, onDateChange, onOpen, onActivity }: Props) {
  ...
  const prepare = async () => {
    ...
    try {
      ...
      if (mounted.current && generation === request.current)
        setError(error instanceof Error ? error.message : "일일 노트를 확인하지 못했습니다.");
    } finally {
  ...
  const save = async () => {
    ...
    try {
      ...
      setNotice(
        saved.indexed ? "일일 노트를 만들었습니다." : "일일 노트를 저장했습니다. 검색 색인은 다시 확인해야 합니다.",
      );
    } catch (error) {
      if (mounted.current && generation === request.current) {
        setPreview(null);
  ...
  return (
    <section className="knowledge-daily" aria-labelledby="knowledge-daily-title">
      ...
      {preview && (
        <div className="knowledge-daily-preview">
          <p>{preview.path}</p>
          {preview.exists ? (
            <>
              <p>저장된 노트가 있습니다. 기존 내용을 열어 확인할 수 있습니다.</p>
              <button type="button" disabled={busy} onClick={() => onOpen(preview.path)}>
                노트 열기
              </button>
            </>
          ) : (
            <>
              ...
              <button type="button" disabled={busy || !nativeMode || !preview.previewId} onClick={() => void save()}>
                확인 후 새 노트 만들기
              </button>
              <button
                type="button"
