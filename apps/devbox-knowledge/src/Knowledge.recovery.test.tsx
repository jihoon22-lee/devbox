...
vi.mock("@devbox/knowledge-features/activity", () => ({
  default: () => {
    throw new Error("injected activity render failure");
  },
}));
vi.mock("@devbox/knowledge-features/search", () => {
  throw new Error("injected lazy import failure");
});
vi.mock("../../../packages/knowledge-features/src/notes/components/MarkdownEditor", () => ({
...
it("isolates actual Knowledge features and retains a dirty NoteDocument through editor recovery", async () => {
  ...
  expect(((await screen.findByLabelText("보존된 노트 내용")) as HTMLTextAreaElement).value).toBe(
  ...
  await waitFor(() =>
    expect((screen.getByLabelText("테스트 노트 편집") as HTMLTextAreaElement).value).toBe("latest recoverable edit"),
  );
}, 20_000);
