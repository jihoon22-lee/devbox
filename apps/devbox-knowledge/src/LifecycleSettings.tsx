  ...
  useEffect(() => {
    ...
    if (nativeMode) {
      void invoke<Policy>("get_close_policy")
        ...
        .catch((error) => {
          if (alive.current) setError(error instanceof Error ? error.message : "종료 설정을 확인하지 못했습니다.");
        });
    }
    return () => {
      alive.current = false;
  ...
  return (
    <section className="panel" aria-labelledby="knowledge-close-title">
      ...
      <label className="row">
        <input
          type="checkbox"
          checked={policy?.closeToTray ?? false}
          disabled={!nativeMode || !policy || busy || (!policy.trayAvailable && !policy.closeToTray)}
          onChange={(event) => void change(event.currentTarget.checked)}
        />
        창을 닫을 때 트레이로 숨기기
      </label>
      <p className="dim">
        기본값은 앱 종료입니다. 트레이로 숨기면 앱이 계속 실행되며, 이미 켠 활동 수집도 계속됩니다. 수집은 활동 화면에서
        별도로 켜거나 중지할 수 있습니다.
      </p>
      <p className="dim">
        트레이 메뉴의 ‘Knowledge 종료 · 수집 중지’를 선택하면 앱과 수집을 함께 종료합니다. 다음 실행에는 저장된 수집
        설정을 사용합니다.
      </p>
