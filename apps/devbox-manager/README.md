...
현재 컨텍스트 메뉴의 실행·폴더 열기·제거는 Manager 기본 root의 휴대용 설치에만 제공한다. 설치 패키지의 실제 설치 위치·uninstaller를 추측하지 않으며, custom root와 app binary/user data를 분리한 확장 제거는 v0.5.0 P2의 별도 기능이다. install path 표시와 Related Tools도 각 계획 항목에서 독립적으로 구현한다.
