...
vi.mock("../api", () => ({
  ...
  KNOWLEDGE_DRAFT_INPUT_ERROR: "Knowledge draft로 전달할 텍스트가 유효하지 않습니다",
  KNOWLEDGE_DRAFT_INVALID_ERROR: "Knowledge draft 응답을 사용할 수 없습니다",
...
function openPreview(value = "current output") {
  ...
  return screen.getByRole("dialog", { name: "Knowledge draft 미리보기" });
}
...
describe("Knowledge draft output action", () => {
  it("previews locally, focuses Cancel, and publishes only after confirmation", async () => {
    ...
    expect(dialog.textContent).toMatch(/chars · .* bytes/u);
    ...
    expect(screen.queryByRole("dialog", { name: "Knowledge draft 미리보기" })).toBeNull();
    expect(screen.getByRole("status").textContent).toContain("Knowledge draft 미리보기로 전달했습니다");
  ...
  it("rejects an unbounded output with a fixed error before native invocation", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Save draft" }));
    expect(screen.getByRole("alert").textContent).toContain("Knowledge draft로 전달할 텍스트가 유효하지 않습니다");
    expect(createKnowledgeDraftHandoffMock).not.toHaveBeenCalled();
  });
  ...
  it("closes on output change and ignores a late publish response", async () => {
    ...
    expect(screen.getByRole("button", { name: "Save draft to Knowledge" })).toBeTruthy();
  });
  ...
  it("does not echo an unexpected native error or copy output", async () => {
    ...
    expect(alert.textContent).toContain("Knowledge draft를 만들거나 전달하지 못했습니다");
