...
- **설정** — 툴바 `설정`과 명령 팔레트에서 연다. 팬 하나 닫기 확인 여부, 시작할 때 터미널
  하나 열기, 빠른 호출·트레이 유지, 세션 유지 방식, 선택 시 자동 복사, 글꼴 크기,
  글꼴(고정 목록), 색 테마(어두움/밝음/고대비), 커서 모양·깜박임,
  스크롤백 줄 수(1,000~100,000, 기본 10,000)를
...
- **빠른 호출(Quick Summon)** — 기본 `Ctrl+Alt+Space` 전역 단축키로 실행 중인 창을 새로
  만들지 않고 표시·focus하거나, 이미 focus된 창을 숨긴다. 네 가지 고정 조합 중에서 바꾸거나
  기능을 끌 수 있으며, 등록할 수 없는 조합은 다른 앱/Windows 예약 충돌 가능성과 함께 설정에
  명시한다. 변경 중 새 조합 등록이 실패하면 가능한 경우 이전 단축키를 복구한다. 선택적
  `닫을 때 트레이에 유지`를 켜면 닫기(X)는 창만 숨겨 현재 탭·활성 팬·PTY를 그대로 두고,
  트레이의 `완전히 종료`만 프로세스와 native PTY를 끝낸다. 트레이를 만들 수 없으면 닫기 동작은
  안전하게 앱 종료로 남는다. 설정을 끄면 기존처럼 닫기(X)가 앱을 종료한다.
...
- 공식 Tauri global-shortcut plugin은 Rust 쪽에서만 등록·해제하며 renderer에는 plugin의
  임의 register 권한을 열지 않는다. native 경계는 설정 UI와 같은 네 가지 조합만 허용한다.
...
- `localStorage`: cwd 핀·최근 경로 5개, selection 자동 복사 여부, 터미널 글꼴 크기, version 2
  설정(확인 동작·시작 동작·사이드 패널·빠른 호출 단축키·트레이 유지·유지 방식·글꼴 id·테마·
  커서·스크롤백), version 2
  마지막 레이아웃. version 1 레이아웃은 균등 분할 비율을 채워 읽고 다음 저장에서 version 2로
  바뀐다. version 1 설정은 기존 값을 보존하고 빠른 호출 기본값을 채우며 다음 설정 저장에서
  version 2로 바뀐다. 팬 크기 비율은 저장하지만 확대 상태는 창이 살아 있는 동안만 유지한다. 터미널
