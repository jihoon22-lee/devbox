...
beforeEach(() => {
  ...
  call.mockReset().mockImplementation(async (_description, _component, method) => {
    if (method === "terminal_sessions")
      return [{ id: "10000000-0000-4000-8000-000000000001", context: nativeContext, state: "active" }];
    if (method === "terminal_commands")
      return { profiles: [{ id: "profile", name: "Synthetic profile", revision: "a".repeat(64) }] };
    return {};
  });
});
afterEach(cleanup);
...
it("reuses the restore generation and operation after an ambiguous reply", async () => {
  ...
  call.mockImplementation(async (_description, _component, method, args) => {
    if (method === "terminal_sessions")
      return [{ id: "10000000-0000-4000-8000-000000000001", context, state: "interrupted", restoreGeneration: 3 }];
    if (method === "terminal_commands") return { profiles: [] };
    if (method === "restore_terminal") {
      requests.push(args);
      throw new Error("synthetic lost reply");
    }
    return {};
