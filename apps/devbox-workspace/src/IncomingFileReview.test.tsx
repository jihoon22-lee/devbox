...
const review: IncomingReview = {
  ...
  route: "files",
  context: null,
  target: { kind: "entity", entity: "file", id: "reference-1" },
};
function mount(context: ProjectContext | null, onOpen: ComponentProps<typeof IncomingFileReview>["onOpen"]) {
  return render(
    <IncomingReviewContext.Provider value={{ review, clear: () => {} }}>
      <IncomingFileReview description={{ context } as Description} onOpen={onOpen} />
...
it("accepts the same native context with reordered JSON keys and uses the editor's context key", async () => {
  const reordered: ProjectContext = {
    revision: 1,
    target: { kind: "windows" },
    worktreeId: "tree-1",
    projectId: "project-1",
  ...
  expect(open).toHaveBeenCalledWith({
    ...
    path: "C:\\fixture\\selected.txt",
    line: null,
    receivedReference: "reference-1",
  });
  expect(screen.queryByRole("alert")).toBeNull();
});
it("still rejects a changed native revision", async () => {
  vi.mocked(openReceivedFile).mockResolvedValue({
    path: "C:\\fixture\\selected.txt",
