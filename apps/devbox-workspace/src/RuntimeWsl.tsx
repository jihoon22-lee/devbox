...
} from "@devbox/workspace-features/terminal-dashboard";
...
export default function RuntimeWsl({ description, active }: { description: Description; active: boolean }) {
  ...
  const latest = useRef({ description, active });
  latest.current = { description, active };
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (!lastGood.current || value.revision >= lastGood.current.revision) {
        ...
        setSnapshot(value);
        setSelected((current) =>
          value.distros.some((distro) => distro.name === current)
            ? current
            : ((value.distros.find((distro) => distro.default) ?? value.distros[0])?.name ?? ""),
        );
      }
      setFreshness("fresh");
      setIssue("");
      ...
      if (mounted.current) {
        setFreshness("error");
        setIssue("WSL 상태를 읽지 못했습니다. 마지막으로 확인한 정보를 표시합니다.");
      }
    } finally {
  ...
  const perform = async (key: string, operation: () => Promise<unknown>) => {
    ...
    try {
      ...
      if (mounted.current) setIssue(error instanceof Error ? error.message : "WSL 작업을 완료하지 못했습니다.");
    } finally {
  ...
  const actionable = Boolean(
    snapshot && Date.now() <= snapshot.capturedAtMs + snapshot.staleAfterMs && freshness !== "error",
  );
  return (
    <section className="workspace-wsl" aria-label="WSL 및 컨테이너">
      <h2>WSL 및 컨테이너</h2>
      {issue && <p role="status">{issue}</p>}
      <DistroPanel
        distros={snapshot?.distros ?? []}
        dashboardDistros={snapshot?.distros ?? []}
        selectedDistro={selected}
        onSelectDistro={setSelected}
        containers={current?.containers ?? []}
        dockerMissing={current?.dockerAvailability === "missing"}
        busy={busy}
        snapshotState={freshness}
        snapshotActionable={actionable}
        onRefresh={() => void refresh()}
        onAction={(id, action) => {
          if (
            !actionable ||
            current?.dockerAvailability !== "available" ||
            !current.containers.some((container) => container.id === id)
          )
            return;
          void perform(`${id}:${action}`, () => runDockerControl(call, selected, id, action));
        }}
        onOpenTerminal={(distro) =>
          void perform(`terminal:${distro}`, () =>
            call("open_distro_terminal", { operationId: crypto.randomUUID(), distro }),
          )
        }
        onOpenJournalInLogLens={(distro) =>
          void perform(`journal:${distro}`, () => call("open_wsl_journal_in_log_lens", { distro, unit: null }))
        }
        onOpenFileInLogLens={(distro) => {
          setFileDistro(distro);
          setPath("");
        }}
      />
      {fileDistro && (
        <form
          onSubmit={(event) => {
            ...
            if (!path.startsWith("/") || path.split("/").includes("..") || /[\x00-\x1f\x7f]/.test(path)) {
              setIssue("WSL 파일의 절대 경로를 입력해 주세요.");
              return;
            }
            const distro = fileDistro;
            const wslPath = path;
            void perform(`file:${distro}`, async () => {
              await call("open_wsl_file_in_log_lens", { distro, wslPath });
              setPath("");
              setFileDistro(null);
            });
          }}
        >
          ...
          <button disabled={busy !== null || !path}>Logs에서 열기</button>
          <button
            type="button"
            disabled={busy !== null}
            onClick={() => {
