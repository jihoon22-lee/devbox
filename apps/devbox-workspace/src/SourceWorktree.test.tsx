...
const preview = {
  previewId: "native-review",
  branch: "native-branch",
  targetDir: "C:/reviewed/new-tree",
  root: "C:/reviewed/source",
};
beforeEach(async () => {
  description = await describeProduct("workspace");
  call.mockReset();
  call.mockImplementation(async (_description, _component, method) =>
    method === "preview_worktree" ? preview : method === "create_worktree" ? { path: preview.targetDir } : null,
  );
});
afterEach(cleanup);
...
it("creates only a reviewed native token and proposes registration on a separate click", async () => {
  ...
  expect(
    call.mock.calls.every(([, component, , , route]) => component === "workspace.source" && route === "source"),
  ).toBe(true);
});
it("cancels an active creation with its operation ID and retains the failed input", async () => {
  let reject!: (error: Error) => void;
  call.mockImplementation(async (_description, _component, method) =>
    method === "preview_worktree"
      ? preview
  ...
  await click("확인한 작업 폴더 생성");
  const operationId = (
    call.mock.calls.find(([, , method]) => method === "create_worktree")![3] as { operationId: string }
  ).operationId;
