...
export interface EditableDefinitions {
...
export default function DefinitionEditor({ description, view, disabled, onDirtyChange, onSaved }: Props) {
  ...
  function begin(next: Target) {
    setTarget(next);
    setBase(view.editRevision);
    setDraft(JSON.stringify(view[next], null, 2));
    setStatus("");
    setError("");
  }
  function exportProject() {
    const url = URL.createObjectURL(
      new Blob([JSON.stringify(view.project, null, 2) + "\n"], { type: "application/json" }),
    );
  ...
  return (
    <section className="workspace-definition-editor" aria-label="프로젝트 설정 편집" aria-busy={busy}>
      {error && <p role="alert">{error}</p>}
      {status && <p role="status">{status}</p>}
      {draft === null ? (
        <>
          <button type="button" disabled={disabled || busy} onClick={() => begin("project")}>
            공유 정의 편집
          </button>
          <button type="button" disabled={disabled || busy} onClick={() => begin("local")}>
            이 컴퓨터 설정 편집
          </button>
          <button type="button" disabled={disabled || busy} onClick={exportProject}>
            공유 정의 내보내기
          </button>
        </>
      ) : (
        <>
          ...
          <p>
            {target === "project"
              ? "저장 위치: .devbox/project.json. 공유할 정의만 입력하세요."
              : "설정은 이 컴퓨터에만 저장됩니다. 비밀 및 API 환경 연결은 기존 참조를 유지하세요."}
          </p>
          {!preview ? (
            <>
              <label>
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    void act(async (request) => {
                      ...
                      if (sequence.current === request) setDraft(content);
                    });
                  }}
                />
              </label>
            ...
            <section aria-label="설정 저장 확인">
              ...
              <div className="workspace-definition-comparison">
                <div>
                  <h5>저장 전</h5>
              ...
              <p>
                적용 설정의 추가: {preview.effectiveDiff.added.join(", ") || "없음"} · 변경:{" "}
                {preview.effectiveDiff.changed.join(", ") || "없음"} · 삭제:{" "}
                {preview.effectiveDiff.removed.join(", ") || "없음"}
                {preview.effectiveDiff.expectedPortsChanged ? " · 예상 포트 변경" : ""}
              </p>
                ...
                onClick={() =>
                  void act(async (request) => {
                    ...
                    try {
                      ...
                      setDraft(null);
                      setStatus(
                        saved.warning ? "저장했습니다. 디스크 반영 상태를 다시 확인해 주세요." : "설정을 저장했습니다.",
                      );
                    } finally {
                      if (sequence.current === request) onSaved();
                    }
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void act(async (request) => {
                    await call("cancel", { previewId: preview.previewId });
                    if (sequence.current !== request) return;
                    pending.current = null;
                    setPreview(null);
                  })
