...
} from "@devbox/workspace-features/overview-template-editor";
...
const fields: [keyof Omit<ProfileTemplateDraft, "id">, string][] = [
  ["name", "템플릿 이름"],
  ["windowsPath", "기본 Windows 폴더"],
  ["wslDistro", "기본 WSL 배포판"],
  ["wslPath", "기본 WSL 폴더"],
  ["gitRoot", "기본 Git 폴더"],
  ["expectedPortsText", "기본 포트 (쉼표로 구분)"],
...
export default function WorkspaceTemplateManager({
  ...
  useEffect(() => {
    onBusyChange(busy || !!edit || !!archive);
  }, [busy, edit, archive, onBusyChange]);
  async function act(action: () => Promise<void>) {
    ...
    try {
      ...
      if (alive.current) setError(cause instanceof Error ? cause.message : "템플릿을 저장하지 못했습니다.");
    } finally {
  ...
  return (
    <section aria-label="템플릿 관리" aria-busy={busy}>
      <h2>템플릿 관리</h2>
      <p>새 프로젝트에 사용할 기본값입니다. 수정하거나 보관해도 이미 만든 프로젝트 설정은 유지됩니다.</p>
      {error && <p role="alert">{error}</p>}
      <button
        disabled={disabled || busy || !!edit || !!archive}
        onClick={() => {
          setError("");
          setEdit({ id: null, revision: registry.revision, draft: emptyProfileTemplateDraft() });
        }}
      >
        새 템플릿
      </button>
      {(registry.importedTemplates ?? []).map((entry) => (
        <details key={entry.id}>
          ...
          <button
            ...
            onClick={() => {
              setError("");
              setEdit({ id: entry.id, revision: registry.revision, draft: templateDraftFromTemplate(entry.template) });
            }}
          >
            {entry.archived ? "템플릿 복원 검토" : "템플릿 수정"}
          </button>
          {!entry.archived && (
            <button
              disabled={disabled || busy || !!edit || !!archive}
              onClick={() => {
                setError("");
                setArchive({ entry, revision: registry.revision });
              }}
            >
              템플릿 보관
            </button>
      ...
      {edit && (
        <form
          ...
          onSubmit={(event) => {
            ...
            const result = validateProfileTemplateDraft(edit.draft);
            if (!result.template) {
          ...
          {fields.map(([field, label]) => (
            <label key={field}>
              ...
              <input
                ...
                maxLength={field === "name" ? 120 : 32768}
                required={field === "name"}
                onChange={(event) => setEdit({ ...edit, draft: { ...edit.draft, [field]: event.target.value } })}
              />
            </label>
          ))}
      ...
      {archive && (
        <section aria-label="템플릿 보관 확인">
          <p>
            {archive.entry.template.name}을 새 프로젝트 선택 목록에서 숨깁니다. 기존 프로젝트와 템플릿 내용은 유지하며
            나중에 복원할 수 있습니다.
          </p>
          <button
            disabled={disabled || busy}
