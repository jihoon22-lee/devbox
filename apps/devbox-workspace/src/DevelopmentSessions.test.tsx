...
it("previews commands without starting them and keeps restore-only approval separate", async () => {
  ...
  await waitFor(() =>
    expect(call.mock.calls.find(([, , method]) => method === "start_development_session")?.[3]).toEqual({
      id: session.id,
      revision: 2,
...
it("does not display a late plan from the previous worktree", async () => {
  ...
  call.mockImplementation(async (_description, _component, method) => {
    if (method === "development_candidates") return { jobs: [job], truncated: false };
    if (method === "development_sessions") return { sessions: [], intents: {} };
    if (method === "prepare_development_session")
  ...
  view.rerender(
    <DevelopmentSessions
      description={{ ...description, context: { ...context, worktreeId: "second" } }}
      registry={null}
    />,
  );
  finish?.({ session, jobs: [job], preflight });
  await waitFor(() =>
    expect((screen.getByRole("button", { name: "환경 확인·실행 검토" }) as HTMLButtonElement).disabled).toBe(false),
  );
    ...
    if (method === "prepare_development_session")
      return {
        ...
        preflight: {
          ...
          entries: [{ kind: "port", key: "8080", state: "occupied", blocking: true }],
        },
      };
    return {};
  });
...
it("shows a stopped-target report without creating or approving a session", async () => {
    ...
    if (method === "prepare_development_session")
      return {
        ...
        preflight: {
          ...
          restoreBlocked: true,
          executionBlocked: true,
          entries: [{ kind: "distro", key: "synthetic", state: "stopped", blocking: true }],
        },
      };
    return {};
  });
  ...
  expect(
    call.mock.calls.some(([, , method]) => ["start_development_session", "stop_development_session"].includes(method)),
  ).toBe(false);
});
...
it("previews native summary metadata and reuses the operation receipt", async () => {
    ...
    if (method === "prepare_session_summary") {
      ...
      return {
        ...
        draft: {
          title: "개발 세션 요약",
          body: "실패한 실행: 확인 불가",
          metadata: { binding: { context, sessionId: session.id, revision: session.revision } },
        },
      };
    }
    return {};
  ...
  expect(requests[0]).toEqual(
    expect.objectContaining({ sessionId: session.id, revision: session.revision, includeProblems: false }),
  );
});
...
it("keeps same-project controls and summary visible for reordered native context keys", async () => {
  ...
  call.mockImplementation(async (...args) => {
    if (args[2] === "development_sessions")
      return { sessions: [{ ...session, context: nativeContext, phase: "stopped" }], intents: {} };
    if (args[2] === "prepare_session_summary")
      return {
        operationId: "summary",
        draft: {
          title: "Native summary",
          body: "retained report",
          metadata: { binding: { context: nativeContext, sessionId: session.id, revision: session.revision } },
        },
      };
