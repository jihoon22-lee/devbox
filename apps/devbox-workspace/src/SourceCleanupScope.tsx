...
interface Status {
  available: { id: string; root: string }[];
  hasApproval: boolean;
  selectedIds: string[];
}
interface Preview {
  ...
  members: {
    id: string;
    root: string;
    review: { executionKeys: string[]; sources: { path: string; digest: string | null }[] };
  }[];
}
interface Props {
...
export default function SourceCleanupScope({
  description,
  enabled,
  blocked = false,
  onBusyChange,
  ...
  async function act(action: () => Promise<void>) {
    ...
    try {
      ...
      if (alive.current) setError(cause instanceof Error ? cause.message : "정리 범위를 확인하지 못했습니다.");
    } finally {
      if (alive.current) {
  ...
  return (
    <section className="workspace-source-cleanup-scope" aria-label="다른 작업 폴더 정리 범위" aria-busy={busy}>
      <h2>다른 작업 폴더 정리 범위</h2>
      <p>
        같은 저장소에 등록된 작업 폴더를 선택해 Git 실행 근거를 검토합니다. 범위 승인은 폴더를 삭제하지 않습니다. 아래
        정리 후보 검사와 최종 확인에서 삭제할 대상을 선택합니다.
      </p>
      <p>
        열린 파일 탭이 있는 폴더는 정리할 수 없습니다. 연결할 수 없는 승인 폴더가 있으면 범위를 다시 선택하거나 철회할
        수 있습니다.
      </p>
      ...
      {!preview && (
        <>
          <button type="button" disabled={blocked || busy || dirty} onClick={() => void act(load)}>
            정리 범위 확인
          </button>
          {status && (
            <>
              <p role="status">
                {status.hasApproval
                  ? "저장된 정리 범위가 있습니다. 실제 검사는 실행 직전에 다시 검증합니다."
                  : "다른 작업 폴더의 정리 범위를 승인하지 않았습니다."}
              </p>
              <fieldset disabled={blocked || busy}>
                <legend>검토할 등록 폴더 (최대 8개)</legend>
                {status.available.map((tree) => (
                  <label key={tree.id}>
                    <input
                      type="checkbox"
                      ...
                      onChange={(event) =>
                        setSelected((current) =>
                          event.target.checked ? [...current, tree.id] : current.filter((id) => id !== tree.id),
                        )
                      }
                    />
                    {tree.root}
              ...
              {dirty && (
                <button type="button" disabled={blocked || busy} onClick={() => setSelected(status.selectedIds)}>
                  범위 선택 되돌리기
                </button>
              )}
              <button
            ...
            <button
              ...
              onClick={() =>
                void act(async () => {
                  ...
                  onChange();
                  await load();
                  if (alive.current) setNotice("다른 작업 폴더의 정리 승인을 철회했습니다.");
                })
              }
            >
              정리 범위 승인 철회
      ...
      {preview && (
        <section aria-label="정리 범위 승인 확인">
          <p>
            이 폴더들의 Git 설정과 hook으로 상태를 검사하고, 별도로 확인한 안전한 폴더만 정리할 수 있습니다. 설정·실행
            파일·프로젝트 정의가 바뀌면 다시 검토합니다.
          </p>
          {preview.members.map((member) => (
            <div key={member.id}>
              ...
              <details>
                <summary>확인한 파일과 hook 경로</summary>
                <ul>
                  {member.review.sources.map((source, index) => (
                    <li key={index}>
          ...
          <button
            type="button"
            disabled={blocked || !enabled || busy}
            onClick={() =>
              void act(async () => {
                const previewId = preview.previewId;
                pending.current = null;
                setPreview(null);
                await call("approve_cleanup_scope", { previewId });
                if (!alive.current) return;
                onChange();
                await load();
                if (alive.current) setNotice("검토한 정리 범위를 승인했습니다. 정리 후보를 새로 검사해 주세요.");
              })
            }
          >
            검토한 정리 범위 승인
