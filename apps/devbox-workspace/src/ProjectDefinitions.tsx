...
interface View {
  ...
  effective: {
    tasks: Record<string, { source: string; selector: string }>;
    toolchains: Record<string, { tool: string; version: string }>;
  };
  sources: string[];
...
export default function ProjectDefinitions({ description, onDirtyChange, onChanged }: Props) {
  ...
  async function act(action: (request: number) => Promise<void>) {
    ...
    try {
      ...
      if (sequence.current === request)
        setError(cause instanceof Error ? cause.message : "프로젝트 설정을 확인하지 못했습니다.");
    } finally {
      if (sequence.current === request) {
        busyRef.current = false;
        setBusy(false);
  ...
  return (
    <section className="workspace-definitions" aria-label="프로젝트 설정" aria-busy={busy}>
      <button
        type="button"
      ...
      {open && (
        <>
          ...
          {view && (
            <>
              <p role="status">
                {view.definitionsTrusted
                  ? "현재 실행 정의의 승인을 확인했습니다."
                  : "현재 실행 정의를 검토해야 합니다."}
              </p>
              <p>승인은 현재 작업 폴더와 참조 파일 내용에 연결됩니다. 파일 내용이 바뀌면 다시 검토합니다.</p>
              <ul>
                {Object.entries(view.effective.tasks).map(([id, task]) => (
                  <li key={id}>
                    {id}: {task.source} · {task.selector}
                  </li>
                ))}
              </ul>
              {view.unavailableSources.length > 0 && (
                <p role="status">확인할 수 없는 실행 소스: {view.unavailableSources.join(", ")}</p>
              )}
              <details>
                <summary>공유 프로젝트 정의</summary>
                <pre>{JSON.stringify(view.project, null, 2)}</pre>
              </details>
              <details>
                <summary>이 컴퓨터의 설정</summary>
                <pre>{JSON.stringify(view.local, null, 2)}</pre>
              </details>
              {!preview && (
                <Suspense fallback={<p role="status">설정 편집 화면을 불러오고 있습니다…</p>}>
                  <DefinitionEditor
                    key={contextKey}
                    description={description}
                    view={view}
              ...
              {!preview && !editing && view.hasApproval && (
                <button
                  type="button"
                  disabled={busy}
                  onClick={() =>
                    void act(async (request) => {
                      await call("revoke_trust", { revision: view.registryRevision });
                      if (sequence.current !== request) return;
                      onChanged();
                      await load(request);
                    })
          ...
          {preview && (
            <section aria-label="실행 정의 승인 확인">
              <h3>이 실행 정의를 승인할까요?</h3>
              <p>
                아래 소스에서 선언한 작업과 도구 설정을 신뢰합니다. 승인 자체가 명령을 실행하거나 패키지를 설치하지는
                않습니다.
              </p>
              <ul>
                {preview.definition.sources.map((source) => (
