...
export default function ContextStatus({
  ...
  return (
    <div className="workspace-context-status" aria-label="현재 개발 context">
      <span title={root}>
        {name} · {root.split(/[\\/]/).filter(Boolean).slice(-1)[0]}
      </span>
      <span>{description.context.target.kind === "windows" ? "Windows" : "WSL"}</span>
      <span>
        {current?.summary
          ? "환경 참조 " + (current.summary.environmentReference ? "지정됨" : "없음")
          : "환경 참조 확인 전"}
      </span>
      <span>비밀 참조 {current?.summary ? current.summary.secretReferences + "개 지정됨" : "확인 전"}</span>
      <span title={current?.summary?.toolchains.join(", ")}>
        도구 요구 {current?.summary ? current.summary.toolchains.length + "개 선언됨" : "확인 전"}
      </span>
      <span>실행 중 작업 {current?.runningTasks ?? "확인 전"}</span>
      <button onClick={() => navigate("problems")}>
        문제{" "}
        {current?.initialized
          ? current.problems.filter((item) => !item.stale).length +
            (current.sources.some((source) => source.state !== "ready") ? " + 확인 필요" : "")
          : "수집 전"}
      </button>
    </div>
  );
