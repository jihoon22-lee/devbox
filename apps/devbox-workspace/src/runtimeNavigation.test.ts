...
const log = {
  ...
  fromRoute: "tasks",
  context,
  source: { kind: "runtimeRun", runId: "run-1", stream: "stdout", revision: "b".repeat(64) },
};
describe("Runtime navigation", () => {
  it("accepts the current native run reference independent of object key order", () => {
    expect(
      runtimeLogRequest(
        { ...log, context: { target: context.target, revision: 1, worktreeId: "tree-1", projectId: "project-1" } },
        context,
        "tasks",
      )?.source.runId,
    ).toBe("run-1");
  ...
  it("rejects late view/project changes and path-bearing references", () => {
    ...
    expect(
      runtimeLogRequest({ ...log, source: { ...log.source, path: "C:/private/data.db" } }, context, "tasks"),
    ).toBeNull();
...
describe("Runtime diagnostic navigation", () => {
  ...
  it("retains the verified file range", () => {
    expect(runtimeDiagnostic(diagnostic, context, "tasks")).toEqual({
      id: diagnostic.id,
      relativePath: "src/main.rs",
      line: 12,
      column: 3,
  ...
  it("discards stale destinations and malformed ranges", () => {
    ...
    for (const relativePath of [
      ...
      expect(runtimeDiagnostic({ ...diagnostic, relativePath }, context, "tasks")).toBeNull();
    for (const column of [0, -1, 0.5, "3", undefined])
      expect(runtimeDiagnostic({ ...diagnostic, column }, context, "tasks")).toBeNull();
  });
});
...
describe("one-time Terminal Logs requests", () => {
  const request = {
    id: "c".repeat(32),
    context,
    source: { kind: "wslFile", distro: "Ubuntu", path: "/var/log/synthetic.log" },
  };
  it("accepts only the native current-context adapter and normalizes journal null", () => {
    ...
    expect(
      terminalLogRequest(
        { ...request, context: null, source: { kind: "wslJournal", distro: "Ubuntu", unit: null } },
        null,
      )?.source,
    ).toEqual({ kind: "wslJournal", distro: "Ubuntu" });
  });
  it("rejects late project requests, traversal and extra execution fields", () => {
    ...
    expect(
      terminalLogRequest({ ...request, source: { ...request.source, path: "/var/../private" } }, context),
    ).toBeNull();
    expect(terminalLogRequest({ ...request, source: { ...request.source, command: "cat file" } }, context)).toBeNull();
    expect(
      terminalLogRequest({ ...request, source: { kind: "wslJournal", distro: "Ubuntu", unit: "--help" } }, context),
    ).toBeNull();
  });
});
