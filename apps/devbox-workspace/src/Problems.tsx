...
type Target =
  ...
  | { kind: "run"; runId: string; stream: string };
interface Item {
  id: string;
  source: string;
  revision: string;
...
export interface ProblemsSnapshot {
  ...
  summary?: { toolchains: string[]; secretReferences: number; environmentReference: boolean } | null;
  problems: Item[];
  sources: Array<{ source: string; identity: string; state: string; revision: string; truncated: boolean }>;
}
...
export default function Problems({
  ...
  onFile: (request: { id: string; relativePath: string; line: number; column: number | null }) => void;
  onLog: (request: RuntimeLogOpenRequest) => void;
  onRuntime?: (target: RuntimeFocusTarget) => void;
  ...
  const latest = useRef(description.context);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (mounted.current && sameRuntimeContext(description.context, latest.current)) {
        setUnavailable(true);
        setIssue("문제 목록을 읽지 못했습니다. 마지막 결과를 유지합니다.");
      }
    } finally {
  ...
  const open = async (item: Item, log = false) => {
    ...
    try {
      ...
      if (
        ...
        !sameRuntimeContext(result.context, latest.current)
      )
        return;
      ...
      if (mounted.current && sameRuntimeContext(context, latest.current))
        setIssue("원본 버전이 바뀌었거나 파일·로그를 더 이상 사용할 수 없습니다. 원본에서 진단을 다시 확인해 주세요.");
    } finally {
  ...
  if (!description.context)
    return (
      <section>
        <h1>문제</h1>
        <p>문제를 확인할 프로젝트를 선택해 주세요.</p>
      </section>
    );
  const rows = (current?.problems ?? []).filter(
    (item) =>
      (level === "all" || item.severity === level) &&
      (!filter || item.message.toLocaleLowerCase().includes(filter.toLocaleLowerCase())),
  );
  return (
    <section className="workspace-problems">
      <h1>문제</h1>
      <p>열린 문서와 최근 Source·의존성·작업·세션 조회 결과를 모아 표시합니다.</p>
      <div className="problem-controls">
        <label>
          문제 검색 <input value={filter} onChange={(event) => setFilter(event.target.value)} />
        </label>
        <label>
          수준{" "}
      ...
      {current?.sources.some((source) => source.state !== "ready") && (
        <p role="status">갱신 중이거나 확인할 수 없는 원본이 있습니다. 이전 결과는 위치 이동에 사용하지 않습니다.</p>
      )}
      {(current?.truncated || current?.sources.some((source) => source.truncated)) && (
        <p role="status">원본 또는 표시 한도에 도달해 일부 문제만 표시합니다.</p>
      )}
      <ul className="problem-list">
        {rows.map((item) => (
          <li key={item.id} className={item.stale ? "problem-stale" : ""}>
            <span className={"problem-severity problem-" + item.severity}>{severity[item.severity]}</span>
            <span>{labels[item.source] ?? item.source}</span>
            <p>{item.message}</p>
            {item.target.kind === "file" && (
              <small>
                {item.target.relativePath}:{item.target.line}
                {item.target.column !== null ? ":" + item.target.column : " · 열 위치 미확인"}
              </small>
            )}
            {item.stale && <span>이전 결과</span>}
            <div>
              <button disabled={busy || item.stale || unavailable} onClick={() => void open(item)}>
                {item.target.kind === "route" ? "원본 화면 열기" : "위치 열기"}
              </button>
              {item.log && (
                <button disabled={busy || item.stale || unavailable} onClick={() => void open(item, true)}>
                  연결된 실행 로그
                </button>
              )}
            </div>
