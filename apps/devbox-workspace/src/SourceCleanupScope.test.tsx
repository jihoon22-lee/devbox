...
const preview = {
  ...
  members: [
    { id: "native-sibling", root: "C:/native/sibling", review: { executionKeys: ["core.hooksPath"], sources: [] } },
  ],
};
beforeEach(async () => {
  description = await describeProduct("workspace");
  call.mockReset();
  call.mockImplementation(async (_description, _component, method) =>
    method === "cleanup_scope_status" ? empty : method === "preview_cleanup_scope" ? preview : null,
  );
});
afterEach(cleanup);
...
it("enumerates only on request, selects no sibling by default and approves only a native token", async () => {
  ...
  await click("선택한 정리 범위 검토");
  expect(call).toHaveBeenCalledWith(
    description,
    "workspace.source",
    "preview_cleanup_scope",
    { worktreeIds: ["native-sibling"] },
    "source",
  );
  ...
  expect(call.mock.calls.every(([, , method]) => !method.startsWith("repo_") && method !== "create_worktree")).toBe(
    true,
  );
});
it("retains selected inputs on cancelled review and allows private revocation when Git is unavailable", async () => {
  ...
  call.mockImplementation(async (_description, _component, method) =>
    method === "cleanup_scope_status" ? { ...empty, hasApproval: true, selectedIds: ["native-sibling"] } : null,
  );
  await click("범위 선택 되돌리기");
  await click("정리 범위 확인");
...
it("retires a preview arriving after the surface closes", async () => {
  ...
  call.mockImplementation(async (_description, _component, method) =>
    method === "cleanup_scope_status"
      ? empty
      : method === "preview_cleanup_scope"
        ? new Promise((done) => {
            resolve = done;
          })
        : null,
