...
const preview = {
  ...
  binding: { root: "/home/test/한글 project", target: { kind: "wsl" as const, distroId: distro.id } },
  discovery: { kind: "newProject" as const },
};
afterEach(() => {
  cleanup();
  call.mockReset();
...
it("cancels a late native preview after the form closes", async () => {
  ...
  await waitFor(() =>
    expect(call).toHaveBeenCalledWith("workspace.registry", "cancel_registration", { previewId: preview.previewId }),
  );
...
it("reviews WSL template defaults against the selected distro without registering or granting trust", async () => {
  const template = {
    ...
    template: {
      ...
      name: "WSL 웹 기본값",
      windowsPath: null,
      wsl: { distro: "Missing preset", path: "/missing/preset" },
      gitRoot: null,
      expectedPorts: [4321],
  ...
  call.mockImplementation(async (_component, method) =>
    method === "list_wsl_distros" ? [{ ...distro, running: true }] : preview,
  );
  const reviewed = vi.fn();
  const { container } = render(
    <WslProjectForm templates={[template]} disabled={false} onBusyChange={() => {}} onReviewed={reviewed} />,
  );
  await screen.findByRole("option", { name: "합성 Ubuntu · 실행 중" });
  ...
  expect(call).toHaveBeenCalledWith("workspace.registry", "preview_template_profile_wsl", {
