...
interface Restore { sourceRevision:string; revision:string; restoreRevision:string; profiles:number; preferences:number }
interface History { entries:Array<{sourceRevision:string;profiles?:number;preferences?:number;issue:string|null}> }
...
export default function TerminalImport({description,active}:{description:Description;active:boolean}) {
  ...
  const refresh=useCallback(async()=>{setJobs(await call<Job[]>("terminal_imports"));setHistory(await call<History>("terminal_import_history"));},[call]);
  useEffect(()=>{if(!active)return;let disposed=false;const poll=()=>void call<Job[]>("terminal_imports").then(value=>{if(!disposed)setJobs(value);}).catch(()=>{if(!disposed)setIssue("이전 작업 목록을 읽지 못했습니다.");});poll();void call<History>("terminal_import_history").then(value=>{if(!disposed)setHistory(value);}).catch(()=>{if(!disposed)setIssue("이전 기록을 읽지 못했습니다.");});const timer=setInterval(poll,2000);return()=>{disposed=true;clearInterval(timer);};},[active,call]);
  ...
  return <details className="workspace-terminal-import"><summary>WSL Desktop 프로필·설정 가져오기</summary>
    ...
    <ul>{jobs.map(job=><li key={job.id}>{states[job.state]??"상태 확인 필요"}
      {job.issue&&<span> · {job.issue==="terminal_import_copy_cleanup_pending"?"임시 복사본 정리 재시도 필요":job.issue==="terminal_legacy_missing"?"이전 WSL Desktop 데이터가 없습니다.":"원본 또는 중단된 작업 확인 필요"}</span>}
      {job.issue==="terminal_import_copy_cleanup_pending"&&<button disabled={busy} onClick={()=>void perform(async()=>{await call("cleanup_terminal_import",{id:job.id});})}>임시 복사본 정리 재시도</button>}
      ...
      {job.state==="ready"&&<button disabled={busy} onClick={()=>void perform(async()=>{setRestore(null);setReview(await call<Review>("preview_terminal_import",{id:job.id}));setReplace(false);})}>이전 내용 검토</button>}
    </li>)}</ul>
    ...
    {history.entries.length>0&&<section aria-label="터미널 이전 기록"><h3>가져오기 직전 상태로 복원</h3>
      <ul>{history.entries.map((entry,index)=><li key={entry.sourceRevision}>이전 기록 {index+1} · {entry.issue?"보관 파일 확인 필요":`프로필 ${entry.profiles}개 · 설정 ${entry.preferences}개`}
        <button disabled={busy||Boolean(entry.issue)} onClick={()=>void perform(async()=>{setReview(null);setRestore(await call<Restore>("preview_terminal_import_restore",{sourceRevision:entry.sourceRevision}));})}>복원 내용 검토</button>
      </li>)}</ul>
      {restore&&<section aria-label="터미널 설정 복원 확인"><p>프로필 {restore.profiles}개·설정 {restore.preferences}개의 이전 상태로 되돌립니다. 가져온 뒤 수정한 프로필과 설정도 이 상태로 교체됩니다. 현재 상태는 별도로 보관합니다.</p>
        <button disabled={busy} onClick={()=>void perform(async()=>{await call("restore_terminal_import",{sourceRevision:restore.sourceRevision,expectedRevision:restore.revision,restoreRevision:restore.restoreRevision});setRestore(null);})}>검토한 이전 상태로 복원</button>
        <button disabled={busy} onClick={()=>setRestore(null)}>취소</button>
      </section>}
    </section>}
  </details>;
}
