...
const view = {
  ...
  effective: { tasks: { dev: { source: "package.json", selector: "dev" } }, toolchains: {} },
  sources: ["package.json"],
  unavailableSources: [] as string[],
  definitionsTrusted: false,
  hasApproval: false,
};
beforeEach(async () => {
  vi.clearAllMocks();
  ...
  description = {
    ...
    context: { projectId: "project", worktreeId: "tree", revision: 2, target: { kind: "windows" } },
  };
  call.mockImplementation(async (_description, _component, method) =>
    method === "preview_trust" ? { previewId: "native-token", definition: view } : method === "load" ? view : {},
  );
});
afterEach(cleanup);
...
it("cancels a late preview after a context change instead of presenting it for approval", async () => {
  ...
  rendered.rerender(
    <ProjectDefinitions
      description={{ ...description, context: { ...description.context!, revision: 3 } }}
      {...props}
    />,
  );
