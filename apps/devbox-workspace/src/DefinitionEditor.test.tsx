...
const preview = {
  previewId: "native-edit",
  target: "project",
  before: view.project,
  after: { schemaVersion: 1, expectedPorts: [8080] },
  effectiveDiff: { added: [], changed: [], removed: [], expectedPortsChanged: true },
};
let description: Description;
beforeEach(async () => {
  vi.clearAllMocks();
  description = await describeProduct("workspace");
  call.mockImplementation(async (_d, _c, method) =>
    method === "preview_edit" ? preview : { saved: true, warning: null },
  );
});
afterEach(cleanup);
it("reviews before saving, retains the native base, and sends only the one-time token", async () => {
  const dirty = vi.fn(),
    saved = vi.fn();
  render(
    <DefinitionEditor description={description} view={view} disabled={false} onDirtyChange={dirty} onSaved={saved} />,
  );
  fireEvent.click(screen.getByRole("button", { name: "공유 정의 편집" }));
  fireEvent.change(screen.getByRole("textbox", { name: "설정 JSON" }), {
    target: { value: JSON.stringify(preview.after) },
  });
  ...
  expect(call).toHaveBeenCalledWith(
    ...
    "preview_edit",
    { target: "project", content: JSON.stringify(preview.after), editRevision: "native-base" },
    "overview",
  );
...
it("preserves a conflicted draft and requires a new review after a consumed token", async () => {
  call.mockImplementation(async (_d, _c, method) => {
    if (method === "apply_edit") throw new Error("파일 변경 충돌");
    return preview;
  });
  render(
    <DefinitionEditor
