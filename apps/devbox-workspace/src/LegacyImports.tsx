...
import LegacyTemplateImport,{type ImportedTemplate} from "./LegacyTemplateImport";
...
export default function LegacyImports({selected=false,existingProfiles=[],existingTemplates=[],onImported=async()=>{},onWorkspaceReview,disabled=false}:{selected?:boolean;existingProfiles?:ImportedProfile[];existingTemplates?:ImportedTemplate[];onImported?:()=>Promise<void>;onWorkspaceReview?:(jobId:string)=>void;disabled?:boolean}) {
  ...
  async function act(method:string,args:Record<string,unknown>) {
    if(busy||profileBusy||templateBusy)return;
  ...
  return <section aria-label="기존 설정 보관" aria-busy={busy||profileBusy||templateBusy||active(job)}>
    <h2>기존 설정 보관</h2>
    <p>기존 앱의 설정과 미저장 복구 내용을 읽어 별도로 보관합니다. 원본 파일은 유지됩니다.</p>
    <label>기존 앱 <select value={source} disabled={busy||profileBusy||templateBusy||active(job)} onChange={event=>setSource(event.target.value as Source)}>{Object.entries(sources).map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label>
    <button disabled={busy||profileBusy||templateBusy||active(job)} onClick={()=>void act("prepare_legacy_snapshot",{source})}>설정 확인 및 보관</button>
    <button disabled={busy||profileBusy||templateBusy} onClick={()=>void act("legacy_snapshot_job",{})}>보관 상태 새로 고침</button>
    ...
    {catalog.snapshots.length>0&&<section aria-label="이전 보관 기록">
      ...
      <ul>{catalog.snapshots.map((entry,index)=><li key={entry.id}>
        ...
        <button disabled={busy||profileBusy||templateBusy||active(job)||!!entry.issue||!entry.manifest} aria-label={`보관 ${index+1} 내용 다시 확인`} onClick={()=>void act("verify_legacy_snapshot",{snapshotId:entry.id})}>내용 다시 확인</button>
      </li>)}</ul>
    </section>}
    ...
    {job&&<>
      ...
      {job.phase==="ready"&&job.manifest&&<>
        ...
        {job.source==="workbench"&&job.manifest.files.some(file=>file.name==="project-profiles.json"&&!file.issue)&&(selected?<LegacyProfileImport key={job.id} jobId={job.id} existing={existingProfiles} onImported={onImported} onBusyChange={setProfileBusy} disabled={disabled||busy||templateBusy}/>:<p>Workspace를 시작한 뒤 보관한 프로필 가져오기를 검토할 수 있습니다.</p>)}
        {job.source==="workbench"&&job.manifest.files.some(file=>file.name==="profile-templates.json"&&!file.issue)&&(selected?<LegacyTemplateImport key={`templates-${job.id}`} jobId={job.id} existing={existingTemplates} onImported={onImported} onBusyChange={setTemplateBusy} disabled={disabled||busy||profileBusy}/>:<p>Workspace를 시작한 뒤 보관한 템플릿 가져오기를 검토할 수 있습니다.</p>)}
        {selected&&onWorkspaceReview&&job.source==="code-pad"&&job.manifest.files.some(file=>file.name==="session.json"&&!file.issue)&&<LegacyWorkspace key={job.id} jobId={job.id} disabled={disabled||busy||profileBusy||templateBusy} onReview={onWorkspaceReview}/>}
      </>}
    </>}
  </section>;
}
