...
type Job={id:string;source:Source;phase:"reading"|"preserving"|"ready"|"cancelled"|"failed";snapshotId:string|null;manifest:{files:Entry[];missing:string[]}|null;issue:string|null};
...
const active=(job:Job|null)=>job?.phase==="reading"||job?.phase==="preserving";
...
export default function LegacyImports() {
  ...
  async function act(method:string,args:Record<string,unknown>) {
    ...
    try {
      ...
      if(alive.current&&epoch.current===current)setJob(value);
  ...
  return <section aria-label="기존 설정 보관" aria-busy={busy||active(job)}>
    <h2>기존 설정 보관</h2>
    <p>기존 앱의 설정과 미저장 복구 내용을 읽어 별도로 보관합니다. 원본 파일은 유지됩니다.</p>
    <label>기존 앱 <select value={source} disabled={busy||active(job)} onChange={event=>setSource(event.target.value as Source)}>{Object.entries(sources).map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label>
    <button disabled={busy||active(job)} onClick={()=>void act("prepare_legacy_snapshot",{source})}>설정 확인 및 보관</button>
    <button disabled={busy} onClick={()=>void act("legacy_snapshot_job",{})}>보관 상태 새로 고침</button>
    {error&&<p role="alert">{error}</p>}
    {job&&<>
      <h3>{sources[job.source]}</h3>
      {active(job)&&<><p role="status">{job.phase==="reading"?"기존 설정을 확인하고 있습니다…":"확인한 설정을 보관하고 있습니다…"}</p><button disabled={busy} onClick={()=>void act("cancel_legacy_snapshot",{jobId:job.id})}>보관 취소</button></>}
      ...
      {job.phase==="ready"&&job.manifest&&<>
        <p role="status">설정 보관이 완료되었습니다.</p>
