...
it("revalidates a diagnostic before navigating and preserves an unknown column", async () => {
  ...
  await waitFor(() =>
    expect(onFile).toHaveBeenCalledWith(expect.objectContaining({ relativePath: "src/a.ts", line: 3, column: null })),
  );
  expect(call.mock.calls.find(([, , method]) => method === "resolve")?.[3]).toEqual({
    id: "problem",
    revision: "revision",
    log: false,
  });
...
it("retains unavailable evidence but prevents stale navigation", async () => {
  call.mockResolvedValue({
    ...snapshot,
    problems: [{ ...problem, stale: true }],
    sources: [{ source: "lsp", state: "unavailable", truncated: false }],
  });
...
it("discards a late snapshot and late navigation after switching worktrees", async () => {
  ...
  view.rerender(
    <Problems description={{ ...description, context: { ...context, worktreeId: "second" } }} {...props} />,
  );
  await act(async () => {
    finish?.({ context, target: problem.target });
  });
...
it("opens a revalidated port selection without requesting a process action", async () => {
  call.mockImplementation(async (_description, _component, method) =>
    method === "snapshot"
      ? { ...snapshot, problems: [{ ...problem, target: { kind: "port", port: 8080 } }] }
      : { context, target: { kind: "port", port: 8080 } },
  );
  const onRuntime = vi.fn();
  render(
    <Problems description={description} onFile={vi.fn()} onLog={vi.fn()} onRuntime={onRuntime} navigate={vi.fn()} />,
  );
  ...
  expect(
    call.mock.calls.every(
      ([, component, method]) => component === "workspace.problems" && ["snapshot", "resolve"].includes(method),
    ),
  ).toBe(true);
});
