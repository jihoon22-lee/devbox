...
export interface Worktree {
  ...
  projectId: string;
  revision: number;
  binding: { root: string; target: ProjectContext["target"] };
...
export interface Registry {
  ...
  importedProfileBindings?: ProfileBinding[];
}
...
const discoveryLabels = {
  ...
  linkedWorktree: "기존 프로젝트의 연결된 작업 폴더입니다.",
  aliasOrMove: "기존 프로젝트의 경로가 변경되었습니다.",
  replacedRoot: "등록된 경로의 폴더가 교체되었습니다.",
};
...
export default function RegistryGate({
  ...
  const review =
    ...
    ["project", "worktree"].includes(incoming.review.target.entity)
      ? incoming.review
      : null;
  ...
  useEffect(() => {
    if (registry) onSnapshot?.(registry);
  }, [registry, onSnapshot]);
  useEffect(() => {
    if (status.phase === "selected") onReady?.();
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!disposed && alive.current && loadId.current === requestId)
          setError(cause instanceof Error ? cause.message : "정보를 불러오지 못했습니다.");
      }
    }
    void load();
    return () => {
  ...
  useEffect(() => {
    ...
    void act(async () => {
      ...
      setRoot(suggestedRoot.path);
      setName(suggestedRoot.name);
      setTemplateId("");
      const next =
    ...
    <section className="workspace-registry" aria-label="프로젝트 관리" aria-busy={busy}>
      ...
      {error && (
        <>
          ...
          <button disabled={busy} onClick={() => void act(status.phase === "setup" ? startRegistry : refresh)}>
            {status.phase === "setup" ? "다시 시도" : "프로젝트 정보 다시 읽기"}
          </button>
        </>
      )}
      ...
      {status.phase === "setup" && !error && <p role="status">프로젝트 목록을 준비하고 있습니다…</p>}
      {status.phase === "selected" && !setupOnly && (
        <>
          {editing && (
            <p role="status">편집 내용을 저장하거나 진행 중인 작업을 마친 뒤 프로젝트를 변경할 수 있습니다.</p>
          )}
          {context && (
            <p>
              현재 선택한 작업 폴더:{" "}
              {registry?.worktrees.find((tree) => tree.id === context.worktreeId)?.binding.root ?? "목록 확인 중"}{" "}
              <button
          ...
          {!setupOnly && (
            ...
            >
              {!!registry?.importedTemplates?.length && (
                <label>
                  프로젝트 템플릿{" "}
                  <select
                    aria-label="프로젝트 템플릿"
                  ...
                  >
                    ...
                    {registry.importedTemplates
                      .filter((entry) => !entry.archived)
                      .map((imported, index) => (
                        <option key={imported.id} value={imported.id}>
                          {imported.template.name}
                          {registry.importedTemplates!.filter((entry) => entry.template.name === imported.template.name)
                            .length > 1
                            ? ` · 보관 항목 ${index + 1}`
                            : ""}
                        </option>
              ...
              {registry?.importedTemplates?.find((imported) => imported.id === templateId) && (
                <TemplateMetadata
                  template={registry.importedTemplates.find((imported) => imported.id === templateId)!.template}
                />
              )}
          ...
          <button
            disabled={setupOnly || busy || !!preview || editing}
            aria-expanded={wslOpen}
            onClick={() => {
              if (!wslOpen) {
                setWslBusy(true);
              }
              setWslOpen((value) => !value);
            }}
          >
            {wslOpen ? "WSL 폴더 입력 닫기" : "WSL 프로젝트 추가"}
          </button>
          {wslOpen && (
            <Suspense fallback={<p role="status">WSL 폴더 입력 화면을 불러오고 있습니다…</p>}>
              <WslProjectForm
          ...
          {preview && (
            <section aria-label="프로젝트 등록 확인">
              <h2>등록 확인</h2>
              <p>{discoveryLabels[preview.discovery.kind]}</p>
              <p>{preview.binding.root}</p>
              <p>명령 실행에 대한 신뢰는 별도로 확인합니다.</p>
              {preview.templateProfile && (
                <>
                  <p>선택한 템플릿으로 아래 프로필을 만들고 이 폴더에 연결합니다.</p>
                  <ProfileMetadata profile={preview.templateProfile.profile} />
                </>
              )}
              {preview.importedProfileId && (
                <p>
                  가져온 프로필을 이 폴더에 연결합니다. 프로필의 포트 설정은 프로젝트·로컬 설정이 없는 경우 기본값으로
                  사용합니다.
                </p>
              )}
              ...
              <button
                disabled={
                  busy || !name.trim() || (editing && ["aliasOrMove", "replacedRoot"].includes(preview.discovery.kind))
                }
                onClick={() =>
                  void act(async () => {
              ...
              >
                {["aliasOrMove", "replacedRoot"].includes(preview.discovery.kind) ? "경로 다시 연결" : "등록"}
              </button>
              <button disabled={busy} onClick={() => void act(cancelPreview)}>
                취소
              </button>
            </section>
          )}
          {review && (
            <section aria-label="프로젝트 열기 검토">
              <p>
                {review.label} 프로젝트의 연결을 확인해 주세요. 현재 프로젝트는 프로젝트 선택 버튼을 누른 뒤 바뀝니다.
              </p>
              {registry &&
                review.context &&
                !registry.worktrees.some(
                  (tree) => tree.id === review.context?.worktreeId && tree.revision === review.context.revision,
                ) && (
                  <p role="alert">
                    요청한 연결이 변경되었거나 삭제되었습니다. 현재 목록에서 연결을 다시 확인해 주세요.
                  </p>
                )}
              <button onClick={incoming.clear}>검토 닫기</button>
            </section>
          )}
          ...
          {registry?.projects.map((project) => (
            <section
              ...
              data-command-review={review?.context?.projectId === project.id || undefined}
            >
              <h2>{project.name}</h2>
              <button disabled={setupOnly || busy} onClick={() => setRename({ id: project.id, name: project.name })}>
                이름 변경
              </button>
              {rename?.id === project.id && (
                ...
                >
                  <label htmlFor="workspace-rename">새 이름</label>
                  <input
                    id="workspace-rename"
                    required
                    maxLength={120}
              ...
              {registry.worktrees
                ...
                .map((worktree) => (
                  <div key={worktree.id} data-command-review={review?.context?.worktreeId === worktree.id || undefined}>
                    ...
                    <p>
                      {worktree.trustedDigest
                        ? "실행 정의 검토 기록이 있습니다. 현재 상태는 프로젝트 설정에서 확인하세요."
                        : "실행 신뢰 확인 전"}
                    </p>
                    <button
                      disabled={
                        busy ||
                        editing ||
                        (context?.worktreeId === worktree.id && context.revision === worktree.revision)
                      }
                      onClick={() =>
                        void act(async () => {
                    ...
                    <button disabled={setupOnly || busy || editing} onClick={() => setRemove(worktree)}>
                      등록 해제
                    </button>
                    {remove?.id === worktree.id && (
                      <section aria-label="등록 해제 확인">
                        <p>이 작업 폴더의 등록을 해제합니다. 실제 폴더와 Git 파일은 보존됩니다.</p>
                        <button
                          disabled={busy || editing}
                          onClick={() =>
                            void act(async () => {
          ...
          {registry && !setupOnly && (
            <>
              <button
                ...
                disabled={busy || editing || !!preview}
                onClick={() => setTemplatesOpen((value) => !value)}
              >
                {templatesOpen ? "템플릿 관리 닫기" : "템플릿 관리 열기"}
              </button>
              {templatesOpen && (
                <Suspense fallback={<p role="status">템플릿 관리 화면을 불러오고 있습니다…</p>}>
                  <WorkspaceTemplateManager
                    registry={registry}
                    disabled={operationBusy || editing || !!preview || !!rename || !!remove || !!unlinkProfile}
                    onSaved={refresh}
          ...
          {(registry?.importedProfiles ?? []).map((imported) => (
            <section
              key={imported.id}
              aria-label={`${imported.local ? "프로필" : "가져온 프로필"} ${imported.profile.name}`}
            >
              <h2>
                {imported.local ? "프로필" : "가져온 프로필"}: {imported.profile.name}
              </h2>
              <ProfileMetadata profile={imported.profile} />

              <p>환경 설정과 서비스 참조는 보관되었습니다. 실행 연결은 해당 기능에서 확인해야 합니다.</p>
              {!(registry?.importedProfileBindings ?? []).some((binding) => binding.importedId === imported.id) && (
                <p>아직 프로젝트 폴더에 연결하지 않았습니다.</p>
              )}

              {(registry?.importedProfileBindings ?? [])
                .filter((binding) => binding.importedId === imported.id)
                .map((binding) => (
                  <div key={binding.target}>
                    <p>
                      연결한 폴더: {registry?.worktrees.find((tree) => tree.id === binding.worktreeId)?.binding.root}
                    </p>
                    <button disabled={setupOnly || busy || editing} onClick={() => setUnlinkProfile(binding)}>
                      프로필 연결 해제
                    </button>
                    {unlinkProfile === binding && (
                      <section aria-label="프로필 연결 해제 확인">
                        <p>보관한 프로필과 프로젝트 등록을 유지하고 둘 사이의 연결을 해제합니다.</p>
                        <button
                          disabled={busy || editing}
                          onClick={() =>
                            void act(async () => {
                              await registryCall("unbind_imported_profile", {
                                revision: registry!.revision,
                        ...
                        <button disabled={busy} onClick={() => setUnlinkProfile(null)}>
                          취소
                        </button>
                      </section>
                    )}
