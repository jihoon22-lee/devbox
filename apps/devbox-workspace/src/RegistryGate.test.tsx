...
const preview = {
  ...
  binding: { root: "C:\\fixture\\프로젝트", target: { kind: "windows" } },
  discovery: { kind: "newProject" },
};
afterEach(cleanup);
beforeEach(() => {
  call.mockReset();
...
it("selects only an explicit native context and refreshes after successful admission", async () => {
  ...
  call.mockImplementation(async (_component, method) =>
    method === "status"
      ...
      : method === "snapshot"
        ? registry
        : method === "legacy_snapshot_job"
          ? null
          : method === "list_window_history"
            ? { items: [], unrecognized: 0 }
            : method === "list_legacy_snapshots"
              ? { snapshots: [], unrecognized: 0 }
              : {},
  );
...
it("reviews a concrete template profile and registers only after explicit approval", async () => {
  const template = {
    ...
    gitRoot: null,
    expectedPorts: [4321],
    runManagerServiceIds: ["saved-service"],
  };
  const imported = { id: "native-template", sourceSnapshotId: "snapshot", template };
  const candidate = {
    id: "new-native-profile",
    sourceSnapshotId: "snapshot",
    sourceTemplateId: imported.id,
    profile: { ...template, id: "new-profile-id", windowsPath: preview.binding.root, environment: null },
  };
  call.mockImplementation(async (_component, method) => {
    if (method === "status") return { phase: "selected" };
  ...
  const { container } = render(<RegistryGate />);
  fireEvent.change(await screen.findByRole("combobox", { name: "프로젝트 템플릿" }), {
    target: { value: imported.id },
  });
  expect(
    within(screen.getByRole("combobox", { name: "프로젝트 템플릿" }).closest("form")!).getByText("4321"),
  ).toBeTruthy();
  ...
  expect(call).toHaveBeenCalledWith("workspace.registry", "preview_template_profile_windows", {
...
it("loads WSL discovery only after opening its folder form", async () => {
  ...
  call.mockImplementation(async (component, method, ...args) =>
    method === "list_wsl_distros" ? [] : original(component, method, ...args),
  );
  render(<RegistryGate />);
...
it("reviews a Source worktree proposal in its WSL distro without starting it", async () => {
  ...
  expect(call.mock.calls.some(([, method]) => method === "preview_windows" || method === "apply_registration")).toBe(
    false,
  );
  ...
  call.mockImplementation(async (_component, method) =>
    method === "status"
      ...
      : method === "snapshot"
        ...
        : method === "legacy_snapshot_job"
          ? null
          : { snapshots: [], unrecognized: 0, items: [] },
  );
  const ready = vi.fn();
  render(<RegistryGate onReady={ready} />);
...
it("does not retry a failed automatic start until requested", async () => {
  call.mockImplementation(async (_component, method) => {
    if (method === "status") return { phase: "setup" };
    throw new Error("저장소 준비 실패");
  });
  render(<RegistryGate />);
