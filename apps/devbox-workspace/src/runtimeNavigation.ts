...
export function sameRuntimeContext(value: unknown, current: ProjectContext | null): boolean {
  ...
  return (
    ...
    value.worktreeId === current.worktreeId &&
    value.revision === current.revision &&
    value.target.kind === current.target.kind &&
    (value.target.kind !== "wsl" ||
      (current.target.kind === "wsl" && value.target.distroId === current.target.distroId))
  );
}
...
export function runtimeDestination(
  ...
  if (
    !object(value) ||
    !keys(value, ["route", "context", "fromRoute"]) ||
    value.fromRoute !== "runtime" ||
...
export function runtimeLogRequest(
  value: unknown,
  context: ProjectContext | null,
  route: string,
): (RuntimeLogOpenRequest & { source: Extract<RuntimeLogOpenRequest["source"], { kind: "runtimeRun" }> }) | null {
  if (
    !object(value) ||
    !keys(value, ["id", "source", "context", "fromRoute"]) ||
    !["tasks", "runtime"].includes(String(value.fromRoute)) ||
    route !== value.fromRoute ||
    !sameRuntimeContext(value.context, context) ||
  ...
  if (
    !object(source) ||
    !keys(source, ["kind", "runId", "stream", "revision"]) ||
  ...
  return {
    id: value.id,
    source: { kind: "runtimeRun", runId: source.runId, stream: source.stream, revision: source.revision },
  };
}
...
export function runtimeDiagnostic(
  ...
  context: ProjectContext | null,
  route: string,
): { id: string; relativePath: string; line: number; column: number | null } | null {
  if (
    !object(value) ||
    !keys(value, ["id", "relativePath", "line", "column", "runId", "revision", "context", "fromRoute"]) ||
    route !== "tasks" ||
    value.fromRoute !== "tasks" ||
    !context ||
    !sameRuntimeContext(value.context, context) ||
    typeof value.id !== "string" ||
    !/^[a-f0-9]{32}$/.test(value.id) ||
    ...
    value.relativePath.length > 32768 ||
    /[\\:\x00-\x1f\x7f]/.test(value.relativePath) ||
    value.relativePath.split("/").some((part) => !part || part === "." || part === "..") ||
    !Number.isSafeInteger(value.line) ||
    (value.line as number) < 1 ||
    (value.column !== null && (!Number.isSafeInteger(value.column) || (value.column as number) < 1))
  )
    return null;
  return {
    id: value.id,
...
export function terminalLogRequest(value: unknown, context: ProjectContext | null): RuntimeLogOpenRequest | null {
  if (
    !object(value) ||
    !keys(value, ["id", "source", "context"]) ||
    (value.context !== null && !sameRuntimeContext(value.context, context)) ||
    typeof value.id !== "string" ||
    !/^[a-f0-9]{32}$/.test(value.id) ||
    !object(value.source)
  )
  ...
  const source = value.source;
  if (
    typeof source.distro !== "string" ||
    source.distro.length < 1 ||
    source.distro.length > 128 ||
    /[\x00-\x1f\x7f]/.test(source.distro)
  )
    return null;
  if (
    source.kind === "wslFile" &&
    keys(source, ["kind", "distro", "path"]) &&
    ...
    return { id: value.id, source: { kind: "wslFile", distro: source.distro, path: source.path } };
  if (
    source.kind === "wslJournal" &&
    keys(source, ["kind", "distro", "unit"]) &&
    (source.unit === null ||
      (typeof source.unit === "string" &&
        !source.unit.startsWith("-") &&
        /^[A-Za-z0-9_.@:-]{1,128}$/.test(source.unit)))
  )
    return {
      id: value.id,
      source: {
