...
export default function WslProjectForm({disabled,onBusyChange,onReviewed,templates=[]}:{disabled:boolean;onBusyChange:(busy:boolean)=>void;onReviewed:(preview:Preview,name:string)=>void;templates?:ImportedTemplate[]}) {
    ...
    <form onSubmit={event=>{event.preventDefault();if(disabled||busy||!selected||(!selected.running&&!start))return;activity(true);setError("");
      void (async()=>{
        try {
          ...
          const preview=template?await nativeCall<Preview>("workspace.registry","preview_template_profile_wsl",{templateId:template.id,distroId:id,root,name:template.template.name,startStopped})
      ...
      {<><label>WSL 프로젝트 템플릿 <select value={templateId} disabled={disabled||busy} onChange={event=>setTemplateId(event.target.value)}>
