...
export default function WslProjectForm({
  disabled,
  onBusyChange,
  onReviewed,
  templates = [],
  ...
  function activity(value: boolean) {
    setBusy(value);
    onBusyChange(value);
  }
  async function refresh() {
    ...
    try {
      ...
      if (alive.current && request === generation.current)
        setError(cause instanceof Error ? cause.message : "WSL 목록을 읽지 못했습니다.");
    } finally {
  ...
  return (
    <section aria-label="WSL 프로젝트 폴더" aria-busy={busy}>
      <p>등록된 배포판에서 Linux 폴더를 확인합니다.</p>
      {error && <p role="alert">{error}</p>}
      <button disabled={disabled || busy} onClick={() => void refresh()}>
        WSL 목록 새로 고침
      </button>
      ...
      <form
        onSubmit={(event) => {
          ...
          if (disabled || busy || !selected || (!selected.running && !start)) return;
          activity(true);
          setError("");
          void (async () => {
            try {
      ...
      >
          ...
          >
            ...
            {distros.map((distro) => (
              <option key={distro.id} value={distro.id}>
                {distro.name} · {distro.running ? "실행 중" : "중지됨"}
              </option>
            ))}
          </select>
        </label>
        {
          <>
            ...
            {template && <TemplateMetadata template={template.template} />}
          </>
        }
        <label>
          Linux 프로젝트 폴더{" "}
