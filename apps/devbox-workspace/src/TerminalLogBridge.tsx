...
export default function TerminalLogBridge({
  description,
  consumedId,
  onOpen,
}: {
  ...
  useEffect(() => {
    ...
    void listen("workspace://terminal-log-ready", () => void read())
      .then((stop) => {
        if (disposed) stop();
        else {
          unlisten = stop;
      ...
      .catch(() => {
        if (!disposed) setIssue("터미널 로그 연결을 시작하지 못했습니다.");
      });
    return () => {
      disposed = true;
      unlisten?.();
    ...
    void (async () => {
      for (let attempt = 0; attempt < 3 && !disposed; attempt++) {
        try {
          ...
          if (attempt === 2 && !disposed) setIssue("로그 요청의 완료 상태를 확인하지 못했습니다.");
        }
