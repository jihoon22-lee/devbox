...
export function issueMessage(issue: string): string {
  return issues[issue] ?? "작업을 완료하지 못했습니다. 상태를 확인하고 다시 시도해 주세요.";
}
...
export async function componentCall<T>(
  ...
  if (
    ["workspace.runtime", "workspace.processes", "workspace.process-actions", "workspace.logs"].includes(component) ||
    ...
    (component === "workspace.files" &&
      ["reconnect_wsl_files", "open_file", "send_editor_selection"].includes(method)) ||
