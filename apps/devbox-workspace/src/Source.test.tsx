...
const review = {
  ...
  sources: [{ path: "C:\\fixture\\.git\\config", kind: "config", digest: "a".repeat(64) }],
  executionKeys: ["credential.helper"],
  environmentKeys: ["SSH_ASKPASS"],
};
...
beforeEach(async () => {
  ...
  description = {
    ...
    context: { projectId: "project", worktreeId: "tree", revision: 1, target: { kind: "windows" } },
  };
...
it("requires explicit Git review and preserves the actual commit draft across revoke and reapproval", async () => {
  ...
  render(
    <StrictMode>
      <Source description={description} root="C:\\fixture" onBusyChange={busy} onDirtyChange={dirty} />
    </StrictMode>,
  );
  ...
  await waitFor(() =>
    expect((message.closest(".workspace-source-actions") as HTMLFieldSetElement).disabled).toBe(false),
  );
  await act(async () => {
    fireEvent.change(message, { target: { value: "keep this commit draft" } });
  });
  ...
  expect(
    call.mock.calls
      .filter(([, , method]) => method === "approve_trust")
      .every(
        ([, component, , args, route]) =>
          component === "workspace.source" &&
          route === "source" &&
          (args as { previewId: string }).previewId === "native-preview",
      ),
  ).toBe(true);
});
it("discards a late native review after unmount without approving it", async () => {
  ...
  const rendered = render(
    <Source description={description} root="C:\\fixture" onBusyChange={vi.fn()} onDirtyChange={vi.fn()} />,
  );
  await click("Git 실행 검토");
  rendered.unmount();
...
it("blocks review in the first committed frame while initial inspection is pending", async () => {
  ...
  call.mockImplementation(async (_description, _component, method) =>
    method === "trust_status"
      ...
      : { previewId: "native-preview", status: status() },
  );
  let firstFrameDisabled = false;
  function FirstFrame() {
    useLayoutEffect(() => {
      firstFrameDisabled = (screen.getByRole("button", { name: "Git 실행 검토" }) as HTMLButtonElement).disabled;
    }, []);
