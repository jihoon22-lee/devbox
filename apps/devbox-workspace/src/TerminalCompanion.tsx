...
function connect(): Promise<Peer> {
  initialization ??= invoke<Peer>("plugin:workspace|terminal_describe").then(async (peer) => {
    if (
      peer.handshake.product !== "workspace" ||
      peer.handshake.protocolVersion !== 1 ||
      (peer.context !== null && !isProjectContext(peer.context)) ||
      !peer.id ||
      !/^[a-f0-9-]{36}$/.test(peer.id)
    )
    ...
    const header = makeRequest(peer.handshake, "terminal", Date.now(), peer.context);
    const layout = await invoke<{ revision: string; layout: unknown }>("plugin:workspace|terminal_execute", {
      request: { header, method: "terminal_layout", args: {} },
    });
...
export default function TerminalCompanion() {
  ...
  if (error)
    return (
      <main>
        <p role="alert">터미널 창의 세션을 확인하지 못했습니다. Workspace의 터미널 화면에서 다시 열어 주세요.</p>
      </main>
    );
  if (!peer) return <p role="status">터미널 세션에 연결하고 있습니다…</p>;
  return (
    <Suspense fallback={<p role="status">터미널을 불러오고 있습니다…</p>}>
      <Terminal />
    </Suspense>
  );
}
