...
interface Session {id:string;context:ProjectContext;state:string}
const labels:Record<string,string>={preparing:"준비 중",active:"실행 중",stopping:"종료 중",stopped:"종료됨",interrupted:"복구 검토 필요"};
export default function Terminal({description}:{description:Description}) {
  ...
  return <section className="workspace-terminal-manager">
    ...
    <button disabled={busy||!description.context} onClick={()=>void open()}>현재 프로젝트의 터미널 열기</button>{" "}
    <button disabled={busy} onClick={()=>void action("terminal_sessions",{})}>새로 고침</button>
    {!description.context&&<p role="status">먼저 프로젝트와 작업 트리를 선택해 주세요.</p>}
    {issue&&<p role="alert">{issue}</p>}
    <ul>{sessions.map(session=><li key={session.id}>
      <span>{session.context.projectId} · {session.context.worktreeId} · {labels[session.state]??"상태 확인 필요"}</span>{" "}
