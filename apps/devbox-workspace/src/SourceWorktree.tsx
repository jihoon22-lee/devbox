...
export default function SourceWorktree({ description, enabled, onBusyChange, onDirtyChange, onPropose }: Props) {
  ...
  useEffect(() => {
    onDirtyChange(branch.length > 0 || target.length > 0);
  }, [branch, target, onDirtyChange]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (operation.current)
        void call("repo_local_cancel", { request: { operationId: operation.current } }).catch(() => {});
      onBusyChange(false);
      onDirtyChange(false);
    };
  ...
  async function act(action: () => Promise<void>) {
    ...
    try {
      ...
      if (alive.current) setError(cause instanceof Error ? cause.message : "작업 폴더 생성을 완료하지 못했습니다.");
    } finally {
      if (alive.current) {
  ...
  return (
    <section className="workspace-source-worktree" aria-label="새 Git 작업 폴더" aria-busy={busy}>
      <h2>새 작업 폴더</h2>
      ...
      >
        ...
        <input
          ...
          required
        />
        <button disabled={!enabled || busy || preview !== null || !branch.trim() || !target.trim()}>
          작업 폴더 생성 검토
        </button>
      ...
      {preview && (
        <section aria-label="작업 폴더 생성 확인">
          <p>저장소: {preview.root}</p>
          <p>새 branch: {preview.branch}</p>
          <p>새 폴더: {preview.targetDir}</p>
              ...
              void act(async () => {
                ...
                try {
                  ...
                  if (alive.current) {
                    ...
                    setBranch("");
                    setTarget("");
                    setNotice("작업 폴더를 생성했습니다. 등록할 폴더를 검토해 주세요.");
                  }
                } finally {
      ...
      {busy && operation.current && (
        <button
          ...
          onClick={() => {
            ...
            void call("repo_local_cancel", { request: { operationId } }).catch(() => {
              if (alive.current) setNotice("취소 요청을 확인하지 못했습니다. 작업 결과를 기다리는 중입니다.");
            });
          }}
        >
          생성 취소 요청
      ...
      {created && (
        <div>
          <p>{created}</p>
          <button type="button" disabled={busy} onClick={() => onPropose(created)}>
            생성한 폴더 등록 검토
          </button>
        </div>
      )}
