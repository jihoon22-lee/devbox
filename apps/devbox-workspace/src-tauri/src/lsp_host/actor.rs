...
use super::{approval::Snapshot, input};
...
pub(super) fn stops(method: &str) -> bool {
    matches!(method, "stop_language_server" | "stop_all_language_servers")
}
...
pub(super) fn admission(method: &str, args: Value) -> Result<(Option<String>, Vec<String>)> {
    match Method::parse(method, args)? {
        ...
        Method::LanguageServerStatuses {} | Method::LanguageServerLogs {} => Ok((None, vec![])),
...
fn control_error(error: LspManagerError) -> &'static str {
    match error {
...
async fn execute(manager: &LspManager, method: Method) -> Result<Value> {
    match method {
            ...
            Some(command)=receiver.recv()=>{
                ...
                tasks.spawn(async move {
                    ...
                    let operation=async move {
                        ...
                        let result=execute(&manager,command.method).await;
