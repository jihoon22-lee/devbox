...
/// Implemented only by native root observations, never deserialized from IPC.
pub trait RootLease: Send + Sync {
    fn root(&self) -> &Path;
    fn target(&self) -> &product_contract::ExecutionTarget;
    fn native_root_identity(&self) -> FilesystemIdentity;
    fn revalidate(&self) -> Result<()>;
}
pub type Scope<'a> = Option<(&'a ProjectContext, &'a dyn RootLease)>;
...
fn native_path(raw: &str) -> Result<PathBuf> {
    ...
    crate::windows_path::admit(&path).map_err(|_| "file_target_unavailable")?;
...
fn check_scope(scope: Scope<'_>, context: &Option<ProjectContext>, path: &Path) -> Result<()> {
    if let Some(expected) = context {
        ...
        if current != expected || &current.target != lease.target() || !within(lease.root(), path)?
        {
...
impl Object {
    fn open(path: &Path, directory: bool, admission: Admission) -> Result<Self> {
        admission(path).map_err(|_| "file_target_unavailable")?;
        let (handle, identity) =
    ...
    fn revalidate(&self, directory: bool) -> Result<()> {
        (self.admission)(&self.path).map_err(|_| "file_target_unavailable")?;
...
impl Grant {
    fn open(path: &Path, context: Option<ProjectContext>, admission: Admission) -> Result<Self> {
        ensure_no_links(path).map_err(|_| "unsafe_file_path")?;
        let original = Object::open(path, false, admission)?;
        let canonical =
            display_path(&std::fs::canonicalize(path).map_err(|_| "file_unavailable")?)?;
        let mut parents = Vec::new();
        let mut parent = canonical.parent();
        while let Some(path) = parent {
            parents.push(Object::open(path, true, admission)?);
            parent = path.parent();
        }
        let grant = Self {
            file: Object::open(&canonical, false, admission)?,
            parents,
            context,
        };
        ensure_no_links(path).map_err(|_| "unsafe_file_path")?;
...
impl FileOwner {
    ...
    pub fn protect_storage(&mut self, storage: ProtectedStorage) {
        self.protected = Some(storage);
    }
    ...
    pub fn session_path_eligible(&self, root: Option<&str>, raw: &str) -> bool {
    ...
    pub fn has_documents_under(&self, root: FilesystemIdentity) -> bool {
    ...
    pub fn approve_native_selection(&mut self, path: &Path) -> Result<String> {
        ...
        self.ensure_user_path(&path)?;
        let grant = Grant::open(&path, None, self.admission)?;
    ...
    pub fn save(&mut self, scope: Scope<'_>, request: SaveFileRequest) -> Result<SavedFileWire> {
        ...
        let next = Grant::open(&path, context, self.admission);
        match next {
    ...
    pub fn guard_editor_write(&self, context: &ProjectContext, raw: &str) -> Result<()> {
        let path = display_path(Path::new(raw))?;
...
mod tests {
    ...
    struct FixtureLease {
        observation: devbox_filesystem::project::ProjectObservation,
        target: product_contract::ExecutionTarget,
    }
    impl RootLease for FixtureLease {
        fn root(&self) -> &Path {
    ...
    fn probe_fixture(root: &Path) -> Result<FixtureLease> {
        #[cfg(windows)]
        let target = product_contract::ExecutionTarget::Windows;
        #[cfg(not(windows))]
        let target = product_contract::ExecutionTarget::Wsl {
            distro_id: "native-test-fixture".into(),
        };
        Ok(FixtureLease {
            observation: devbox_filesystem::project::ProjectObservation::capture(
                root,
                crate::windows_path::admit,
            )?,
            target,
        })
    }
    ...
    fn context(lease: &FixtureLease) -> ProjectContext {
        ProjectContext {
            project_id: "project".into(),
            worktree_id: "tree".into(),
            revision: 1,
