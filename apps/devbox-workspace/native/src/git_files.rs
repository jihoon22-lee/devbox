...
pub fn boundary(deadline: u64) -> Result<()> {
...
pub fn transport(project: &Path, path: &Path) -> Result<()> {
    transport_with_admission(project, path, crate::windows_path::admit)
}
pub fn transport_with_admission(project: &Path, path: &Path, admission: Admission) -> Result<()> {
    admission(path).map_err(|_| "git_source_transport_denied")?;
...
pub fn resolve(project: &Path, base: &Path, value: &Path, deadline: u64) -> Result<PathBuf> {
    resolve_with_admission(project, base, value, deadline, crate::windows_path::admit)
}
pub fn resolve_with_admission(
    ...
    transport_with_admission(project, &path, admission)?;
    for prefix in &traversed {
        transport_with_admission(project, prefix, admission)?;
    }
    for prefix in &traversed {
        boundary(deadline)?;
...
impl Default for GitFiles {
    fn default() -> Self {
        Self::new(crate::windows_path::admit)
    }
}
impl GitFiles {
    ...
    fn pin(&mut self, path: &Path, directory: bool, deadline: u64) -> Result<()> {
        boundary(deadline)?;
        (self.admission)(path).map_err(|_| "git_source_transport_denied")?;
    ...
    fn ancestors(
        ...
        transport_with_admission(project, path, self.admission)?;
        ...
        for part in paths {
            ...
            boundary(deadline)?;
            (self.admission)(part).map_err(|_| "git_source_transport_denied")?;
