...
use crate::{files::Admission, git_files};
...
impl WorktreeTarget {
    ...
    pub fn capture_with_admission(
        ...
        let path = git_files::resolve_with_admission(
            project,
            project,
            Path::new(raw),
            deadline,
        ...
        git_files::transport_with_admission(project, &original_parent, admission)?;
        ...
        let parent = crate::storage_paths::display(
            &fs::canonicalize(&original_parent).map_err(|_| "worktree_target_unavailable")?,
        )?;
        git_files::transport_with_admission(project, &parent, admission)?;
    ...
    pub fn revalidate(&self, deadline: u64) -> Result<()> {
        ...
        git_files::transport_with_admission(&self.project, &self.path, self.admission)?;
