...
구현했다. 후보 기준 커밋은 `14716d0d`이고, 현재 `origin/main`(Launcher bootstrap 및
후속 통합 변경 포함)을 no-commit으로 worktree에 반영한 뒤 전용 브랜치
`feat/everything-plus/advanced-filters-saved-query`에서 작업했다. 따라서 이 worktree에는
Launcher의 현재 consumer fixture도 함께 보이며, 별도 Log Lens/window-state 기능은 이
후보의 사용자 흐름에 추가하지 않았다.
...
- 현재 Launcher query payload parser는 `payload.filter`를 동일한 bounded `QueryFilter`로
  소비·재검증해 `query-filter-v1` AppLink로 전달한다. filter를 모르는 구버전 수신기는
  기존 `--query` text만 사용해 안전하게 downgrade된다. Query payload envelope의 외부
  필드는 구버전 Launcher가 새 optional field를 무시할 수 있지만, filter object 자체는
  unknown field를 거부해 계약 밖 조건을 silently 적용하지 않는다.
...
- `apps/everything-plus/src-tauri/src/core/db.rs`
  ...
  - 기존 v0.4.x의 missing/zero `files.root_id`를 등록 root 중 가장 깊은 항목으로
    ...
    content-status index를 추가하고, ownership 없는 row는 검색 projection에서 제외한다.
...
- `apps/catalog.json`
  - 현재 main의 `catalogRevision: 9`를 유지하면서 Everything+의 producer capability
...
- `docs/roadmap.md`
  - #349/#350 cohesive PR 후보와 현재 Launcher consumer의 filter 적용/legacy downgrade를
    기록했다.
...
The task prohibited full workspace builds/tests. Focused Rust unit suites were run with one
worker; frontend execution was not available because this worktree has no installed `vitest`.
The full `cargo test`, `cargo check`, `pnpm build`, frontend test, Windows packaged smoke, and
CI gates remain required before PR/merge.
...
pnpm exec vitest ...
not run: local `vitest` executable is unavailable

git diff --check
exit code: 0

jq -e . apps/catalog.json && python3 -c 'import tomllib; tomllib.load(open("apps/everything-plus/src-tauri/Cargo.toml","rb"))'
exit code: 0
```

No commit, rebase, push, PR, full build, or workspace-wide test was performed. The worktree is
intentionally dirty for the parent agent to review and integrate.
...
2. Windows W3 validation (actual Tauri opener, reparse behavior, ACL/permission failure)
   and the full repository CI gates still need to run on the parent PR.
