...
## WSL1 syscall 차이와 실제 파일 동작

- `c34bdac` [일반 CI](https://github.com/jihoon22-lee/devbox/actions/runs/34392522451) PASS. [제품 검사](https://github.com/jihoon22-lee/devbox/actions/runs/34392522523)는 WSL 첫 root observation 하나만 FAIL이며, Windows native Workspace **141개**, 전체 packaged shell/journal 이후 context clear, API Studio/Knowledge 수용과 installer coexistence는 PASS다.
- 전용 로컬 Windows TEMP에 최소 합성 rootfs를 등록해 WSL1을 실제 실행했다. `statx`/`renameat2`는 ENOSYS(38), descriptor `fdinfo`는 ENOENT이고 `mountinfo`와 native filesystem metadata는 제공된다. 해당 inode가 backing 파일의 실제 Windows file ID와 같고, 배포판 재시작 후 유지되며 폴더 교체 후 달라지는 것을 대조했다. 각각의 임시 배포판 등록과 소유 디렉터리는 정리했다. 기존 배포판/파일을 fixture로 사용하지 않았다.
- helper는 wslfs + statx ENOSYS에만 비어 있지 않은 NTFS sequence/file ID를 사용한다. [NTFS reference 구조](https://learn.microsoft.com/en-us/windows/win32/devnotes/mft-segment-reference)와 [GNU wslfs 식별값](https://www.gnu.org/software/coreutils/filesystems.html)을 함께 확인했다. Windows GUID/backing-object 경계는 유지하고 다른 filesystem/errno/잘못된 birth-time은 거부한다. WSL1 fdinfo 부재 시에도 같은 root device/fsid와 모호하지 않은 mountinfo를 검증한다.
- `renameat2`가 없는 wslfs에서만 retained parent fd의 `linkat`으로 기존 대상을 덮어쓰지 않고 새 이름을 만든다. 취소/권한·내용·원본/대상 identity 재검사 후 `unlinkat`으로 원본 이름을 제거한다. 두 단계 사이의 실패/종료는 같은 파일의 두 이름을 보존할 수 있으며 원자적 namespace rename으로 주장하지 않는다. 재시도/대상 자동 삭제는 없고 명시적 확인 안내를 반환한다. 다른 Linux filesystem과 Windows의 기존 atomic rename 경로는 유지한다.
- 충돌·중간 취소/두 경로 보존 회귀를 포함한 Code Pad file tests, helper **21개·실제 GNU pipe 5개**, 관련 Linux strict Clippy·musl static release build PASS(**64.592초 / sampled RSS peak 2,072,088,576 bytes / 8 GiB**). 최초 GNU Clippy의 불필요 cast와 musl f_type signedness 차이를 수정하고 두 타깃으로 재검증했다.
